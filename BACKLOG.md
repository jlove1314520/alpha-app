...
- **pull-to-refresh手勢**（2026-08-28登錄，使用者這輪明確要求(b)項，
  refreshTap()按鈕已完成，這是同一個任務的另一半）：頁面頂端下拉觸發跟
  按鈕相同的重整動作，需要touch事件處理+視覺指示（下拉進度/放開觸發的
  轉圈動畫），尚未開始。
...
- **B24：選股驗證——回答「到底選不選得到賺錢/翻倍的股票」**（2026-08-28
  ...
  2. **翻倍率指標**（使用者核心關切）：翻倍率=Top20中12個月內最高漲幅
     ...
     數字呈現，不能只報好的一面）。
  3. **前瞻選股台帳**（今晚就開始累積，最誠實的證據）：新增
     `data/picks_ledger.json`——每日評分產出後自動快照三榜各Top20（代號/
     名稱/當日分數/當日收盤價/時間戳），排程在T+5/T+20/T+60/T+120回填實際
     報酬與相對大盤超額。**鐵律：只能事前快照，嚴禁事後補建**（那是回測
     不是前瞻，跟B19訊號台帳同一條鐵律）。App選股頁/策略分頁新增「選股
     成績單」區塊，顯示歷史快照的實際命中率/平均報酬/翻倍數，標「前瞻
     實測記錄，非回測」。
...
- **B23：動能榜新增量價配合度因子＋創新高因子＋歷史/量能深度延伸**
  （2026-08-27登錄，使用者這輪明確指示，順序：歷史+量能深度 → 創新高 →
  量價配合度 → B16回測，尚未開始）：
  1. **歷史+量能深度延伸**：`price_history.json`目前90天上限、且部分股票
     research端FinMind快取已經停在2024-12-31（見上方還原權息修正發現的
     真bug）——量價因子需要更長且連續的量能序列，這步要先確保深度/連續性
     足夠，必要時延伸`PRICE_HISTORY_DAYS`或改善回補來源。
  2. **創新高因子**：近60日價格創階段高。
  3. **量價配合度因子**（0-10分，加入`scores_momentum`，使用者原話逐字
     記錄）：
     - 加分：(a) 吸收比——近20日「上漲日均量÷下跌日均量」>1.3；
       (b) 量能梯度——突破段均量÷突破前20日均量，1.5~3倍為健康區；
       (c) 回檔量縮——近期回檔日均量÷上漲段均量<0.6（洗盤特徵）。
     - 扣分/警訊（反映在分數並顯示標籤）：(d) 價漲量縮背離——價格創近60日
       高但當波量能<前波高點量能0.7，標「量價背離，續航存疑」；
       (e) 高檔爆量不漲——單日量>20日均量2.5倍且當日漲幅<1%或收黑，且
       股價位於近60日高檔區(>80百分位)，重扣＋標「⚠️ 高檔爆量滯漲，出貨
       嫌疑」；(f) 派發訊號——近10日下跌日均量>上漲日均量1.3倍，扣分＋標
       「下跌放量」。
     - 與創新高因子聯動：創新高＋量價配合度高＝兩者相乘（最強組合）；
       創新高但量價配合度低（無量創高/高檔爆量）＝創新高因子分數打折，
       不讓假突破拿高分（使用者原話：「寧可漏掉，不要騙自己」）。
  4. **紀律（使用者原話）**：全部門檻（1.3倍/2.5倍/0.7…）目前是經驗值，
     未經統計驗證，頁面要標「參數未驗證」，B16回測時要對這些門檻做參數
     敏感度掃描（±30%掃一遍，只有單點有效就是曲線擬合，判定為過擬合）。
     量價因子＋創新高因子＋相對強度三者相關性高，合計權重不得超過動能榜
     55%，其餘留給籌碼/族群/資金流。
  尚未開始。
