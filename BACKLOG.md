...
## 📝 登記待辦（2026-09-15，週六.七，只登記、未開工）

來源：PENDING_QUEUE.md「週六實測九項」第七項原話——「登記 P1（先不做，寫進
BACKLOG 附估時）：美股即時篩選（yfinance + IBKR 掃描器 API，誠實標回測資料
品質）；ADR 溢價；法說會列表＋MOPS 簡報 PDF 連結（Phase 2 一併）；群益下單
adapter（等總司令確認資金所在）」。以下僅登記＋估時，未開工、未寫任何程式碼。

1. **【P1】美股即時篩選**（yfinance批次掃描＋IBKR Scanner API，誠實標回測
   資料品質）。估時：1～2工作日（8～16小時）。範圍：設計篩選條件（成交量/
   漲跌幅/市值門檻等）、串IBKR `reqScannerSubscription`或yfinance批次查詢、
   結果寫入面板。**風險**：yfinance批次查詢頻率上限未查證（見CLAUDE.md「搜尋
   紀律：三來源查證」，動工前需先查）；IBKR Scanner是否受paper帳戶限制未知，
   需先用`ibkr_quotes.py`既有連線實測一次再估準確工時。
2. **【P1】ADR溢價**（美股ADR價格 vs 台股原股換算價的溢價率）。估時：
   4～6小時。範圍：需要ADR對應的換股比例（通常固定，需查證來源，可能要人工
   維護一份少量對照表，如台積電ADR:原股=1:5）、ADR即時價（IBKR或yfinance）、
   台股原股價（既有quotes_sinopac/quotes_tw）、即時匯率（既有`data/fx.json`）
   四項合成溢價率＝`(ADR價×匯率/換股比例)/原股價-1`。**風險**：換股比例目前
   沒有現成資料源，需先查證是否有API或只能人工維護少量清單（台股有ADR的公司
   本來就不多，人工維護可能是務實做法）。
3. **【P1】法說會列表＋MOPS簡報PDF連結**（Phase 2財報管線一併規劃，非獨立
   小工）。估時：本身1工作日（6～8小時，僅列表+PDF連結，不含PDF內容解析）；
   若要進一步解析PDF內容（OCR/文字擷取財務簡報重點），屬於另一個量級的工作，
   估時另計、待Phase 2規劃階段再評估。來源：MOPS公開資訊觀測站法說會日程
   （需查證是否有結構化API或只能解析HTML頁面）。
4. **【P1，待查證是否可行】群益下單adapter**（等總司令確認資金所在）。
   **現況已知**：`C:\alpha\CLAUDE.md`「重要決策與原因」已記載「群益/國泰台股
   目前沒有合適的下單API」——這是先前查證過的結論，不是本輪新估。**估時分兩
   種情境**：(a) 若群益實際上有唯讀/下單API但先前查證有遺漏，需重新三來源
   查證（官方文件/開發者入口/社群案例）確認可行性，查證本身估2～3小時；
   (b) 若確認無官方API可用，本項無法做，需總司令決定改用永豐（Shioaji，
   已有現成介接）或其他券商，屬於資金/帳戶配置決策，不是工程問題，本估時
   不含此決策後續的實作工時。
...
登記、不執行）：#20純毛利率因子(Gross Profitability,Novy-Marx)、
#21月營收意外漂移×低關注度(改造已FAIL的#14)、#22品質×營收加速×
法人吸籌複合訊號+低波動閘門、#23 Piotroski F-score當價值榜排雷閘門、
#24除權息季節行為效應。**使用者原始編號#18~#22跟馬拉松當時已自主佔用
的#18(短期反轉)/#19(跨市場美股隔夜外溢)衝突，重新編號成#20~#24**，
已在`HYPOTHESIS_QUEUE.md`排隊順序總結說明這個調整。

登記前查證過重複性：#20（GP水位）跟已FAIL的`f_gross_margin_stability`
（毛利率穩定度，`TRIALS_LEDGER.md`#67）是不同構造（水位vs波動度），
不是重複；其餘4條查證`HYPOTHESIS_QUEUE.md`/`STRATEGY_GRAVEYARD.md`
全文關鍵字，皆為全新方向。每條都寫了經濟理由+具體假設定義+已知相關
背景+下檔保護要求（使用者統一裁示：MDD/地雷率/regime存活，不能只贏
買進持有），完整內容見`research/HYPOTHESIS_QUEUE.md`對應章節。

依`hypothesis_queue`具名鎖協調（跟自主馬拉松共用同一份檔案），取鎖→
編輯→commit→釋放鎖，過程中沒有跟馬拉松同時寫入衝突。

---

## ❌ 2026-09-02（研究帽）分點免費自爬TWSE可行性探勘：不可行，掛驗證碼

使用者裁示「三、分點免費自爬TWSE（先探勘再動）」。實測發HTTP請求查
`bsr.twse.com.tw/bshtm/bsMenu.aspx`，確認查詢頁掛圖形驗證碼
（`CaptchaImage.aspx`+5碼文數字輸入框）——**結構性阻擋，免費+不違反
ToS的自動化路徑走不通**，不是限流/格式問題可以繞過。完整探勘過程/
證據/替代方案分析見`research/bsr_scraping_feasibility.md`。

**未執行任何爬蟲**，依指令「抓不到就誠實回報、不硬幹、不造假」原地
停下。現有付費替代（FinMind sponsor / 富果分點API）維持「卡付費資料
源、待使用者決定」現狀不變。均價估算子項（不含分點、只用既有三大法人
合計資料）技術上可行但需要先建個股每日籌碼JSON管線，工作量不小，
留待使用者決定是否投入，這次不擅自開工。
...
- **⚠️ B25：回測regime標記與分情境報告（2026-09-02完成，隨機對照組欄位
  誠實留白）**：
  ...
  3. **只做報告，不做任何權重調整或情境切換邏輯**——先看事實，切換策略
     需另行提案並經使用者同意。
  **完成範疇（誠實記錄）**：只做了價值成長板（`value_board_v2`）——
  題材動能榜/未來性濾網沒有PIT回測引擎，無法比照辦理（B24-500那輪已
  記錄的既有限制）。10種情境組合的年化報酬/勝率/MDD都算出來了，**隨機
  對照組百分位這欄誠實留白**（B24-500的100次draws沒存逐日序列，重新
  產生要數小時，超出這輪範圍）。標⚠️是因為規格要求的三板/隨機對照組
  兩個維度沒有完整達成，不是造假或半途而廢——每個沒做到的地方都在
  `research/B24_RESULTS.md`「B25分情境績效報告」章節寫清楚原因。關鍵
  發現：「空頭+非高波動」是表現最差的情境組合（年化-26.95%、勝率
  47.2%、MDD-35.14%），提示既有`regime_overlay.py`只靠波動率偵測危機
  可能漏掉「緩跌盤跌」這種空頭型態。詳細數字/方法/限制見
  `research/B24_RESULTS.md`、`research/data/b25_regime_report_value_
  board.csv`、`research/b25_regime_report.py`。

- **✅ B26：B24報告慣例補強（2026-09-02發現已隨B24-500完成，補記狀態）**：
  1. 報告新增欄位：調整後Sharpe（原值×0.5與×0.7兩欄，標示「實盤預期
     區間」）、CVaR(95%)。
  2. 所有回測結論句必須引用調整後區間，不得只引用原始Sharpe。
  **2026-09-02查證發現：這兩項在B24-500正式全樣本回測跑的時候就已經
  一併完成**（見上方「2026-08-29 B24-500正式全樣本回測」條目第3點+
  `research/B24_RESULTS.md`完整數字表：Sharpe×0.5/×0.7兩欄、
  CVaR(95%,日)、勝率皆有，且第53行明確聲明「所有結論句只引用調整後
  Sharpe區間，不單獨引用原始Sharpe」），只是先前沒有回頭把這個BACKLOG
  條目狀態改掉，補記✅不是新做的工作。
...
- **B35：秒級即時（手機端）架構項——只登記規格，動工前必須先提案（2026-09-03
  ...
  功能），這裡改用B35避免撞號，已跟使用者說明清楚**。
  1. **瓶頸診斷**：不在永豐（Shioaji）API——來源端已經可以做到逐筆tick
     （見B34），真正的瓶頸在「本機→git push→GitHub Pages CDN→手機
     fetch」這條慢鏈路，git push+CDN傳播延遲讓資料到手機端時已經不是
     秒級。
  2. **解法方向**：雲端中繼——本機tick資料先送進雲端（websocket伺服器
     或資料庫），手機直接連雲端（websocket或輪詢雲端API），繞開GitHub
     Pages這條靜態網站部署鏈路完全不適合做秒級更新的架構限制。
  3. **鐵律（使用者原話，逐字記錄）**：這屬於架構變更，**動工前必須
     先提完整提案＋金鑰/連線安全風險評估給總司令核准，現在不要開工**
     ——只登記這個未來方向，不寫任何程式碼、不選定任何雲端服務商、
     不申請任何雲端帳號/金鑰。
  尚未開始，等待使用者核准提案後才動工。
