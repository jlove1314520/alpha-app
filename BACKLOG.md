...
行事曆→拆兩榜順序，全部六項已完成。B16回測驗證是下一步，見❌待處理。）

## ❌ 待處理

- **B16（P0）：價值成長榜+題材動能榜都必須各自回測驗證**（2026-08-27
  提升為P0，尚未開始——範圍極大，是獨立的一大塊工作，不是這輪能一併完成）：
  ...
  - 未經使用者同意不得解鎖holdout。
  - 在回測完成前，兩榜頁面都已固定顯示「本榜為資料排序，尚未經過組合
    策略回測驗證，不代表能贏大盤」（已完成，見上方✅）。
  - 這塊工作會需要跟研究端既有的holdout紀律/factor_ic.py/TRIALS_LEDGER.md
    框架整合，建議另外排一輪專門處理，不要在原本已經很長的session裡
    倉促動工。
