...
- **P1-新 選股改為「全市場 + 資料完整度」，不要用門檻排除**（2026-08-27登錄）：
  現況341/2,586檔、平均coverage 0.597，coverage<0.5硬性門檻排除約一半。
  改法：(1) 移除硬性排除，全部進榜；(2) 每列顯示資料完整度視覺條+標示缺
  哪幾項因子；(3) coverage低者視覺弱化（降對比+「資料稀疏，分數僅供參考」
  註記），不隱藏；(4) 保留流動性門檻不變；(5) 頁面加總分/完整度是兩件事
  的說明。**進度：尚未開始。**
