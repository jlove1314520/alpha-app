...
def describe_foreign_holding(path: Path) -> dict:
    """data/foreign_holding.json（2026-09-15新增，源頭二.3第1名）——TWSE官方
    MI_QFIIS（外資及陸資投資持股統計），`.github/scripts/fetch_foreign_holding.py`
    每日排程產生，跟stock_detail.json同一批market.yml步驟。"""
    d = json.loads(path.read_text(encoding="utf-8"))
    stocks = d.get("stocks", {})
    dates = {s.get("date") for s in stocks.values() if s.get("date")}
    return {
        "generated_at": d.get("fetched_at"),
        "records": len(stocks),
        "source": d.get("source"),
        "detail": f"資料日={sorted(dates)} errors={d.get('errors')}",
    }
...
def describe_us_13f_holdings(path: Path) -> dict:
    ...
    13F整合），`.github/scripts/fetch_us_13f_holdings.py`每日排程產生。"""
    d = json.loads(path.read_text(encoding="utf-8"))
    filers = d.get("filers", {})
    total_holdings = sum(len(f.get("holdings") or {}) for f in filers.values())
    return {
        "generated_at": d.get("fetched_at"),
        "records": total_holdings,
        "source": d.get("source"),
        "detail": f"申報人={list(filers.keys())} 命中追蹤清單檔數={total_holdings} errors={d.get('errors')}",
    }
...
APP_DATA_SOURCES = [
    ...
    {"panel": "個股頁·籌碼·借券賣出當日成交量", "source": "data/securities_lending_sell.json（TWSE官方TWT93U＋TPEx官方tpex_short_sell，2026-09-15新增，源頭一.2b資料層／源頭一.3個股頁UI，免金鑰）"},
    {"panel": "個股頁·籌碼·千張大戶（僅台股）", "source": "data/holders.json（集保結算所官方opendata.tdcc.com.tw，2026-09-15源頭一.3接上個股頁UI，每週五20:00排程；資料層本身源頭一.2a已建）"},
