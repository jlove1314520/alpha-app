...
|---|---|
| 機構 | 臺灣證券交易所（TWSE） |
| 我們現況 | **⚪ 未查證**——repo內對`MI_QFIIS`或「外資持股比率」關鍵字零命中。 |
| 對應機構用途 | 個股外資持股水位監控（外資是否加碼/減碼） |
| 後續 | 應查`www.twse.com.tw/rwd/zh/fund/MI_QFIIS`是否為官方端點格式（比照既有T86端點`www.twse.com.tw/rwd/zh/fund/...`的路徑慣例），源頭二.2實測 |
...
- **🟢 已整合且合規**：#7當沖、#8鉅額交易、#17集保、#21利率、#22 SEC EDGAR、#27(b) Nasdaq threshold list ——6項
...
- **⚪ 完全未查證**：#4外資持股比率、#13私募、#14法說會音檔、#19財政部海關、#20經濟部工業生產、#23 EDGAR全文檢索、#25 BLS、#26 Census、#27暗池、#28 CFTC COT ——約10項

**下一步（源頭二.2，本輪未做，待總司令裁示上述robots.txt衝突後再繼續）**：
對⚪未查證項目逐一打最小請求測試可用性；對🟡項目補齊查證缺口
（尤其#9內部人轉讓正確功能代碼、#5借券正確端點）。
