...
### 5b. 借券餘額（出借總量）—— 2026-09-20 三來源查證：TWSE有逐檔逐日官方歷史（籌碼原子.出借總量查證）

| 欄位 | 內容 |
|---|---|
| 查證動機 | `research/ATOM_CHIP_IC_MAP_SPEC.md`第1節揭露：FinMind可得的是「借券賣出餘額」（放空方），「借券餘額（出借總量）」尚未查證 |
| **來源①官方網站／開放資料入口** | TWSE「借券資訊」頁 <https://www.twse.com.tw/zh/products/sbl/disclosures/twt72u.html>（=`/zh/page/trading/exchange/TWT72U.html`）：標題「證交所借券系統與證商/證金營業處所借券餘額合計表」，頁面載明「本資訊自民國93年11月22日起開始提供」，有日期查詢欄。頁面HTML內的資料端點是`/lending/TWT72U`（本機`requests`直接讀該頁原始碼取得）。 |
| **來源②官方API文件** | TWSE openapi swagger（`openapi.twse.com.tw/v1/swagger.json`，143個路徑）**只有**`/SBL/TWT96U`（可借券賣出股數）含借券，**沒有**借券餘額——所以這條資料不在openapi清單，在`www.twse.com.tw/rwd`主站家族（跟T86、TWT93U同一類）。TPEx openapi（`tpex.org.tw/openapi/swagger.json`）相關端點只有`tpex_margin_sbl`（**借券賣出餘額**）、`tpex_short_sell`、`tpex_intraday_fee`、`tpex_margin_trading_lend`，**沒有**出借總量。 |
| **來源③其他供應商** | FinMind資料表清單（<https://finmind.github.io/tutor/TaiwanMarket/Chip/>）：`TaiwanDailyShortSaleBalances`＝信用額度總量管制餘額表（含借券**賣出**餘額，2005-07起）、`TaiwanStockSecuritiesLending`＝借券成交明細（交易流量，不是餘額）；**沒有**出借總量餘額表。 |
| **實測端點（2026-09-20，`requests`＋UA，每次間隔3秒共約8次請求）** | `https://www.twse.com.tw/rwd/zh/lending/TWT72U?date=YYYYMMDD&selectType=SLBNLB&response=json`（`SLBNLB`＝證交所借券系統＋證商/證金營業處所合計；`SLB`＝僅證交所借券系統）。回傳`{stat,title,fields,data,notes}`，`data`每列＝證券代號、名稱、前日借券餘額(1)、本日異動借券(2)、本日異動還券(3)、本日借券餘額(4)=(1)+(2)-(3)（股）、收盤價、借券餘額市值、市場別（實測皆為「集中市場」）。**一次請求＝一個交易日全市場**。`notes`原文：「借券餘額表係揭露已借入尚未返還之借券數量，其中包含已於市場借券賣出但尚未返還之數量」。 |
| **歷史深度（實測，SLBNLB）** | 2006-01-04：38列（4位數代號35）；2008-12-31：158列（155）；2010-01-04：324列（321，2330＝311,234,000股）；2012-01-04：886列（879，2330＝164,755,000股）；2026-09-18：2068列。**早期覆蓋稀疏但隨年份增加**，2010起約320檔、2012起約880檔（含ETF）。**只涵蓋上市（集中市場）**。 |
| **上櫃** | TPEx openapi無此欄（見來源②）；TPEx網站頁面本輪**未查**，故**不宣稱上櫃沒有**，只宣稱「本輪openapi清單內沒有」。且籌碼原子宇宙的三大法人族本來就只涵蓋上市（規格第10節），上櫃缺口對Tier A/B的影響同向。 |
| 官方是否允許程式存取 | 🟢（TWSE官方公開查詢端點，免驗證碼／登入；**未公布次數上限**，比照「TWSE/TPEx OpenAPI」條款：實測會軟性限流，回填時固定≥3秒間隔、遇非JSON連續3次即停） |
| 結論 | **(a)成立**：找到有歷史的官方端點。已在規格補第11節（標「看過結果前」，尚未跑任何IC），並另立回補項`籌碼原子.補出借總量快取`。這是「借券餘額（出借總量）」，與已回補中的「借券賣出餘額」是兩個不同的量。 |
| 已知限制 | 時點PIT：借券系統餘額表約於收盤後公布（公布時間未實測，規格沿用lag1日的保守做法）；早期年份（2006~2009）覆蓋很稀疏，不宜當全宇宙原子；未查證的是TPEx網頁版與2004-11~2005年初的首日資料。 |
