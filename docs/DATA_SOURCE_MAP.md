...
| 8 | 當沖比重 | TWSE `www.twse.com.tw/rwd/zh/afterTrading/TWTASU`（研究已用，`research/twse_day_trading_client.py`）；TWSE openapi `/exchangeReport/TWTB4U`（上市股票每日當日沖銷交易標的及統計，**逐檔**，2026-09-15本機`curl`實測swagger.json確認）；TPEx `/tpex_intraday_trading_statistics`（上櫃，逐檔） | 每交易日 | ⚠️ `TWTASU`**只有全市場加總（每日1列5欄），無逐檔**（`#57`已查證此限制，`docs/FIRST_HAND_SOURCES.md`有詳細記錄）；`TWTB4U`/TPEx對應端點**可能**有逐檔資料，尚未實測欄位內容，需源頭一.2c核對後才能確認能否補足個股頁需求 | 🟡 **部分已有**：`TWTASU`用於研究（`#57`因無逐檔已改標「前置未備」），個股頁尚未顯示當沖比重，見源頭一.2c／源頭一.3 |
