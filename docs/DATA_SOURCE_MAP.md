...
| 7 | 借券（賣出成交／餘額） | TWSE openapi `/SBL/TWT96U`（上市上櫃股票**當日可借券賣出股數**，屬可借額度非成交明細）；TPEx `/tpex_margin_sbl`（上櫃股票融券借券賣出**餘額**）、`/tpex_short_sell`（上櫃當日融券賣出與借券賣出**成交量值**，2026-09-15本機`curl`實測TPEx swagger.json確認端點存在） | 每交易日 | ⚠️ TPEx兩端點看起來含逐日餘額/成交量值，可望回測；**TWSE（上市）目前只查到「可借額度」端點，未找到上市股借券賣出成交明細的對應端點**，需下一輪源頭一.2b實際呼叫兩端點核對欄位與歷史深度後才能下最終結論 | 🟡 **待建**：本輪僅完成端點初步定位（非最終定論），實際接入留給源頭一.2b |
