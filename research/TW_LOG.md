...
## 第346輪（2026-09-04T22:32+08:00，TW）——稽核回收上一個異常中止session的合法未commit產物：leave-one-factor-out（train-only權重）＋margin_utilization regime-conditional portfolio構造checkpoint進度

取鎖時偵測到`LOCK_STALE`（pid 50288持有30.1分鐘後被回收，上一輪疑似異常中止未釋放鎖）。三軌時間戳：TW 19:28（最舊）／US 20:51／FUT 21:07——依輪替選TW。

開工先查`git status`，發現一批合法但未commit的產物，時間跨度約20:44~22:13，橫跨可能超過一輪：`research/TRIALS_LEDGER.md`/`MARATHON_LOG.md`/`HYPOTHESIS_QUEUE.md`有未commit修改，另有4個未追蹤檔案——`leave_one_factor_out_bigsample.py`（22:04）、`run346_leave_one_out.log`（22:13）、`margin_utilization_regime_portfolio_v1.py`（21:25）、`run343_full_gate3.log`（20:44）。逐項核實：

1. **`leave_one_factor_out_bigsample.py`＋`run346_leave_one_out.log`**：對應`TRIALS_LEDGER.md`已寫好但未commit的`#118`（leave-one-factor-out train-only權重版，獨立確認round343稽核發現的`#113`孤兒紀錄）。讀腳本內容確認：沿用`load_dev()`＋`holdout.assert_no_holdout_leakage()`，無`load_full_history()`/`unlock_holdout_once()`，holdout安全；讀log確認數字與`#118`條目逐位吻合，非捏造。**唯一缺口**：`#118`聲稱「完整見`LEADS.md`（本輪新增portfolio_multifactor_v2補充段落）」，但`LEADS.md`實際上完全沒有這段落（本輪git status確認`LEADS.md`本身無任何修改）——這是同一種「聲稱寫了但沒真的寫」的落差，跟round343發現的`#113`孤兒紀錄同一種失敗模式，只是這次多虧異常中止前TRIALS_LEDGER.md／腳本／log都留在磁碟上，才有辦法補救不用重跑。**本輪已在`LEADS.md``portfolio_multifactor_v2`條目補上這段遺漏內容**，並修正原文裡誤把「本輪」寫成「第346輪」／「第347輪」兩種不同輪次標號的混淆（因為異常中止的那個session從未真正進入輪次計數器，計數器至今仍停在345，本次recovery正式列為第346輪，遺留log檔名裡的「346」字樣視為前一個session自報、非正式編號，兩者現在對齊了）。
2. **`margin_utilization_regime_portfolio_v1.py`＋`run343_full_gate3.log`**：對應`HYPOTHESIS_QUEUE.md`未commit修改裡`#30`條目的portfolio層構造進度更新（regime-conditional避開高融資使用率個股，危機regime挑最低使用率TOP20、非危機regime挑流動性最高TOP20，隨機控制組判準是打贏「危機期隨機選股」而非買進持有大盤）。讀腳本確認同樣holdout安全（`load_dev()`＋`assert_no_holdout_leakage()`）。**這是checkpoint式多輪任務，尚未結案**：TRAIN真實訊號+成本敏感度已完成，TRAIN隨機控制組進度僅20/100，VALIDATION尚未開始。本輪**不**代為繼續執行（避免這輪時間預算被單一任務占滿，且checkpoint機制本身就是設計給下一輪接續用的），只誠實commit目前進度，讓下一輪讀`HYPOTHESIS_QUEUE.md``#30`條目最新狀態就能知道要重跑同一支腳本接續。`run343_full_gate3.log`是這個構造工作更早期的一次執行log，內容跟`train_only_ic_weight_bigsample_full_gate.py`（round343 TW_LOG提到「腳本檔案本身從磁碟消失」異常的那次嘗試）相關，保留作為異常現象的佐證紀錄，不刪除。
...
## 第373輪（2026-09-05T19:00+08:00，TW）——季節性（月效應/財報季效應）家族橫斷面構造可行性查核，正式結案：三條路徑皆卡在資料源限制

**取鎖**：乾淨（非陳舊鎖檔）。**輪替判斷**：TW 10:05（round365，最舊）／US 10:34（round366）／FUT 18:30（round372，最新）——選TW。

**工作單位**：round365下一步(a)點名的候選——`MARATHON_PROTOCOL.md`第3節「季節性（月效應/財報季效應，需先設計橫斷面構造方法）」家族的前置可行性查核。純讀既有程式碼（`pit.py`/`factors.py`）與文件（`DATA.md`），零新增FinMind API呼叫。

**評估的三條構造路徑**：

1. **財報公告時點差異**（文獻機制：Bagnoli, Kross & Watts 2002，相對同業較晚公告財報通常代表壞消息，是有economically plausible理由的cross-sectional訊號）——查`pit.py::quarterly_pit()`/`balance_sheet_pit()`：`pit_date`對FinMind免費層**全部**是「`fiscal_period_end` + `QUARTERLY_DISCLOSURE_LAG_DAYS`(45天)」這條**單一公式**算出來的，`pit_source`恆為`'assumed'`（`pit.py`docstring第3~7行明載「FinMind免費層沒有真實公告日欄位」）。這條公式對每一檔股票、每一季都套用同一個天數，**結構上就沒有跨股票的橫斷面差異**——不是樣本不夠多測不出來，是假設值本身的建構方式決定了它不可能有變異。
2. **月營收公告時點差異**（同一機制，改用月營收公告日）——`pit.py::month_revenue_pit()`確實有`create_time`欄位，且`has_real`為真時會標`pit_source='real'`，理論上有真實的跨股票公告時間差異可用。但查`DATA.md`第394/397行：**真實`create_time`只從約2026-04起才有值，更早的月份`create_time`全部是空字串、退回統一的「次月10日」規則**（同樣無跨股票變異）。本專案既有TRAIN(2015-2020)/VALIDATION(2021-2024)兩期回測窗口完全早於2026-04，這代表**在這兩個窗口內，這條路徑一樣沒有可用的橫斷面變異**——不是設計不出構造方法，是真實資料的歷史長度還不夠長，覆蓋不到需要回測的期間。
3. **規模別一月效應**（size-conditional January effect，經典異常，小型股一月報酬顯著較高，文獻機制是稅損賣壓年底出清、年初回補+機構window dressing）——查`factors.py`第30~31行：`TaiwanStockMarketValue`（市值）明文記載是FinMind**付費層限定**。台股軌至今沒有任何市值/流通股數資料源（`TRIALS_LEDGER.md`#107/`HYPOTHESIS_QUEUE.md`#29「等權重再平衡」條目也記錄過同樣限制），無法做規模分層，這條路徑同樣卡在資料源不存在。

**結論**：三條路徑**沒有一條卡在「訊號本身測了不成立」，全部卡在資料源結構性限制**（要嘛假設值天生無變異、要嘛真實資料歷史長度不夠、要嘛缺市值資料源）——跟round365「成長與預估上修」家族的結案性質完全相同（資料源限制，不是負面訊號結果），依`MARATHON_PROTOCOL.md`第5節「先確認有沒有可行構造方法，沒有就記錄資料源不存在跳過」的同一精神，正式標記**季節性家族暫不列入候選、結案**。不進入任何統計檢定，不計入`bonferroni_n`。

**已寫入**：`TRIALS_LEDGER.md`「已調查但不計入試驗數」表新增一列、`TW_LEADS.md`「下一輪建議」段落補上新備註（季節性標刪除線結案，並補完整說明）、`TW_MARATHON_STATE.md`覆寫最新狀態（round356舊條目輪替搬進`TW_STATE_ARCHIVE.md`）。
...
## 2026-09-06T16:30+08:00 — 馬拉松第403輪：TW

取鎖乾淨（非陳舊鎖檔，cycle`20260906-163037`）。三軌時間戳：FUT 12:00（第399輪，最舊，但`FUT_MARATHON_STATE.md`round399明文下一輪選到FUT要讓回TW/US）／TW 15:33（第401輪）／US 16:06（第402輪，最新）——依換軌例外條款在TW/US間選較舊的TW。

**本輪工作單位＝收成round401投遞的`20260906-153323-37f5`（`tw_regime_conditions_value_pe`）並完成`CRITERIA_V2_LOCK.md`第39行第一關（情境分群檢驗）判讀**。`run_detached.py status`確認`finished, exit=0`（8.0分鐘）。讀log：185/248檔有`f_value_pe`值，121個不重疊20交易日快照。四組條件——(a)大盤位階：多頭+0.0516(n_snapshots=93,n_obs=11476)｜空頭+0.0984(28,3526)；(b)波動度：高+0.0570(69,8639)｜低+0.0696(52,6363)；(c)市值規模：大+0.0827(121,4997)｜中+0.0362(121,5007)｜小+0.0593(121,4997)；(d)流動性：高+0.0561(121,7511)｜低+0.0623(121,7458)。**8組全部為正、無方向反轉，n_obs全部遠超100門檻（最小3526）**，判定通過情境分群關卡（比照`REGIME_CONDITIONS.md`既有7因子的判讀方式：方向一致、樣本數達標）。空頭強於多頭（+0.098 vs +0.052）跟`f_revenue_surprise`「空頭/高波動下反而更強」同一種模式，經濟解釋為價值因子的估值折價保護在熊市/危機期更明顯兌現，跟文獻方向一致。已寫入`TRIALS_LEDGER.md`#166、`TW_LEADS.md`#2更新、`REGIME_CONDITIONS.md`新增`f_value_pe`小節。

**本輪工作單位(2)＝投遞`CRITERIA_V2_LOCK.md`第39行第二關（成本敏感度）背景工作**：新增`deep_dive_f_value_pe.py`，方法比照`deep_dive_f_value_pb.py`（同因子家族既有模板，非複製檔案本身，改寫`TARGET_FACTOR`/檔名/PIT備註）——十分位多空（k=10%）、TRAIN(2015-2020)/VAL(2021-2024)兩期、1x/2x/3x成本倍數、100次配對式隨機控制組、CAPM beta，改用`factor_ic.SAMPLE_SIZE`現行300檔（跟`regime_conditions_value_pe.py`同一份樣本）而非`f_value_pb`當初用的舊100檔樣本。`ast.parse`+`holdout.is_holdout_consumed()`（`False`）確認後`run_detached.py submit --name tw_deep_dive_value_pe_cost_sensitivity --timeout-min 30 --expect data/deep_dive_f_value_pe.csv`（job`20260906-163407-6cc8`），session內`wait --max-min 4`仍`STILL_RUNNING`（300檔樣本首次載入預期約13分鐘量級，未逾常，未搶跑其他heavy job，heavy-job-slot本輪僅此一個工作在用）。全程零新增API呼叫（複用round401已建立的300檔因子快取）。

**下一輪TW軌接手**：`run_detached.py status`確認`20260906-163407-6cc8`是否`finished`；若是，讀`data/deep_dive_f_value_pe.csv`與log裡TRAIN/VAL兩期×三成本倍數的`ann_return`/`beta`/`alpha`/`random_control_percentile`，比照`deep_dive_f_value_pb.py`（`TRIALS_LEDGER.md`#13/#14附近）跟其他候選的判讀方式（percentile是否達99~100量級門檻、TRAIN/VAL方向是否一致、beta是否偏離market-neutral過多），寫入`TRIALS_LEDGER.md`新編號＋`TW_LEADS.md`#2更新；若逾30分鐘timeout被砍，記錄`timeout`並檢查是否為首次load耗時比預期長。若成本敏感度也通過，`CRITERIA_V2_LOCK.md`第39行流程最後一關是alpha/beta顯著性關卡，尚未設計，留給之後判讀時一併規劃。`is_holdout_consumed()`開工/收工前皆確認`False`。完整見`TRIALS_LEDGER.md`#166、`TW_LEADS.md`#2、`REGIME_CONDITIONS.md`、`deep_dive_f_value_pe.py`（新增，可重複執行）、`data/jobs/20260906-163407-6cc8.log`。

## 第405輪 2026-09-06T17:31+08:00（TW軌）

**本輪工作單位＝收成round403投遞的成本敏感度背景工作並重投**。`run_detached.py status`確認`20260906-163407-6cc8`（`tw_deep_dive_value_pe_cost_sensitivity`）已`timeout`（30分鐘被砍，`exit=-9`）。`run_detached.py log`確認只跑完「TRAIN 2015-2020, cost 1x」一組（含100次隨機控制組，net_total_return=+29.96%/ann_return=+4.60%/beta=+0.103/alpha=+4.70%/percentile=100.0）就被砍在下一組（TRAIN 2x）進行中——推算全部6組（TRAIN/VAL兩期×1x/2x/3x三成本）跑完量級落在90~150分鐘，比round403當初估計的30分鐘明顯低估，原因是300檔樣本（`CALIBRATION_PROBE.md`裁示後的新常數，非`f_value_pb`當初的100檔）讓每組100次隨機控制組耗時大增，跟US軌同規模任務（`deep_dive_f_us_value_bm_clean_universe.py`93.5分鐘、`deep_dive_f_us_low_vol_clean_universe_retry`136.1分鐘）同量級。
...
## 2026-09-08T20:00+08:00 — 馬拉松第455輪：#61央行理監事會議決策事件，三來源查證官方歷史重貼現率變動日期

依round453交辦，接手`hypothesis_queue`已設計好的新假設軸`#61`第1步：三來源查證
央行官方歷史決策日期清單是否可行。

**查證結果**：(1)官方「重要指標」重貼現率時間數列頁面（`lp-640-1-1-20.html`）
為結構化表格，2015~2024年區間可取得**11筆變動事件**（TRAIN 5筆/VAL 6筆）；
(2)官方「主動公開政府資訊」理監事會議決議彙整＋2020年「預定日期」公告確認
央行每年12月會公告次年四場會議確切日期，2020年案例（3/19會議→3/20生效）
與變動事件表吻合；(3)國立臺北大學課程講義PDF的「3/6/9/12月第三個星期四」
經驗法則，比照`#60`教訓核對後證實**不可靠**（2022-09-23對應會議日是第4個
星期四不是第3個），故不採信為推算依據，只記錄逐筆核對過的官方日期。

新增`cbc_policy_decision_data.py`（硬編碼11筆變動事件，附完整來源URL與
查證方式，非程式化API——官方頁面無公開CSV/JSON下載端點）。

**誠實揭露兩層資料完整度**：子測試2（方向分組）核心資料已取得但樣本量極小
（11筆事件）；子測試1（決策日本身，含持平會議）需要的完整~40場會議日期
清單尚未補齊，只確認2020年一年可查到，其餘9年待下一輪。

`trial_registry.py --check`確認PASS（exit=0，218列，本輪非試驗判定不涉及
