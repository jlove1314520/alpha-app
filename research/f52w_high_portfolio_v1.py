"""52週高點接近度策略層構造 — `HYPOTHESIS_QUEUE.md`#17。

**誠實現況（不是重新驗證因子IC）**：`f_52w_high_prox`（當前收盤價/過去252
交易日滾動最高價）已經在`TRIALS_LEDGER.md`#84通過因子級cheap IC gate（TRAIN
mean_ic=+0.0760 IR=+0.389、VAL mean_ic=+0.0863 IR=+0.465 hit_rate=0.68，
...
**逐字比照`dividend_yield_portfolio_v1.py`（#4）架構**：同樣是單因子、
同產業z-score標準化、月頻Top20——連checkpoint可續跑機制都直接沿用同一套
設計（#4連續五輪卡在單次執行需要35~40分鐘、無人值守呼叫結束背景行程會被
一併終止的教訓，這裡從一開始就用checkpoint、不重蹈覆轍）。
...
- `dividend_yield_portfolio_v1.py`的`_eligible_single_factor()`同款邏輯
  （單因子composite只可能n_components=1，不能套用多因子門檻的
  `n_components>=2`，否則0檔候選、0筆交易——這是#4已經踩過並修好的坑，
  這裡直接用對的版本，不重踩）。

2026-09-02 由`HYPOTHESIS_QUEUE_PROTOCOL.md`自動排程新增，佇列#17第2關以後
起跑（第1關cheap IC gate已CHEAP_PASS，見`TRIALS_LEDGER.md`#84）。
"""
...
COMPONENTS = ["f_52w_high_prox"]  # 刻意單因子，52週高點接近度假設

CHECKPOINT_PATH = Path(__file__).parent / "data" / "f52w_high_portfolio_v1_checkpoint.json"
...
def _raw_components(row: pd.Series) -> dict[str, float | None]:
    out = {}
    for comp, col in (("f_52w_high_prox", "f_52w_high_prox"),):
        v = row.get(col)
...
def _eligible_single_factor(cs: pd.DataFrame) -> pd.DataFrame:
    """跟`dividend_yield_portfolio_v1.py::_eligible_single_factor()`同一套
    邏輯（逐字比照，見該函式docstring說明為什麼不能套用`pbv2._eligible()`
    的`n_components>=2`多因子門檻）。"""
...
def run_one(label, data, market_df, industry_map, liquidity, start, end,
    ...
    cfg = BacktestConfig(start_date=start, end_date=end, max_positions=TOP_N,
                          rebalance_every_n_days=REBALANCE_DAYS, book_name="f52w_high_portfolio_v1")

    if "real" not in ckpt:
        print(f"  [{label}] 計算真實訊號回測...")
        result = run_backtest(signal_fn, data, market_df, cfg)
        holdout.assert_no_holdout_leakage(result.trades, date_col="date", context=f"f52w_high_portfolio_v1 {label}")
    ...
    for i in range(start_i, n_random):
        if deadline is not None and time.time() > deadline:
            ...
            return None
        rfn = make_random_signal_fn(industry_map, liquidity, seed=20260902 + i)
...
def main() -> None:
    ...
    holdout.assert_no_holdout_leakage(market_raw, context="market_raw in f52w_high_portfolio_v1")
    ...
    for sid, d in data.items():
        holdout.assert_no_holdout_leakage(d, date_col="date", context=f"data[{sid}] in f52w_high_portfolio_v1")

    industry_map = load_industry_map()
    liquidity = {sid: pbv2._liquidity_proxy_series(d) for sid, d in data.items()}

    # 7分鐘算力預算，理由跟`dividend_yield_portfolio_v1.py`同一套（安全落在
    # 10分鐘外層工具逾時與25分鐘鎖檔陳舊門檻內）。
    TIME_BUDGET_SECONDS = float(os.environ.get("F52W_TIME_BUDGET_SECONDS", "420"))
    deadline = time.time() + TIME_BUDGET_SECONDS

    print(f"\n========== 第7關 train/val樣本外（月頻/Top20/單因子52週高點接近度，可續跑checkpoint，本次預算{TIME_BUDGET_SECONDS}秒）==========")
    results = {}
    ...
    if incomplete_label is not None:
        print(f"\n**本次{TIME_BUDGET_SECONDS}秒時間預算內未跑完（卡在{incomplete_label}），"
              ...
              f"重新執行`python f52w_high_portfolio_v1.py`會自動從中斷處接續，"
    ...
    pd.DataFrame(results.values()).to_csv("data/f52w_high_portfolio_v1_results.csv", index=False)
    print("\n已存 data/f52w_high_portfolio_v1_results.csv")
    ...
    print("\n" + "=" * 70)
    print("**全部7~8關通過（52週高點接近度單因子組合，月頻Top20）！**")
