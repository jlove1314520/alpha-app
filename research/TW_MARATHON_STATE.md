...
**最後更新：2026-08-23T13:30:00+08:00**（馬拉松第三輪執行後）
...
4. **下一輪優先（任一）**：(a) 拆解TRAIN期絕對報酬為負是否為週轉成本drag——嘗試拉長換倉週期（例如60日）或縮小十分位比例，重跑`deep_dive_f_quality_roe_stability.py`比較TRAIN期結果是否轉正；(b) 深挖 `f_value_pb` 前，**必須先補 PIT 驗證**（查 FinMind `TaiwanStockPER` 的 PBR 更新時點是否等到財報公告才變動），不能跳過直接深挖。
5. 完成上面之後，照 `MARATHON_PROTOCOL.md` 第 3 節清單系統化掃過還沒碰過的因子家族：短期反轉、BAB/特異波動率、Amihud流動性、季節性、資產成長異常、Piotroski F-score、accruals盈餘品質。
