...
**最後更新：2026-08-29T00:34+08:00（馬拉松第200輪）**——取鎖乾淨，依輪替選TW。查明並修好round197卡住的根因：`twse_t86_client.py`每次查一檔股票都重新讀全部3319個T86快取檔，改成process內分組快取後單檔查詢從20-37秒降到0.4秒（5檔對照測試確認新舊輸出一致），已commit。但接續跑`deep_dive_portfolio_v2_random_control_n100.py`時，sample載入順利過關後，`run_one(...,n_random=100)`階段process無聲消失（疑似OOM），是新問題，下一輪建議先用小n_random二分定位。既有`portfolio_multifactor_v2`判定不變。`is_holdout_consumed()`確認`False`。詳見`TW_LOG.md`第200輪記錄。
