...
**最後更新：2026-08-26T19:45+08:00（馬拉松第107輪）**——取鎖時偵測到`LOCK_STALE`（pid 136608持有29.9分鐘，上一輪疑似異常中止；commit前發現實際上有殘留孤兒工作——round 105/106的驅動腳本跟US軌文件未commit，本輪一併補上，詳見`TW_LOG.md`本輪記錄）。三軌輪替（TW 17:36/US 18:10/FUT 17:05）本應選FUT但近10輪已達20%資源配置上限，改選TW。延續`TW_LEADS.md`#3「TRAIN期絕對報酬拆解」開放問題：新增`decompose_f_quality_roe_stability_rebalance.py`，60日換倉版本TRAIN/VAL同號皆正（支持週轉成本drag假說），但**意外發現重跑20日對照組不再重現round2/3記錄的TRAIN負值**——推論是backfill_universe.py期間為樣本內部分股票補齊了先前缺失的財報歷史，改變了因子數值。**尚未升格判定**，下一輪最高優先：完整重跑`deep_dive_f_quality_roe_stability.py`本身（含100次隨機控制組）確認新基準。完整見`TW_LOG.md`本輪記錄、`TW_LEADS.md`#3、`TRIALS_LEDGER.md`#69。
