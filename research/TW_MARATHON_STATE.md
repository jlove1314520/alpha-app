...
**已知的立即可做工作（優先序）：**
1. `f_value_pb`／`f_value_pe`／`f_quality_roe_stability` 重測——2026-08-22 因 FinMind 流量限制完全未測，2026-08-23 確認流量限制已解除。**這是第一輪最推薦的起點**，因為已經有實作好的程式碼（`factors.py`），只差重跑 `factor_ic.py`。
2. 完成上面之後，照 `MARATHON_PROTOCOL.md` 第 3 節清單系統化掃過還沒碰過的因子家族：短期反轉、BAB/特異波動率、Amihud流動性、季節性、資產成長異常、Piotroski F-score、accruals盈餘品質。
