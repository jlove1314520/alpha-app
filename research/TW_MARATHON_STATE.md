...
**最後更新：2026-08-25T00:00:00+08:00**（馬拉松第43輪執行後，取鎖時偵測到`LOCK_STALE`——上一輪pid 100692持有鎖滿300.1分鐘後被回收，第42輪期貨軌之後某一輪疑似異常中止，未留下正常結束的log；本輪開始前state檔案918 done跟第40輪記錄一致，無資料落差）
...
**⚠️ 目前最高優先序：全市場宇宙回補未達門檻，優先於測新因子/深挖候選**（見 `MARATHON_PROTOCOL.md` 第5b節）。累積覆蓋率：**992/3196（31.0%）**，門檻80%。下一輪若輪到TW軌，先跑 `python backfill_universe.py --batch-size 300`（除非一開始就立刻被限流，那種情況改做深挖或系統化掃因子家族）。
