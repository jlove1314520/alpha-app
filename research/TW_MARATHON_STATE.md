...
**最後更新：2026-09-04T07:33+08:00（馬拉松第334輪）**——取鎖乾淨（非陳舊鎖檔）。三軌時間戳：TW 06:10（第331輪，最舊）／FUT 06:33（第332輪）／US 07:03（第333輪）——依輪替選TW。**接續第331輪完成`#79`後的下一步，本輪重跑`#77`（`f_rel_strength`產業中性版）300檔樣本cheap gate**：`factor_ic_sector_neutral_rel_strength.py`原腳本零修改重跑（吃到新的300檔`SAMPLE_SIZE`），248/300檔可用，216/248有非ETF產業分類，組內中位數成員數從4檔升到6檔、可用產業組數從10組升到20組。TRAIN mean_ic=-0.0113（n=46），VAL mean_ic=-0.0065（n=39），**train/val同號（皆負，跟原100檔一致）**，null percentile從100檔樣本的82.8**驟降**到**41.9**（未過90.0門檻，且大幅遠離而非邊緣）。執行時間約2分鐘，全程讀取本機快取，零新增API呼叫。**判定：`#77`從`CALIBRATION_PROBE.md`標記的「未定（待300檔重跑）」正式改回確定FAIL**——跟`#79`（percentile小幅提升）方向相反，這條反而在更大樣本、組內更密的情況下訊號更弱，反駁了原本「地基不夠大稀釋訊號」的猜測，指向100檔樣本的82.8較可能是小樣本雜訊放大出的假邊緣訊號。已寫入`TRIALS_LEDGER.md`#101、`STRATEGY_GRAVEYARD.md`（同條目續記，未新開）、`HYPOTHESIS_QUEUE.md`#11（同步補充）。下一輪可接續依`CALIBRATION_PROBE.md`指令重跑`#91`（`revenue_trend_surprise_low_attention`，TW軌「未定」清單最後一項）。`is_holdout_consumed()`確認`False`（全程讀取本機快取，零新增API呼叫）。
