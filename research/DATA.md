...
### 美股 PIT 資料源調查（五續）：第12項子步驟(b)(c)——filer category 反推邏輯實作與PLTR重測，**結論：此路不通，未能解釋原本的14/24誤判**（2026-08-25 馬拉松第77輪，接續上一輪異常中止留下的腳本）

**背景與交接說明**：上一次嘗試這個工作單位的執行個體（推測是第77輪之前某次啟動）寫完了 `research/sec_edgar_filer_category_infer.py`（含完整docstring、四項LIMITATIONS、依第74輪查證的門檻表實作Era A/B/C分類邏輯＋2020年營收測試），但**從未執行過就中止**（`marathon_lock.py` 鎖檔卡在 pid 131600 達60分鐘後被本輪判定陳舊並回收，該次執行沒有留下任何log/heartbeat記錄）。腳本本身檔案完整、邏輯自洽，本輪的工作是把它跑起來完成子步驟(c)（拿AAPL/MSFT/PLTR驗證），不是重寫。

**執行結果**（`python sec_edgar_filer_category_infer.py`，smoke test）：

- **AAPL（19個財年）／MSFT（17個財年）：全部 `LAF`**，float從未低於$700M，符合預期（兩檔長年掛牌巨型股，沒有意外）。
- **AAPL 發現一個獨立的資料品質問題**：2009-03-28這個`fiscal_year_end`重複出現**三次**，float/revenue數值完全相同（$94.593B / $20.964B）。**這直接推翻了腳本docstring裡「EntityPublicFloat不像其他XBRL概念會因比較期揭露而重複，不需要去重」的假設**（該假設是第70輪探測時「沒發現重複就假設沒有」，這輪是第一次針對這個特定假設做實測，結果是假設不成立，至少AAPL FY2009這筆是重複的）。**不影響本輪的分類結論**（三筆數值相同，去重與否分類結果一樣），但如果未來要拿這份資料做逐筆計數/統計（而不是只取分類結果），需要先去重，不能直接假設一個`end`日期只對應一筆。
- **PLTR（6個財年，2020–2025）：全部 `LAF`，沒有任何一年被classify成AF或NAF。** 原因：PLTR自2020年9月IPO當年的public float就高達$13.3B，遠超$700M的LAF門檻，此後逐年更高——**float本身從未落入AF（$75M–<$700M）或NAF（<$75M）區間，所以Era C的營收測試（`revenue < $100M AND float < $700M`）從未被觸發**（該測試的第二個條件`float_val < _LAF_FLOAT`永遠為False）。

**這個結果推翻了第74輪的預期，是本輪最重要的發現**：第74輪查證門檻表後推測「PLTR 14/24誤判很可能有相當比例是2020年營收測試被反推邏輯忽略造成的」，暗示的假設是「PLTR早期float應該落在會被營收測試影響的AF/NAF邊界，所以需要接營收測試才能正確分類」。**但本輪反推分類器（含完整營收測試邏輯）判定PLTR六個財年全部是LAF——跟`era_reliability()`原本對PLTR套用的全市場LAF假設完全一致，沒有任何一年的分類結果不同。** 也就是說：**「filer category誤判」這個假說被證偽了**——如果反推出來的類別跟現有假設相同，那麗65輪發現的PLTR 14/24 gap超標就不可能是「用錯申報期限（例如該用NAF的90/45天卻被當成LAF的60/40天）」造成的，因為兩種方法算出來的類別根本沒有分歧。

**這代表第12項這整個調查方向（用`EntityPublicFloat`反推個股逐年申報人分級來修正`era_reliability()`）對PLTR案例沒有解釋力，不是子步驟(b)寫得不夠好，而是問題本身的根源不在這裡。** 真正造成PLTR 14/24筆申報gap超標的原因目前仍未知，可能候選（這輪沒有查證，留給未來如果要繼續深挖）：COVID期間SEC曾提供暫時性申報寬限（2020年初，時間點跟PLTR IPO接近但PLTR是2020-09上市，晚於多數COVID寬限令的窗口，需要查證是否仍適用）、真實的申報延遲（不是分類錯誤，是公司自己真的晚交）、gap計算方式本身的定義問題（例如`filingDate`−`reportDate`的計算基準跟filer category deadline的計算基準可能不是同一件事）。

**依`MARATHON_PROTOCOL.md`第74輪就已預告的退路**：這個結果符合第41輪『退而求其次』選項的適用情境——**建議`era_reliability()`維持現狀（用途限縮成「只信任已驗證過的長年掛牌大型股如AAPL/MSFT，其他一律標記`unverified`」），不要投入更多輪次嘗試用filer category反推來救PLTR這類近期IPO股的PIT可信度**。第12項到此可視為已有明確結論（負向結果），不是懸而未決。

完整程式碼見 `research/sec_edgar_filer_category_infer.py`（docstring含完整方法論跟四項LIMITATIONS）；本次執行輸出未落地成CSV（只是smoke test，六個財年的PLTR資料量小，直接看終端機輸出即可判讀，未來如果要做更大規模驗證才需要落地存檔）。
