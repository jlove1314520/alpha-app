    ...
    strategies = [
        build_strategy(
            ...
            limitations=[
                ...
                "B24-500判定：不及格（兩次獨立跑法——原始跑+atomic write修法後的乾淨重跑——"
                "皆不顯著，p=0.2672/0.1441與0.3795/0.1685），策略報酬雖贏過買進持有+隨機對照組"
                "但無法排除beta曝險解釋，App選股頁「尚未經過組合策略回測驗證」字樣繼續掛著",
                "可重現性殘留限制（2026-08-29乾淨重跑新發現）：精確數字不可逐位元重現"
                "（TRAIN報酬75.87%→71.79%），根因是FinMind額度即時狀態導致每次跑因子完整度"
                "不同（不是快取寫入損毀，那個bug已修好）——判定結論(不及格)在兩次跑之間穩健，"
                "但精確數字不穩健，見B24_RESULTS.md「可重現性乾淨重跑」章節",
            ],
        ),
        build_strategy(
