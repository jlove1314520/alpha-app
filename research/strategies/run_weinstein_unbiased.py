"""Runner: Weinstein Stage 2, unbiased universe + matched control group.

Corrects two methodology issues Cowork flagged in the first pilot run
(weinstein_stage2_pilot_v1, see LEADS.md/STRATEGY_LOG.md 2026-08-22):

1. **Universe**: the first run used a hand-picked list of 30 well-known
   large-cap names -- a form of hindsight/survivorship selection bias (they
   were picked BECAUSE they're well-known winners). This run instead draws
   a fixed-seed RANDOM sample from universe.py's full post-2003 universe
   (currently-active + delisted-since-2003, 3,196 names), which is the
   unbiased population the strategy should actually be judged against.
   **Still a scope limitation, disclosed**: this is a random SAMPLE
   (SAMPLE_SIZE below), not every one of the 3,196 names -- fetching full
   daily history for all of them would be several thousand FinMind calls
   and risks the free-tier rate limit. Sampling uniformly at random from
   the correct population is a materially different (and much less biased)
   choice than hand-picking known winners, even though it's still not a
   full-market scan.
2. **Control group**: replaced the static buy&hold comparison with
   validation.control_group.run_matched_control_group(), which reuses the
   real strategy's own trade schedule (same entry/exit dates -> same
   rebalance frequency, same position count over time, same holding-period
   distribution) and only randomizes which stock filled each slot.

Run with: `python -m strategies.run_weinstein_unbiased` from research/.
"""
...
def load_sample_price_data(stock_ids: list[str]) -> dict[str, pd.DataFrame]:
    ...
    for i, sid in enumerate(stock_ids):
        ...
        except Exception as e:  # noqa: BLE001 -- some sampled names (esp. delisted ones) may error; log and skip
            print(f"  [{i+1}/{len(stock_ids)}] {sid}: ERROR ({e}), dropping")
            continue
        if df.empty or len(df) < 200:  # not enough history to ever populate a 150-day MA
...
def main():
    ...
    print("Loading sampled price data (cached after first run)...")
    raw_price_data = load_sample_price_data(sample_ids)
    print(f"  {len(raw_price_data)}/{SAMPLE_SIZE} usable names loaded")

    market_raw = load_dev("TaiwanStockPrice", "TAIEX", START_DATE)
    holdout.assert_no_holdout_leakage(market_raw, context="market_raw in run_weinstein_unbiased")
    ...
    val_result.trades.to_csv("data/backtests/weinstein_stage2_unbiased_validation_trades.csv", index=False)
    val_result.equity_curve.to_csv("data/backtests/weinstein_stage2_unbiased_validation_equity.csv", index=False)
    train_result.trades.to_csv("data/backtests/weinstein_stage2_unbiased_train_trades.csv", index=False)
    train_result.equity_curve.to_csv("data/backtests/weinstein_stage2_unbiased_train_equity.csv", index=False)
