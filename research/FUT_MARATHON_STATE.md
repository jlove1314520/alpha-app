...
**最後更新：2026-08-25T16:04:29+08:00**（馬拉松第66輪期貨軌執行後，取鎖乾淨成功`LOCK_ACQUIRED`，無陳舊鎖檔；basis家族地基第一步——確認TAIEX現貨指數資料源，`TaiwanStockPrice`/`data_id=TAIEX`可用且全歷史6185列乾淨無異常，未動`continuous_contract.py`本身）
...
1. **basis家族地基第二步：寫近月期貨選取邏輯（參考`continuous_contract.py`既有的近月定義即可，不需重新設計）跟`TaiwanStockPrice`/`TAIEX`現貨的逐日join，實際算出basis序列（近月期貨收盤/結算價 − TAIEX現貨收盤）**，檢查join覆蓋率（列數巧合不代表交易日曆100%重合）跟basis分布合理性（正常應該是小幅貼水/升水，不應有離譜異常值）。這是完全獨立於夜盤問題的另一條路，可以隨時接手。
