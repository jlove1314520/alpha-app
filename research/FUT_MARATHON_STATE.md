...
**最後更新：2026-08-25T13:03:21+08:00**（馬拉松第60輪期貨軌執行後，取鎖乾淨成功`LOCK_ACQUIRED`，無陳舊鎖檔；接手盤別效應家族地基第一步，探測`after_market`夜盤原始資料形狀，發現未解決的時序假設風險，尚未動`continuous_contract.py`本身）
...
1. **優先**：驗證夜盤`date`欄位的時序假設——用夜盤收盤跟隔日日盤開盤的相關性當間接證據，或查TAIFEX官方交易時間文件確認。**這是動`continuous_contract.py`寫夜盤連續序列之前的必要前置步驟，不能跳過**，否則後續任何盤別效應假說的方向判定都不可信。
2. 若這輪時間/額度不夠驗證時序，改測期現價差basis家族地基（需要新增台股加權指數現貨資料源，是完全獨立的另一條路，不受夜盤時序問題阻擋）。
