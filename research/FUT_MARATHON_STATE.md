...
**最後更新：2026-08-25T10:05:00+08:00**（馬拉松第54輪期貨軌執行後，取鎖乾淨成功`LOCK_ACQUIRED`，無陳舊鎖檔；換到日內均值回歸家族第一批，地基確認完成，測了2個相反方向假說）
...
1. **優先**：`fut_intraday_gap_continuation`（percentile=92.0）離本批次校正門檻95.0不遠，值得先用更高`N_SHUFFLES`（例如200→2000，同第51輪對三大法人動能假說的處理方式，新寫monkey-patch腳本，不改`fut_cheap_gate.py`本身的預設值）重測，排除是不是解析度不足造成的模糊——如果高解析度後percentile穩定在92附近甚至更低，就是確定沒過，可以結案；如果明顯上升，才需要進一步討論。
2. （次優先）若上述已確認結案，改測剩餘候選家族（`MARATHON_PROTOCOL.md` 第3節，同樣需要小型地基工作）：(a) 期現價差（basis，近月期貨 vs 現貨指數，**需要新增台股加權指數現貨資料源，這是新的小型資料依賴，不是純技術面**）；(b) 盤別效應（日盤 vs 夜盤）——**需要先把`after_market` session納入連續合約建構，這也是地基工作，目前連續合約只用日盤**。同樣一輪最多測2–3個假說，用便宜關卡先篩；地基做不完就只做一半，留給下一輪接手。
...
6. **（較低優先，不擋路）** 目前累計12個FAIL、3個「單測/批次過但累積校正未過或未過批次門檻」，值得留意但不是本輪判斷：這批便宜關卡本身（原200次排列、90百分位門檻）對台指期日頻這個市場/頻率組合是否系統性偏嚴格，還是台指期本身在日頻、無成本、單一訊號框架下確實難找到邊際——**不要在還沒有更多證據前就調整門檻本身或懷疑`fut_cheap_gate.py`框架本身有問題**。
...
見上方「下一輪建議工作單位」，一次只做一項。下一個馬拉松輪次接手時，先讀 `FUT_LOG.md` 最新一條看上一輪實際做到哪裡，再讀 `fut_cheap_gate.py` 了解目前已建立的便宜關卡測試框架（配對式隨機排列控制組，可以直接照樣加新的假說函式進去，不用重寫框架），再讀 `FUT_CONTINUOUS_CONTRACT_DESIGN.md` 了解連續合約設計決策全貌（含漂移幅度量測結果，地基章節已標記完成）。**開始測新因子/策略時，先確認回看窗口長度合理（短中期已驗證安全；有需要可以直接呼叫 `fut_drift_probe.py` 的邏輯查特定窗口長度的漂移量級），並比照 `TW_MARATHON_STATE.md`／`US_MARATHON_STATE.md` 的先例，判定結果記進 `TRIALS_LEDGER.md`（累積總帳）跟 `FUT_LEADS.md`（本軌候選）。已測過FAIL的11個訊號（純技術面/籌碼面/季節性6個 + 三大法人部位水位×3類別+自營商動能4個 + 日內跳空反轉1個）不要重測相同設定。**三大法人期貨部位家族已完全結案（第39輪補齊地基，第42/45/48輪分別測完外資/投信/自營商三類別，第51輪用高解析度重測確認最後2個「不確定」假說也未過：六個假說最終結果4 FAIL+2確認未過+0 PASS），不再是接手優先項，也不需要再回頭補做任何變體。日內均值回歸家族第54輪已確認地基不需要新資料源（`build_continuous_series()`的`adj_open`就夠），`fut_intraday_gap_continuation`（percentile=92.0，方向清楚但只過單測、沒過本批次校正）是目前優先跟進項——先高解析度重測排除模糊地帶，再決定要不要換到basis／盤別效應。剩餘2個候選家族（basis／盤別效應）仍需要先做小型地基/新資料源工作，不能像前13個一樣直接套用既有`build_continuous_series()`輸出，接手時要先評估這輪時間預算夠不夠做完地基那部分。
