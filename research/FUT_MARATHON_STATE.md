...
見上方「下一輪建議工作單位」，一次只做一項。**`fut_basis_carry`深挖已於第75輪完成並結案為FAIL（見上方最新段落、`FUT_LEADS.md`#17、`TRIALS_LEDGER.md`#37），不要再回頭深挖或重測——這條線已經走完全部流程（便宜關卡CHEAP_PASS→1b深挖FAIL→降級），是期貨軌第一個完整跑過1b全部四項檢查的候選，也證明了「便宜關卡通過≠可信候選」，下一輪要吸取這個教訓：任何新的CHEAP_PASS進深挖時，把train/val切分放第一步，不要留到最後才做。**最高優先：basis家族第三個假說（均值回歸，見上方#1）。下一個馬拉松輪次接手時，先讀 `FUT_LOG.md` 最新一條看上一輪實際做到哪裡，再讀 `fut_cheap_gate.py` 了解目前已建立的便宜關卡測試框架（配對式隨機排列控制組，可以直接照樣加新的假說函式進去，不用重寫框架），需要時也可參考`deep_dive_fut_basis_carry.py`當深挖階段的範本（train/val切分、leave-one-year-out、成本敏感度、beta對照四項checklist的具體寫法）。**已測過FAIL/CHEAP_PASS-但降級的18個訊號（純技術面/籌碼面/季節性6個 + 三大法人部位水位×3類別+自營商動能4個 + 日內跳空反轉/順勢2個 + basis carry水位1個(深挖後降級)+basis動能1個，含各自的高解析度重測）不要重測相同設定。**三大法人期貨部位家族、日內均值回歸家族第一批都已完全結案（細節同前，不再是接手優先項）。**盤別效應家族地基：第63輪解決夜盤時序方向，但轉倉時點是否跟日盤同步還沒查，動工前先確認。**basis家族地基已完整可用（第66/69輪完成），第72輪測完水位/動能（水位便宜關卡曾過但第75輪深挖後FAIL、動能便宜關卡FAIL），第三個方向（均值回歸）是這條分支剩下唯一的選項。
