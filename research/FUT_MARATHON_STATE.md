...
**最後更新：2026-08-25T08:32:52+08:00**（馬拉松第51輪期貨軌執行後，取鎖乾淨成功`LOCK_ACQUIRED`，無陳舊鎖檔；三大法人期貨部位家族最後兩個「不確定」假說已用高解析度重測確認結案）
...
1. **優先**：三大法人期貨部位家族已完全結案，接手時改測剩餘候選家族（`MARATHON_PROTOCOL.md` 第3節，同樣需要小型地基工作）：(a) 日內均值回歸——需先確認日頻資料是否支援拆解出「日內」報酬（可能要用隔夜vs當日拆解代替，需要先查證資料形狀）；(b) 期現價差（basis，近月期貨 vs 現貨指數，**需要新增台股加權指數現貨資料源，這是新的小型資料依賴，不是純技術面**）；(c) 盤別效應（日盤 vs 夜盤）——**需要先把`after_market` session納入連續合約建構，這也是地基工作，目前連續合約只用日盤**。同樣一輪最多測2–3個假說，用便宜關卡先篩；地基做不完就只做一半，留給下一輪接手。
...
5. **（較低優先，不擋路）** 目前累計10個FAIL、2個「單測過但累積校正確認未過（非解析度問題）」，值得留意但不是本輪判斷：這批便宜關卡本身（原200次排列、90百分位門檻）對台指期日頻這個市場/頻率組合是否系統性偏嚴格，還是台指期本身在日頻、無成本、單一訊號框架下確實難找到邊際——三大法人期貨部位家族六個假說全測完並經高解析度確認（4 FAIL+2確認未過+0 PASS）進一步累積了「這個框架確實嚴格」的證據，但仍需要更多資料點（例如換一個完全不同市場的對照組結果，或看深挖階段的結果）才能下結論，**不要在還沒有更多證據前就調整門檻本身或懷疑`fut_cheap_gate.py`框架本身有問題**。
