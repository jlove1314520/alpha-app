...
**最後更新：2026-08-24T23:03:00+08:00**（馬拉松第42輪期貨軌執行後，取鎖時偵測到`LOCK_STALE`，上一輪pid 120808持有鎖滿30分鐘後被回收，第39輪之後某一輪疑似完全沒跑完就異常中止，未留下任何log；本輪開始前state檔案跟第39輪記錄一致，無資料落差）
...
1. **優先**：三大法人期貨部位家族還有兩個類別沒測（投信、自營商）——直接沿用本輪新增的`_load_institutional_net_position(series, category=...)`換`category`參數（`"投信"`／`"自營商"`）重跑水位/動能兩型假說，不用重寫地基。**同樣記得樣本只有1605天，統計檢定力天生較低。**
2. 若優先序判斷值得：把`fut_inst_foreign_net_position_change_5d`的`N_SHUFFLES`從200加密（例如→1000+，比照`f_quality_roe_stability`深挖時20→100加密隨機抽樣解析度的做法）重新檢驗是否能跨過累積校正門檻（目前n=25時門檻99.6，本輪97.0未過但只差一點，且200次排列的解析度只到0.5%，無法精細判斷）——**先評估時間預算，1605天樣本＋更高N_SHUFFLES的執行時間目前未知，可能接近30分鐘鎖檔窗口**。
...
7. **（較低優先，不擋路）** 目前累計7個FAIL、1個「批次過但累積校正後不確定」，值得留意但不是本輪判斷：這批便宜關卡本身（200次排列、90百分位門檻）對台指期日頻這個市場/頻率組合是否系統性偏嚴格，還是台指期本身在日頻、無成本、單一訊號框架下確實難找到邊際——需要更多資料點（例如換一個完全不同市場的對照組結果，或看深挖階段的結果）才能下結論，**不要在還沒有更多證據前就調整門檻本身或懷疑`fut_cheap_gate.py`框架本身有問題**。
