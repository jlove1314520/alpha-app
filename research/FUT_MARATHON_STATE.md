...
**最後更新：2026-08-25T05:02:00+08:00**（馬拉松第45輪期貨軌執行後，取鎖乾淨成功`LOCK_ACQUIRED`，無陳舊鎖檔）
...
1. **優先**：三大法人期貨部位家族最後一個類別沒測（自營商）——直接沿用`_load_institutional_net_position(series, category="自營商")`重跑水位/動能兩型假說，不用重寫地基。測完這個家族就三類別（外資/投信/自營商）全覆蓋。**同樣記得樣本只有1605天，統計檢定力天生較低。**
2. 若優先序判斷值得：把`fut_inst_foreign_net_position_change_5d`跟`fut_inst_trust_net_position_change_5d`的`N_SHUFFLES`從200加密（例如→1000+，比照`f_quality_roe_stability`深挖時20→100加密隨機抽樣解析度的做法）一次重新檢驗是否能跨過累積校正門檻（兩者都卡在同樣模式，值得一起重新檢驗）——**先評估時間預算，1605天樣本＋更高N_SHUFFLES的執行時間目前未知，可能接近30分鐘鎖檔窗口**。
3. 剩餘候選（`MARATHON_PROTOCOL.md` 第3節，同樣需要小型地基工作）：(a) 日內均值回歸——需先確認日頻資料是否支援拆解出「日內」報酬（可能要用隔夜vs當日拆解代替，需要先查證資料形狀）；(b) 期現價差（basis，近月期貨 vs 現貨指數，**需要新增台股加權指數現貨資料源，這是新的小型資料依賴，不是純技術面**）；(c) 盤別效應（日盤 vs 夜盤）——**需要先把`after_market` session納入連續合約建構，這也是地基工作，目前連續合約只用日盤**。同樣一輪最多測2–3個假說，用便宜關卡先篩。
4. （較低優先，不擋路）用 FinMind 官方欄位文件或另外查證確認 `after_market`＝夜盤的推論是否正確（目前只是起始日期吻合的間接證據）。
5. （較低優先，不擋路）連續合約累積漂移的經濟成因未拆解（可能跟台股高股息殖利率、期現貼水有關）——可以另立為一個獨立假說排進候選清單，但不急。
6. （較低優先）若之後有策略需要用到夜盤資料，才需要把 `after_market` session 也納入連續合約建構——目前候選策略清單用日頻資料即可，不急。
7. **（較低優先，不擋路）** 目前累計8個FAIL、2個「批次過但累積校正後不確定」（且2個不確定案例都在同一個「淨部位5日動能」訊號家族，外資+投信），值得留意但不是本輪判斷：這批便宜關卡本身（200次排列、90百分位門檻）對台指期日頻這個市場/頻率組合是否系統性偏嚴格，還是台指期本身在日頻、無成本、單一訊號框架下確實難找到邊際——需要更多資料點（例如自營商類別的結果、換一個完全不同市場的對照組結果，或看深挖階段的結果）才能下結論，**不要在還沒有更多證據前就調整門檻本身或懷疑`fut_cheap_gate.py`框架本身有問題**。
...
見上方「下一輪建議工作單位」，一次只做一項。下一個馬拉松輪次接手時，先讀 `FUT_LOG.md` 最新一條看上一輪實際做到哪裡，再讀 `fut_cheap_gate.py` 了解目前已建立的便宜關卡測試框架（配對式隨機排列控制組，可以直接照樣加新的假說函式進去，不用重寫框架），再讀 `FUT_CONTINUOUS_CONTRACT_DESIGN.md` 了解連續合約設計決策全貌（含漂移幅度量測結果，地基章節已標記完成）。**開始測新因子/策略時，先確認回看窗口長度合理（短中期已驗證安全；有需要可以直接呼叫 `fut_drift_probe.py` 的邏輯查特定窗口長度的漂移量級），並比照 `TW_MARATHON_STATE.md`／`US_MARATHON_STATE.md` 的先例，判定結果記進 `TRIALS_LEDGER.md`（累積總帳）跟 `FUT_LEADS.md`（本軌候選）。已測過FAIL的8個訊號（純技術面/籌碼面/季節性6個 + 三大法人部位水位2個，外資/投信）不要重測相同設定。**三大法人期貨部位地基已補齊（第39輪，`fut_probe_institutional_positions.py`，全歷史抓取結果已快取），外資/投信兩個類別的水位/動能假說都已測完（第42/45輪），可以直接接手測自營商類別（同函式換`category="自營商"`），但**必須記得樣本只有1605天（2018-06-05起）不是全歷史**。剩餘2個候選家族（basis／盤別效應）仍需要先做小型地基/新資料源工作，不能像前6個一樣直接套用既有`build_continuous_series()`輸出，接手時要先評估這輪時間預算夠不夠做完地基那部分。
