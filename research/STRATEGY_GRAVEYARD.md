...
理由不是嘴軟，是**資料維度不同**：
- **#55**（三大法人買賣超**廣度**速度）用的是**法人流向**，
  跟已死的三個（信用餘額／報酬離散度／成交值集中度）**完全不同的資料來源**。
- **#57**（當沖比重速度）用的是**投機參與度**，同樣是新的維度。

已死的三個共同點是「價量衍生的截面統計量」；#55／#57 不是。
把它們預先判死，等於用三個價量指標的失敗去否定兩個**還沒測過的行為面指標**——
這正是墓園寫入規則第 3 條（不泛化成整類沒用）要防的錯誤，
也是當初 `fut_basis_carry` 死了但 `fut_basis_mean_reversion_60d` 後來測出
EXPERIMENTAL 的同一個教訓。

**結案條件（鐵律，兩邊都不准偷）**：
1. **#55 與 #57 照原規格跑完**，才可以做家族結案。
2. **不得提前判死**——不能因為 0 勝 3 敗就跳過 #55／#57。
3. **不得為了救活而放寬門檻**——不能因為家族壓力大就調鬆 #55／#57 的關卡。
   要豁免任何一關，必須換一個等價嚴格的檢定，不是拿掉它。

**注意一個統計陷阱**：這三次失敗**不是三份獨立證據**。
若融資餘額、報酬離散度、成交值集中度彼此相關係數 > 0.7，
依「同家族因子只能算一個獨立發現」的同一個道理，
它們的**失敗**也應該當成**大致一份證據**，不是三份。
換句話說，**現有證據比「0 勝 3 敗」聽起來的更弱，更不足以支撐泛化。**
（相關係數尚未實測，這裡只提出方向，不當結論。）
