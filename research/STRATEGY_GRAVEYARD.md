...
## 【家族層級】橫斷面離散度速度（台股）——2026-09-08 建立，**尚未結案**
...
**明確聲明：尚未泛化為「此機制大類在台股無效」。**

理由不是嘴軟，是**資料維度不同**：
- **#57**（當沖比重速度）用的是**投機參與度**，同樣是新的維度，尚未測過。

已死的四個共同點是「價量／信用／法人資金衍生的截面統計量」，
但彼此的**具體來源與規模標準化方式**各自不同（見上表），
不構成「橫斷面統計量這種構造本身沒用」的證據——
這正是墓園寫入規則第 3 條（不泛化成整類沒用）要防的錯誤，
也是當初 `fut_basis_carry` 死了但 `fut_basis_mean_reversion_60d` 後來測出
EXPERIMENTAL 的同一個教訓。

**結案條件（鐵律，兩邊都不准偷）**：
1. **#57 照原規格跑完**（含前置的三來源查證+回填），才可以做家族結案。
2. **不得提前判死**——不能因為 0 勝 4 敗就跳過 #57。
3. **不得為了救活而放寬門檻**——不能因為家族壓力大就調鬆 #57 的關卡。
   要豁免任何一關，必須換一個等價嚴格的檢定，不是拿掉它。

**注意一個統計陷阱**：這四次失敗**不是四份獨立證據**。
本輪已實測部分相關係數：#55與#53相關係數corr_level=+0.463/corr_vel=+0.134、
#55與#54相關係數corr_level=+0.029/corr_vel=+0.148，皆<0.7門檻，
確認#55與前兩者非同家族（詳見`TRIALS_LEDGER.md`#198與#55自身條目）。
若融資餘額、報酬離散度、成交值集中度、法人流離散度彼此仍有部分相關，
依「同家族因子只能算一個獨立發現」的同一個道理，
它們的**失敗**也應該按實測相關係數的高低打折扣，不是機械數成四份。
換句話說，**現有證據比「0 勝 4 敗」聽起來的更弱，更不足以支撐泛化。**
