...
**更正（2026-09-23總司令裁示【稽核解封＋S2對等比較＋凍結regime
家族】驗.二，結案`TRIALS_LEDGER.md`#346）**：修.二稽核曾發現此檔案的
ETF證交稅率修正（0.3%→0.1%）會讓第6關逐年一致性從FAIL翻轉為PASS並
續行過第7-9關（登記為#346，verdict暫標「未結案」等待總司令審閱，見
`AWAITING_REVIEW.md`）。但同一輪【三個方法缺陷＋E-c/E-d走正式閘門】
驗.二已獨立發現一個更根本的問題：`spillover_overlay_v1.py` L73用
`tw_ret`(台股t-1收盤→t收盤報酬)乘上「美股t日訊號決定的曝險」，但美股
t日訊號要等台股t-1收盤之後才會知道（美股與台股交易時段的時區關係），
用收盤到收盤的報酬去乘一個「訊號公布時已經錯過」的曝險，中間吃進一段
本來不可交易的隔夜跳空——這是前視偏誤，不是稅率或參數層級的問題，
**不論稅率修正後的統計數字多漂亮都不影響這個結論**：訊號本身的建構
方式讓回測看到了實際交易時看不到的報酬。**#346正式判定FAIL（前視
偏誤），不再是「未結案」**，`AWAITING_REVIEW.md`對應項目移入已結案。
開盤到收盤（t日開盤→t日收盤）的重建版本另外排入`PENDING_QUEUE.md`
驗.二第二部分，屬於新試驗，不是本筆的翻案。
...
**2026-09-23補測（`PENDING_QUEUE.md`常備.10，DevQueue自走）：個股層級事件研究+
短窗口已測，結案FAIL**——`insider_event_level_shortwindow.py`（新增），只用
開放市場買入（`code='P'`且`acq_disp='A'`，排除選擇權履約/贈與），個股層級事件
＝issuer x filing_date聚合（非跨月）、進場＝filing_date後第一個交易日、pooled
Spearman IC。**t+5**：TRAIN n=82987 IC=+0.0141、VAL n=34630 IC=+0.0100、同號、
null percentile=93.6（門檻90.0，**過**）。**t+10**：TRAIN IC=+0.0155、VAL
IC=+0.0033、同號，但null percentile驟降到46.2（未過）。**t+20**（對照，跟#324
既有結果比較用）：train/val正負號翻轉（+0.0206→-0.0100），跟#324整月聚合版
在t+20的FAIL方向一致。**判準要求t+5與t+10皆PASS才算過，t+10未過，整體FAIL**——
訊號在t+5有微弱但過關的證據、t+10迅速衰退成雜訊、t+20方向翻轉，呈現「極短期
可能有真實但微弱的資訊消化窗，但窗口比5個交易日還短，本次t+5/t+10兩點解析度
不夠精細去定位確切衰退點」的形狀，不宣稱找到有效訊號（t+10不過就整體判FAIL，
不因t+5單獨過關就拆開宣稱部分勝利）。**存活者偏誤但書**：價格覆蓋僅34.7%
（4965/14306 ticker），繼承#75/#324既有缺口，結果僅代表有覆蓋子樣本。原始
記錄：`TRIALS_LEDGER.md`#372。`PENDING_QUEUE.md`常備.10同步標記完成，並更正
其原始文字誤把本章節（內部人交易）的「未測」缺口寫成「月營收」（來源誤植，
詳見常備.10條目本身的更正說明）。

## 事件驅動SUE訊號（E1財報公布/E2月營收公布）GATE_SEQUENCE驗證（2026-09-22結案，TRIALS #325/#326）

> **⚠️ 2026-09-23更正（方法.三續.E1重判）：E1不再是FAIL，已改判CHEAP_
> PASS，本節以下對E1的死因敘述已過時，僅保留供稽核追溯（記錄「當時為何
> 判FAIL」本身仍有價值，說明的是執行偏離不是市場沒訊號）。**
>
> **根因**：下方2026-09-22的死因敘述裡「E1財報SUE：訊號統計量0.0064，
> 未超過控制組最大值0.0093」用的`signal_stat`是`median_diff`——但
> `PENDING_QUEUE.md`方法.二登記的通過判準是「(b)P90差異或(c)右尾佔比
> 顯著」，從未登記median_diff為判準。這是`gate_random_control`的執行
> 偏離（程式碼用了沒被登記的統計量決定生死），不是原本的統計方法本身
> 有問題，也不是2026-09-07控制組標準升級用錯。
>
> **重判結果**（`gate_random_control`改用(b)/(c)/(d)三統計量、1000次
> null重抽、訊號嚴格超過第999名之門檻，見`event_driven_gate_sequence.
> py::_exceeds_rank_999()`）：(b)P90差異訊號+0.0400嚴格超過999名門檻
> +0.0384，**通過**；(c)右尾佔比差異訊號+0.0292未超過999名門檻+0.0340，
> 未通過；(b)或(c)其一通過即滿足OR判準，加上(d)左尾佔比訊號-0.0021未
> 顯著惡化（<=999名門檻+0.0199）——隨機控制組閘門整體改判通過。其餘
> 3關（train-val OOS/成本敏感度/leave-one-out）本來就已PASS，未改動。
> **E1完整4關全過，改判CHEAP_PASS**。已登記TRIALS_LEDGER #332((b)判準，
> CHEAP_PASS)/#333((c)判準，FAIL，OR判準下不影響整體)。
>
> **E2不受影響**：E2（月營收SUE，#326）隨機控制組與train/val OOS雙重
> FAIL，裁示原文明確「不重跑」，維持FAIL結案，不適用本次更正。
>
> **續.B結果（2026-09-23，同日完成）：E1最終仍是FAIL**——波動度配對
> 控制組複驗證實續.B裁示原文預先點名的假陽性風險命中：3維(原配對)
> p90_diff=+0.0413，4維(加計事件前60交易日已實現波動度五分位配對)
> p90_diff崩到+0.0178，崩掉56.9%（門檻50%）。訊號組事件前波動度中位數
> 0.0233明顯高於3維對照組的0.0199，4維配對後對照組波動度升到0.0235
> （與訊號組相近）——**續.A通過的(b)P90判準，主要成分是波動度差異，
> 不是SUE本身的alpha**。已登記`TRIALS_LEDGER.md`#334（FAIL）。
> **E1與E2至此雙雙結案FAIL**，事件驅動大類（財報/月營收SUE二元分組）
> 沒有候選進入下一步（`PENDING_QUEUE.md`規.二集中版框架目前沒有訊號
> 可用，需要另尋新的進場訊號設計或先解決波動度控制的問題再重測）。

**死因（流程對、這條假設無edge，2026-09-22原始判定，E1部分已於上方
2026-09-23更正，E2維持不變）**：`event_driven_prototype.py`（#323原型，
...
## #78 美國高收益債ETF/公債ETF價格比值（HYG/IEF）代理信用利差當TAIEX regime降曝險訊號（2026-09-22結案）

**死因**：第1關cheap gate，`hy_etf_ratio_gate.py`——訊號=HYG/IEF收盤價
比值水位、目標=TAIEX後20交易日報酬、TRAIN(<=2020-12-31)/VAL(2020-12-31~
2024-12-31)兩期Pearson+Spearman相關性+N=500洗牌null。**train/val正負號
相反**：TRAIN r=-0.1338(p=0.0000，n=3266)，VAL r=+0.1580(p=0.0000，
n=917，null percentile=100.0，表面完全過關)。依三項判準之一「train/val
同號」為必要條件，正負號相反直接判FAIL，不因VAL單期看似贏過洗牌null而
放行——這正是#32美元兌台幣匯率、#75內部人淨額(價格版)、#76 VIX期限
結構同一種死法（train/val正負號相反）。VAL期方向符合事前綁定預期
（比值高=風險偏好升溫→TAIEX轉強，正相關），但TRAIN期方向不符，兩期
不一致本身就是判死的核心理由，不因VAL單獨方向正確而放寬。見
`TRIALS_LEDGER.md`#330。

**不泛化成「信用市場定價機制對台股regime沒用」**——這是本佇列第一次
真正完成信用利差這個經濟機制的測試（#77死於資料不可及，機制從未被
測），VAL期方向與顯著性皆符合事前預期，只是TRAIN期不穩定，死的是
「比值水位+M=20單一窗口」這個具體操作化，未測變動率（速度訊號而非
水位）、未測其他窗口（5/10/60日）。地基查證（`hy_etf_ratio_probe.py`，
確認HYG起點2007-04-11、IEF起點2002-07-30皆早於TRAIN_END）保留，可
重複執行供未來變體使用。


## #80 台灣失業率YoY當TAIEX regime降曝險訊號（2026-09-23結案）

**死因**：第1關cheap gate，`dgbas_unemployment_gate.py`——訊號=DGBAS
月度失業率總計欄位YoY差值（百分點差，非成長率，T+30天發布延遲對齊）、
目標=TAIEX後20交易日報酬、TRAIN(<=`TRAIN_END`)/VAL(`TRAIN_END`~
`VAL_END`)兩期Pearson+Spearman相關性+N=500洗牌null。TRAIN r=+0.1205
（p<0.0001，n=5775，null percentile=100.0，表面完全過關）；**VAL
r=+0.0516（p=0.1124，n=949，null percentile=88.2 < 90.0門檻）**。
判準三項中「VAL贏過洗牌null」未過（88.2<90.0），且**VAL期方向為正，
與事前綁定「失業率YoY上升→TAIEX負相關」方向相反**——兩個獨立理由
皆足以單獨判死，疊加後更無爭議。見`TRIALS_LEDGER.md`#345、
`TRIALS_REGISTRY.jsonl` id=345。

**不泛化成「勞動市場/實體經濟基本面對台股regime沒用」**——這是本佇列
第一次真正測試實體經濟基本面（相對於先前10個訊號#31~#34/#76~#79
全部來自金融市場價格），死的是「失業率水準YoY差值+T+30發布延遲+
M=20交易日單一窗口」這個具體操作化，未測經常性薪資成長率（原SPEC
路徑3，因路徑1DGBAS失業率已可行而未查）、未測勞動力參與率
（`#80續1`查證時順帶發現同樣存在的替代序列dataset 6636，未實測）。
資料源本身（DGBAS`mp0101a07.xml`，1978年起連續至2026M08）品質良好、
免認證免CAPTCHA，可重複執行供未來變體使用。


## E-c移動停損10%／E-d MA200 regime出場（2026-09-23結案，驗.四）

**死因**：`REGIME_OVERLAY_PROTOCOL.md`正式閘門，1000次隨機擇時對照組
（block permutation，保證空手總天數與切換次數跟真實序列完全相同，只
隨機化日期落點）——E-c真實表現的保守p值=0.062、E-d=0.394，皆未達
裁示寫死的門檻p<=0.01。也就是說在同樣的空手天數比例、同樣的切換
次數下，隨機挑選進出場時機也有約6%（E-c）或39%（E-d）的機率跟這條
規則一樣好或更好，不足以排除運氣。VAL期兩者MDD都守住天條一
（E-c −32.11%、E-d −21.30%，皆優於−50%），但天條一只是必要條件不是
充分條件，隨機對照組這關沒過就整體FAIL。見`TRIALS_LEDGER.md`
#359(E-c)/#360(E-d)、`PENDING_QUEUE.md`驗.四條目。

**不泛化成「出場規則/降曝險機制本身沒用」**——這是繼binary趨勢濾網
（#243/#244）、已實現波動度（#247）、回撤斷路器（#246）之後，第五、
六個死在同一種形狀的regime/出場機制：方向性道理都成立（下跌就減碼），
但統計檢定力不足以把「真本事」跟「運氣」分開。死的是「移動停損10%／
MA200單一均線」這兩個具體操作化，不是「出場規則」這個類別本身。
`exit_rule_lab.py`（規.三）的模擬引擎與冷卻期重入邏輯保留，可重複
執行供未來變體使用（若總司令要測其他停損%/均線窗口，重跑
`research/regime_overlay_exit_rule_gate.py`即可，不需重寫）。

**附註（2026-09-23總司令裁示【驗.一收尾優先＋資料零價格稽核＋核心
檔案單一寫入者】驗.四配套要求）**：E-c事前登記的參數點（停損10%／
冷卻20交易日）**剛好是24格參數高原網格裡數值最高的一格**，是一個
「峰值」而不是「一整片都好」的真正高原——這代表就算隨機控制組p值
再低一點勉強過關，這個點本身也帶著挑格子的嫌疑，不能只看它自己
表現好就當作穩健訊號的證據，是另一層獨立於p=0.062那個FAIL理由之外
的警訊，一併記錄供未來同類機制參考。
...
## #83 VIX絕對水位（不取VIX9D/VIX比值）當TAIEX regime降曝險訊號
（2026-09-23結案，`PENDING_QUEUE.md`常備.3）

**死因**：第1關cheap gate，`vix_level_gate.py`——訊號=VIX(^VIX)收盤
水位本身（狀態性訊號，不取比值不取變動率）、目標=TAIEX後20交易日
報酬、TRAIN(<=2020-12-31)/VAL(2020-12-31~2024-12-31)兩期Pearson+
Spearman相關性+N=500洗牌null。**train/val正負號相反**：TRAIN
r=+0.0932(p=0.0000，n=5590)，VAL r=-0.1174(p=0.0004，n=917，null
percentile=100.0)。VAL期方向（負相關）完全符合事前綁定預期且顯著
贏過洗牌null，但TRAIN期方向與預期相反（正相關），依「同號」必要
判準判FAIL，不因VAL單期漂亮而放行。見`TRIALS_LEDGER.md`#364。

**這是本佇列regime/timing類假設第5次出現同一種死亡模式**（train期
方向不穩定、VAL期方向正確且顯著）：`#32`美元兌台幣匯率、`#75`內部人
淨額(價格版)、`#76`VIX期限結構比值、`常備.2`VIX期限結構變動率版、
本條VIX絕對水位——五個不同的總體/情緒訊號，同一種失敗形狀。**值得
記錄的模式觀察（非正式判準，僅供後續研究方向參考）**：本條的VAL期
表現（percentile=100.0、方向正確、p=0.0004）是五個裡最強的一個，
但train期樣本數(n=5590，橫跨1997~2020共23年)遠大於VAL期(n=917，
2021~2024共4年)，train期涵蓋多個不同的市場regime（1997亞洲金融
風暴、2000科技泡沫、2008金融海嘯、2020新冠崩盤等），VIX水位與台股
報酬的關係可能本身就不是全樣本穩定的（regime-dependent的關係，
訊號本身可能只在特定次regime內有效）——這個假說未經檢定，不構成
本條判定翻案的理由，僅記錄供未來若要設計「regime-of-regime」類
研究時參考。

**不泛化成**：VIX絕對水位訊號在台股完全無用——只測了水位+M=20日
單一窗口這個具體操作化，未測其他窗口。地基（`vix_level_gate.py`本身
即為新增地基，VIX起點1990-01-02遠早於TRAIN_END）保留可重複使用。

## #84 HYG/IEF比值N日變動率版代理信用利差當TAIEX regime降曝險訊號
（2026-09-23結案，`PENDING_QUEUE.md`常備.4）

**死因**：第1關cheap gate，`hy_etf_ratio_roc_gate.py`——訊號=HYG/IEF
比值N(20)日變動率（沿用`fx_twd_gate.py`(#32)/常備.2既有N日變動率
窗口慣例）、目標=TAIEX後20交易日報酬、TRAIN(<=2020-12-31)/
VAL(2020-12-31~2024-12-31)兩期Pearson+Spearman相關性+N=500洗牌null。
TRAIN r=+0.0178(p=0.3107，n=3247)，VAL r=+0.0327(p=0.3221，n=917，
null percentile=73.6)。**train/val同號、VAL方向符合事前預期，但兩期
相關係數皆不顯著（p>0.31）、VAL未贏過洗牌null（73.6<90門檻）**。見
`TRIALS_LEDGER.md`#365。

**跟前幾筆regime/timing死亡模式不同**：`#76`/`#78`/`常備.2`/`常備.3`
都死於train/val正負號相反這個具體形狀；本條是train/val方向一致但
訊號本身太弱（兩期p值皆遠高於0.05），屬於乾淨的「無edge」FAIL，不是
統計偽影造成的假象——這代表水位版（#78）train/val正負號相反那次結果，
比較可能是水位訊號本身不穩定，而非「有真訊號但被雜訊蓋住」；換成
速度語言後訊號直接消失（趨近零），兩個獨立操作化（水位、變動率）
都拿不到穩健的正結果，強化「HYG/IEF比值對TAIEX regime沒有可用predictive
power」這個結論的證據權重。

**不泛化成**：只測了N=20日變動率+M=20日窗口這個具體操作化，未測其他
N窗口（5/10/60日，見`常備.5`）。地基（`hy_etf_ratio_probe.py`資料
可行性查證）保留可重複使用。

## #85 HYG/IEF比值水位版其他窗口（M=5/10/60日）當TAIEX regime降曝險訊號
（2026-09-23結案，`PENDING_QUEUE.md`常備.5）

**死因**：第1關cheap gate，`hy_etf_ratio_window_grid_gate.py`——訊號=
HYG/IEF比值水位本身（同#78），唯一改變自由度是預測目標窗口M，測
{5,10,60}三格（M=20已由#78測過不重測）。**三格全數train/val正負號
相反**：M=5 TRAIN r=-0.0605(p=0.0005) vs VAL r=+0.0626(p=0.0562)；
M=10 TRAIN r=-0.0919(p=0.0000) vs VAL r=+0.0908(p=0.0057)；M=60
TRAIN r=-0.2389(p=0.0000) vs VAL r=+0.3635(p=0.0000)。0/3格通過。
見`TRIALS_LEDGER.md`#366。

**關鍵發現**：這確認`#78`（M=20）的train/val正負號相反**不是M=20這個
特定窗口的偶然選擇**——四個窗口（M=5/10/20/60）全數同一個方向反轉：
train期恆為負相關、val期恆為正相關，且效果量隨窗口拉長而放大
（M=5時|r|約0.06，M=60時|r|約0.24~0.36）。這個模式更可能反映的是
「train期(2007~2020，涵蓋2008金融海嘯)與val期(2021~2024)這兩段
時期，HYG/IEF比值與TAIEX的關係本身系統性不同」，而非窗口雜訊——
train期橫跨的市場regime組成跟val期差異太大，這是結構性問題不是
可以靠換窗口解決的雜訊問題。

**不泛化成**：HYG/IEF比值訊號在其他市場/其他目標變數上無用——只
測了「對TAIEX後續報酬」這個具體目標。**至此HYG/IEF比值水位版
M∈{5,10,20,60}四個窗口與變動率版（`常備.4`）皆已測完，窗口敏感度
問題已排除，比值機制的信用利差代理假說整體證據強度已相當薄弱，
不建議再花算力測更多HYG/IEF比值變體**（除非總司令另有新的具體
操作化構想）。地基（`hy_etf_ratio_probe.py`）保留可重複使用。

**⚠️ 方法論誠實揭露（2026-09-23，`常備.2`~`常備.5`四筆試驗補記，
[自行裁量]，覆蓋`## #82`~`## #85`）**：這四個新腳本（`vix_term_
structure_roc_gate.py`／`vix_level_gate.py`／`hy_etf_ratio_roc_gate.py`／
`hy_etf_ratio_window_grid_gate.py`）沿用的是`vix_term_structure_gate.py`
（#76）／`hy_etf_ratio_gate.py`（#78）**原本的**daily-overlap（M日前瞻
報酬視窗逐日重疊）＋逐點`rng.permutation`打散虛無分布設計，**不是**
`regime_gate_common.py`的`align_monthly_nonoverlap()`/
`circular_shift_null()`修正版——`## #81`結案段落已明文記錄「#76/#78
不是月頻訊號的問題，需要另外設計M日不重疊區塊抽樣＋circular shift的
變體，下一輪不可直接套用同一函式」，本輪四筆試驗延續舊設計等於延續
了同一個已知風險，屬於補交代而非忽略：

- **這個方法論缺陷的方向是讓PASS變得虛胖**（daily-overlap膨脹表面n、
  低估有效自由度使p值/null百分位過度樂觀；逐點打散破壞訊號自身序列
  相關性、低估虛無分布離散度使「贏過null」變得過度容易）——兩者皆
  偏向製造假陽性，不會製造假陰性。
- **本輪四筆試驗的判定依據**：`常備.2`/`常備.3`/`常備.5`(三格)皆死於
  「train/val正負號相反」，這個判準只看相關係數符號、不涉及p值或
  null百分位，不受這個方法論缺陷影響；`常備.4`死於「VAL未贏過洗牌
  null」（percentile=73.6<90），在一個**偏向樂觀**的null檢定下都沒能
  贏過，換成正確的M日不重疊+circular shift設計（虛無分布只會更寬、
  更難贏），結論只會更確定是FAIL，不會翻案成PASS。
- **結論**：本輪四筆FAIL判定在方法論修正後**不會改變**（同方向或更
  確定FAIL），可以信任；但這不構成「方法論缺陷不重要」的理由——若
  未來`vix_term_structure_gate.py`/`hy_etf_ratio_gate.py`家族出現任何
  看似CHEAP_PASS的結果，在拿去進下一關前必須先補這個修正，理由同
  `## #81`已示範過的cbi_signal_gate.py案例（原CHEAP_PASS被修正後撤銷）。
  `## #81`段落列出的「尚未套用到#76~#80既有gate檔案」待辦清單，本次
  新增的四個檔案應併入同一份待辦，供未來輪次一次性補齊，不是逐一
  發現才逐一記錄。

**2026-09-23補齊（`PENDING_QUEUE.md`常備.12，DevQueue自走）：上述待辦
已實際重跑，數學論證獲實測驗證**——`regime_gate_common.py`新增
`sample_nonoverlapping_blocks()`（日頻訊號版不重疊觀測抽樣，跟
`align_monthly_nonoverlap()`不同：後者假設輸入是離散發布日事件，前者
對逐日連續訊號每隔M筆取一筆），新增`regime_gate_nonoverlap_reverify.py`
一次重跑六個gate（含`常備.5`三個窗口共8組配置）：

| 組別 | M | 逐日重疊n→不重疊n | VAL r | null percentile(門檻90.0) | 判定 |
|---|---:|---|---:|---:|---|
| `#76`(vix_term_structure) | 20 | 3283→165 | +0.1432 | 49.2 | FAIL |
| `#78`(hy_etf_ratio) | 20 | 4183→210 | +0.1916 | 58.4 | FAIL |
| 常備.2(vix_term_roc) | 20 | 3264→164 | +0.0841 | 35.6 | FAIL |
| 常備.3(vix_level) | 20 | 6507→326 | -0.2150 | 74.6 | FAIL |
| 常備.4(hy_ratio_roc) | 20 | 4164→209 | -0.0923 | 37.6 | FAIL |
| 常備.5(M=5) | 5 | 4198→840 | +0.0802 | 42.4 | FAIL |
| 常備.5(M=10) | 10 | 4193→420 | +0.1094 | 40.4 | FAIL |
| 常備.5(M=60) | 60 | 4144→70(VAL僅15) | — | — | FAIL(樣本不足) |

**8/8組在更保守設計下維持FAIL**，印證了「舊設計偏樂觀、換更保守設計
不可能讓FAIL翻案成PASS」的事前論證——不重疊抽樣後n驟降到原本的
5%~10%（例如`#76`從3283降到165），VAL期樣本數普遍只剩40~50筆（M=60
時只剩15筆，連基本樣本數門檻都不夠），這個量化落差本身就是「daily-
overlap虛胖表面n」有多嚴重的直接證據。登記`TRIALS_LEDGER.md`#374
（批次一筆，含8組明細）。至此VIX/HY信用利差代理regime訊號家族
（`#31~34`/`#75~78`/`常備.2~.5`/`#80~81`/驗.三系列）在**兩套獨立
方法論**（原始daily-overlap版與修正版不重疊區塊抽樣版）下都得到一致
結論：台股大盤regime降曝險訊號目前沒有找到穩健候選，`## #81`「尚未
套用」待辦正式結清。
