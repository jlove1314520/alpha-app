...
5. **方法論提醒（2026-09-24總司令裁示【value_board翻案處理＋轉向.一
   候選名單定案＋FinLab借鏡登記】登記.一新增）**：在同一段資料上逐步
   挑最好的版本往下改，等於多次試驗；最後選出的版本是樣本內最大值，
   必須以試驗總數做多重比較校正，並在未使用過的資料上驗證。具體來說：
   一個機制若經過「v1死→改良成v2→v2死→改良成v3」這種鏈式迭代，v3
   看起來通過的顯著性，若只拿v3自己的p值/DSR去對照原始門檻，會系統性
   低估真正的多重比較懲罰——因為挑選「往哪個方向改良」這個動作本身
   也是在同一批資料上搜尋，`selection_bias_ledger.py`的N必須把v1/v2/v3
   都算進去（現行做法已是如此，這裡是明文化這個既有原則，供未來設計
   新機制時對照，不是追溯改判任何既有紀錄）。同一段資料上的鏈式迭代
   最終若要宣稱「找到真訊號」，仍應該在一段從未被這整條鏈碰過的資料
   上做事前登記、只跑一次的驗證（同`轉向.一`「新資料單發檢定」的精神），
   不能只靠「連續迭代到通過為止」這件事本身當作證據。
...
**最終更正（2026-09-23總司令裁示【修正alpha量尺＋f52w補完審查＋稽核
續跑】審.一，完整6步驟審查後的最終結論，結案`TRIALS_LEDGER.md`#379）**：
驗.一引擎修正後（compounding+零價格防呆），#85/#86兩個原本各自獨立的
FAIL理由都表面翻轉為PASS（#370/#378）——VAL alpha從p=0.0831升到
p=0.0017、逐年一致性從4/6年正報酬升到6/6年，一度看起來像是死因純屬
引擎bug製造出來的假FAIL。但完整補審後發現**這個表面翻轉本身也是假象，
只是換了一層更難察覺的形式**：
1. **量尺修正後alpha依然顯著**（尺.一：改0050含息總報酬基準+Newey-
   West HAC標準誤+Dimson beta，VAL alpha p=0.0046，比舊量尺還更顯著）
   ——排除了「顯著性是benchmark量尺錯誤製造出來的」這個可能性。
2. **VAL報酬76.9%集中在電子/半導體供應鏈**（光電業/電子零組件業/
   電子工業/半導體業合計），單一產業代碼上限N/4=5的「產業中性」對照
   組因為natural selection本來就沒有單一產業代碼超過4檔而完全沒有
   改變任何結果（cap未binding，見稽核紀錄誠實揭露此侷限——這代表
   集中度問題出在「電子供應鏈」這個更粗的超類別，不是任何單一細產業
   代碼，用單一產業代碼設上限的對照組設計本身無法偵測到這種跨代碼但
   同超類別的集中度）。
3. **決定性結論：DSR(Deflated Sharpe Ratio)=0.0000，遠低於0.95門檻，
   完全未通過**（#379）。在本專案N=379次已登記試驗的搜尋規模下，
   觀測Sharpe(日0.1091)遠低於純粹運氣就能產生的期望最大Sharpe門檻
   (SR0=0.6727)——f52w的表現完全落在「跟隨機結果無法區分」的範圍內。
   Bonferroni校正同樣未過（p=0.004586 vs 校正門檻0.000132）。
**最終判定FAIL**，依裁示「事前宣告：DSR未過就不得核准，不因p值好看
放寬」，不因為兩個原始死因表面翻轉、也不因為新量尺下依然顯著，就
放寬多重比較這關的門檻。**這是本專案目前為止最清楚的一次示範**：
表面判準（單一p值、單一年份一致性）可以在换引擎/换量尺後接連翻轉，
但只要把試驗總數(N)放進DSR/Bonferroni這種正式的多重比較框架，
「這是不是真訊號」的答案可能完全不變。樣本缺口（2015起不含2008，
天條一無法判定；延伸到2007的資料成本待評估，未執行）另案記錄。

**正式結案（2026-09-23總司令裁示【f52w結案＋研究策略轉向「新資料單發
檢定」＋佇列清理】結案.三）**：上面#379的DSR=0.0000後來查出是單位
錯誤（年化Sharpe混進日頻V估計，尺.二根因，#379已標記INVALID_BUG）；
用修正後方法論（資訊比率+可比試驗V）重做的審.二（#385）得到DSR=
0.1739，同樣遠低於0.95門檻。**總司令另請Cowork複核**：V排除f52w
自身重算，DSR 0.74（N=386）／0.89（N=40），理論雜訊下限（4年VAL，
IR抽樣標準差約0.5）DSR 0.48——**全部<0.95，FAIL在所有合理設定下
都成立**，不是單一V假設下的巧合。**正式結案：FAIL**。

**唯一翻案途徑**：VAL只有4年，觀測IR（年化1.46）落在386次試驗的
雜訊最大值範圍內——樣本太短、試驗數太多，這個組合本身統計檢定力
就不足以區分「真訊號」與「運氣」，不是換更多方法論或更多對照組
能解決的。**下一步不是繼續在同一批資料上換角度重測**（那只會繼續
墊高N、讓門檻更難過），而是「全新資料單發檢定」（見轉向.一）：
IR的抽樣雜訊隨觀測年數的平方根下降，在一段從未被本策略/本因子
碰過的資料上，做一次事前登記、只跑一次、不得換參數重跑的檢定，
不受既有N的多重比較懲罰——這是本專案「發現搜太多次會把自己的
判準搜垮」之後，第一次正式把研究方法本身從「繼續加試驗」轉向
「找乾淨資料」。
...
**開盤到收盤重建版最終結案（2026-09-24總司令裁示【value_board翻案處理
＋轉向.一候選名單定案＋FinLab借鏡登記】驗.二續，`TRIALS_LEDGER.md`
#394）**：`spillover_overlay_v1_o2c.py`接續#386（cheap gate地基，
CHEAP_PASS但診斷84.9%相關性活在跳空裡）走完整第2-9關，**第3關參數
密集高原未過（49點網格0點報酬為正，門檻60%），快殺判定FAIL，未進
第4-9關**。**死因跟close-to-close版(#346)完全不同**：#346死於前視
偏誤（方法論bug），#394死於一個誠實揭露的市場微結構事實——台股
2015-2020這段多頭期，開盤到收盤(intraday)買進持有基準本身累積報酬
為**-77.48%**（CAGR約-13%/年），代表這段期間TAIEX的正報酬幾乎全部
發生在收盤到隔日開盤的跳空/盤前時段，任何只操作盤中曝險的機制都在
對抗一個結構性下滑的基準，不論訊號本身有沒有用都很難打正。第2關
隨機控制組本身仍PASS（訊號的時序配對確實贏過隨機配對），代表#19的
「美股隔夜報酬領先台股」這個經濟機制本身沒有被推翻，死的是「用這個
訊號去操作台股盤中曝險」這個具體實現方式——跳空段（訊號經濟意義真正
發生作用的地方）結構上無法交易，盤中段（唯一能交易的地方）基準本身
結構性下滑，兩者疊加讓這條路徑在台股上走不通。**至此#19（跨市場美股
隔夜報酬外溢效應）在台股的兩種可能實現（close-to-close/open-to-close）
皆已窮盡並判死，結案，不建議再嘗試同一機制的其他變體**（除非未來有
辦法交易到隔夜跳空本身，例如透過美股ADR/期貨等衍生品，但那是完全
不同的執行架構，不是本佇列範圍內的參數調整可以解決的）。
...
## #17 52週高點接近度單因子組合（f52w_high_portfolio_v1）（2026-09-25結案，考.一2007-2014單發檢定）

**死因**：轉向.一「新資料單發檢定」（`SINGLE_SHOT_2007_2014_SPEC.md`，
候選宇宙限`common_stock_only()`普通股）——**4.1資訊比率關卡未過**：
2007-2014全段IR(年化)=0.5784，beta_dimson=+0.568，單尾p=0.0611，
未達事前綁定的Bonferroni校正門檻p<0.025（比門檻寬約2.4倍，不是邊緣
案例）。4.2兩條MDD硬性條件皆PASS（2008年MDD-47.14%嚴格優於0050同期
-55.75%；全段MDD-47.14%>-50%天條一門檻），但依SPEC 4.3「任一項不過
即FAIL」，最終判定FAIL。見`TRIALS_LEDGER.md`#398（父筆#396事前登記，
上溯#85/#86/#370/#377/#378/#379/#385等既有VAL期審查歷程）。

**背景**：這個因子（52週高點接近度）此前在既有2015-2024樣本的VAL期
一度顯著（#377，p=0.0046），但套用DSR/Bonferroni多重比較校正後判定
運氣無法排除（#379/#385，DSR遠低於0.95門檻），因為N=386次試驗＋VAL
只有4年的組合統計檢定力不足以區分真訊號與運氣。本次是在一段從未被
本專案任何試驗碰過的2007-2014資料上做事前登記、只跑一次的單發檢定，
繞開了N次試驗的多重比較懲罰，結果依然是IR不顯著——**這次的FAIL不是
被多重比較校正拖累，是在乾淨資料、無懲罰的條件下直接測不出顯著IR**，
證據力比先前任何一次判定都更明確。

**不泛化成**：52週高點接近度因子在任何市場/任何組合方式都無用——
只測了「TOP20、月頻再平衡、產業中性z-score」這一種具體組合形式，
其他組合方式（不同TOP_N、不同再平衡頻率、跟其他因子混合）理論上仍
可獨立測試，但依裁示【f52w結案＋研究策略轉向】凍結.二，目前暫停
新增任何以此因子為基礎的試驗，需總司令另行裁示才能重啟。

## #4 股利率carry單因子組合（dividend_yield_portfolio_v1）（2026-09-25結案，考.一2007-2014單發檢定）

**死因**：轉向.一「新資料單發檢定」（同上）——**4.2全段MDD硬性條件
未過（違反天條一）**：2007-2014全段MDD=-53.72%，突破-50%門檻。
IR(年化)=0.8319，beta_dimson=+0.705，單尾p=0.0128，**通過**Bonferroni
門檻p<0.025；2008年MDD=-53.72%，嚴格優於0050同期-55.75%，也**通過**。
但全段MDD與2008年MDD數字完全相同——代表全期最深回撤就是2008金融
海嘯本身這一段造成的，不是後段另有其他危機，2008崩盤的深度單獨一項
就已經突破天條一（MDD不得超過50%）的硬性門檻。依SPEC 4.3「任一項
不過即FAIL」，最終判定FAIL。見`TRIALS_LEDGER.md`#399（父筆#397，
上溯#74等既有VAL期審查歷程）。

**關鍵發現**：這是本次單發檢定中**唯一一個IR統計顯著（p<0.025）卻被
MDD硬約束否決的案例**，具體示範了天條一「MDD不得超過50%，這是硬
約束不是評分項」的實際作用——若只看IR/報酬（+203.87% vs 0050
+50.14%），這個候選看起來明顯優於基準；但天條一的存在正是為了否決
「上檔漂亮、下檔沒守」的候選，不因為統計顯著就放行。

**不泛化成**：股利率carry因子本身無效——它的問題不是訊號方向錯，是
「集中度/曝險力道」讓2008年這種系統性崩盤時的下檔保護不夠，理論上
一個加了明確降曝險/縮部位機制的版本可能同時保住IR顯著性又滿足MDD
門檻，但依裁示【f52w結案＋研究策略轉向】凍結.二，目前暫停新增任何
以此因子為基礎的試驗，需總司令另行裁示才能重啟。

**補註（2026-09-26，結.一，總司令裁示【考.一結案確認】，登記於
`PENDING_QUEUE.md`）**：#398 f52w、#399 dividend依事前規則判FAIL**維持
不變**，不追溯改判。補充脈絡：dividend在2007-2014的IR(年化)=0.83、
單尾p=0.013（Bonferroni門檻p<0.025，通過），唯一未過的是全期MDD=
-53.72%超過-50%；同期0050本身MDD=-55.75%（買0050持有同樣違反天條一）。
天條一原意為**帳戶層級**約束，考.一的SPEC 4.2把它誤套在**策略（股票
部位）層級**，這是Cowork的規格缺陷，不是候選本身的過擬合或流程錯誤；
依「事前綁定、不得事後改判」原則，本筆FAIL維持，改以新候選處理——見
`PENDING_QUEUE.md`「新.一」（dividend_yield_v1_common×帳戶70/30，
登記為新試驗、計入N，凍結.二僅為該候選解凍）。上面「不泛化成」段落
所說「需總司令另行裁示才能重啟」，已由該裁示涵蓋這一個候選。
（數字已對照`TRIALS_LEDGER.md`#399逐項核對：IR 0.8319、p=0.0128、
MDD -53.72%、0050同期 -55.75%，皆與裁示原文一致。）
