...
**更正（2026-09-15 下一輪＝假設佇列第八輪查證發現，上一段陳述有事實
錯誤）**：上面「`theme_keywords.json`目前唯一的消費者就是這個測試腳本
本身」**不成立**。實測`scripts/build_themes.py`第48行
`KWMAP = D / "seed" / "theme_keywords.json"`會直接讀取這份檔案並在
`build_keyword_map()`（第101~113行）餵給正式的新聞題材A/C級證據判定
管線（`main()`第203行呼叫），這才是真正在跑的正式管線，不是題材七
待辦1那條還沒建的官網抓取管線——上一輪把兩件事搞混了。好消息是查證後
發現：`build_themes.py`實際做A/C級判定的`evidence_keyword()`不是裸
substring比對，額外要求「完整句型片段literal出現」（例如「打入EG供應鏈」
整段要出現，不是只有「EG」出現），這道關卡大幅限縮了短縮寫詞的意外命中
風險。完整的123題材全面掃描（含真實語料實測驗證）見
`research/theme_keyword_ambiguity_scan.py`，結論：496句真實內文引言+300則
標題的語料裡找到2筆「裸substring命中但詞界比對不命中」的案例，
但0筆通過`build_themes.py`完整判定關卡——驗證了這道句型關卡目前確實有效。
"""
