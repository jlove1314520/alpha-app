...
已歸檔至`research/archive/`。動工前查證（`grep`全`research/*.py`）確認
**沒有任何其他檔案import本檔案的`path1_market_cap`/`path2_market_cap`**
——本檔案只服務`CORE_TILT_SPEC.md`（已作廢）的0050成份股權重重建用途，
不是方法.二/方法.三控制組市值分位配對的實際資料來源（那個用的是
`core_tilt_backtest.py::build_market_cap_lookup()`，PBR×權益法，
跟本檔案完全無程式碼相依，仍在使用中不受影響）。裁示原文原本以為本
檔案被方法.二/方法.三依賴、要求「降級精度但保留」，查證後與程式碼
實際情況不符，已回報供更正認知，據此改為整支歸檔。原樣保留下方內容
供稽核追溯。
