...
36. 個股融券使用率（Short Sale Utilization Ratio）——**2026-09-05
    ...
    hypothesis_queue排程接續最新狀態與`TRIALS_LEDGER.md`新增列。**尚未
    結案**——依alpha顯著性+beta拆解同一把尺，TRAIN期不顯著這件事要留到
    後續關卡一併檢視，不能只看VAL單期顯著就樂觀。`is_holdout_
    consumed()`執行前後皆確認False，**下一輪從第3關參數密集高原（TOP_N
    掃描）開始，不跳關**，剩餘#5/#6/#8/#10仍卡外部依賴（同上）。
