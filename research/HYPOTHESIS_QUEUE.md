...
- **主統計量（維度 a，二階）**：逐檔當沖比重 `dt_{i,t}` ＝ 該股當沖成交值 ÷
  該股成交值，取**截面標準差**。
- **資料缺口（這就是暫不開跑的原因）**：`TWTASU_*.parquet` 實測是
  **每日 1 列 5 欄的全市場加總**（`day_trade_sell_volume`／`day_trade_sell_value`
  ／`margin_offset_volume`／`margin_offset_value`），**沒有逐檔**。逐檔當沖需要
  另一個 TWSE 端點（「每日當日沖銷交易標的及成交量值」），**本輪尚未查證該端點的
  實際路徑、歷史起點與是否含金額欄**——所以這裡不下「有」或「沒有」的結論。
- **開跑前必做（照 `CLAUDE.md` 三來源查證紀律）**：官方入口／官方 API 文件／
  GitHub 或社群實作／其他供應商，四類至少查三類，把連結與查證日期寫進
  `docs/DATA_SOURCE_MAP.md` 再下結論；若確認可得，照 `twse_day_trading_client.py`
  同一套節流與快取設計新增 client 並回填，**回填完成前不得開跑**。
  若三來源查證後確認官方不提供逐檔當沖歷史，本假設比照 #56 撤案。
- **狀態**：規格已寫死，**前置資料未備、尚未開跑**。
...
4. **#57**（先完成逐檔當沖來源的三來源查證與回填）——現在排隊第一，尚未開跑，前置依賴（逐檔當沖端點三來源查證）未完成。
5. ~~#56~~（撤案）
