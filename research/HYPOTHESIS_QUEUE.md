...
28. ~~市場廣度背離（Breadth Divergence）當regime擇時訊號~~——
    **2026-09-04排程接續已結案：FAIL**（第1關sanity PASS後轉具體
    ...
    切換這個具體規則。**佇列#1~28全數結案。剩餘#5/#6/#8/#10仍卡外部
    依賴（本輪2026-09-04重新查證：B24-500仍不及格、題材動能榜PIT
    引擎仍是紙上交易中/無回測，均無新進展）。下一輪需重新確認依賴是否
    解鎖，未解鎖則設計新假設軸#29——應避開「純選股排序」（10條已FAIL）
    跟「timing/overlay類」（#2/#15/#19/#26/#28全數FAIL，#10方法論框架
    已建置但尚無候選可套用）這兩種已知死法/卡點，優先考慮機制上真正
    不同的第三類角度，現在排隊第一，尚未開始第1關。**
