...
43. 三大法人買賣超集中度（Institutional Buying Concentration）當市場
    領漲廣度regime訊號——**2026-09-06 hypothesis_queue排程本輪新增，
    尚未開始第1關**。回顧#1~42全數結案後，設計出跟已測過的四類regime/
    籌碼假設經濟機制皆不同的第44條假設維度：不看單一序列水位（已死11
    條）、不看價格相關結構（#42，已死）、不看個股自身持續性（#13，
    已死）、不看散戶槓桿（#30，已死），改看**三大法人每日買賣超金額
    在整個股票宇宙裡的分布集中度**（資金流廣度，而非價格廣度或報酬
    相關性），零新資料源（複用#13/#41已回補的三大法人日頻買賣超快取），
    現在排隊第一，下一輪從第1關cheap gate開始（沿用
...
這輪只登記假設、未寫任何程式碼、未跑任何測試，`is_holdout_consumed()`
本輪未做任何資料抓取。下一輪從第1關cheap gate開始：先寫
`institutional_concentration_gate.py`，用既有三大法人日頻買賣超快取
計算HHI與前10大佔比兩個版本的集中度指標，對齊TAIEX未來20日報酬，
train/val分期+時序洗牌null，不跳關。其餘#5/#6/#8/#10仍登記在案，任一
外部依賴解鎖時應優先評估是否能接續。
