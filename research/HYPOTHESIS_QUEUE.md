...
36. ~~個股融券使用率（Short Sale Utilization Ratio）~~——**2026-09-05
    ...
    應先擴充`backtest/engine.py`支援放空。**佇列#1~36全數結案，剩餘
    #5/#6/#8/#10仍卡外部依賴（本輪重新查證仍未解鎖），設計新假設軸
    #37（全市場現股當沖比重當市場過熱regime訊號，見下方新章節），
    現在排隊第一，尚未開始第1關。**
37. 全市場現股當沖比重（Day-Trading Ratio）——**2026-09-06T01:43接續排程
    第九輪：地基建置中**（FinMind免費層資料只從2024起、涵蓋不到TRAIN期，
    改建TWSE官方TWTASU+FMTQIK資料源，FMTQIK 120個月已100%回補、
    TWTASU逐日回補進度累積2488/2609（95.4%，剩餘121天），checkpoint
    可續跑，完整內容見上方#37條目最新狀態），現在排隊第一，下一輪重跑
    `backfill_day_trading_ratio.py --skip-market-volume --batch-size 250`
    即可自動接續，預估下一輪即可回補完整TRAIN+VAL期並進第1關cheap gate。
