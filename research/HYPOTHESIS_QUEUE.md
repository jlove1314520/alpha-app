...
29. 等權重再平衡溢酬 Diversification Return / Equal-Weight Rebalancing
    Premium——**2026-09-04排程新增，第1關sanity：PASS（非最終判定）**。
    設計出跟前28條在機制分類上真正不同的第三類假設（既非①方向性選股
    排序、也非②timing/exposure overlay，是③portfolio construction——
    給定同一組標的、不做任何選股判斷、全程滿倉不做曝險縮放，純粹用
    「定期拉回等權重」這個機械式再平衡動作本身測試diversification
    return）。**基準操作化偏離原文的市值加權，改為t0等權重buy-and-hold**
    （本專案無市值資料源，理由見上方#29條目「狀態」小節）。TRAIN/VAL
    兩期rebalanced皆優於buyhold（溢酬+24.81pp/+31.72pp）、Sharpe/MDD
    同步改善、非退化、再平衡確認有實際觸發，完整數字見上方#29條目與
    `TRIALS_LEDGER.md`#107。**2026-09-04接續排程，第2關專屬隨機控制組
    （bootstrap抽80/159檔子集N=100 draws）：CHEAP_PASS**（100/100 draws
    TRAIN且VAL同時為正、全池結果落在分布56~57百分位非離群、中位數溢酬
    明顯>0，本佇列迄今第2關通過方式最乾淨案例，見`TRIALS_LEDGER.md`
    #108）。**佇列#29仍未結案**，下一輪接續第3關（參數密集高原）→第4關
    （成本敏感度，換手成本需誠實計入）→…→第9關（下檔保護，等權重小型股
    曝險在危機期表現尚未驗證，是這條假設最大未知風險）。
