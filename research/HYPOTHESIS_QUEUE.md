...
42. 個股間平均成對相關係數（Average Pairwise Correlation）當系統性
    風險regime訊號——**2026-09-06 hypothesis_queue排程本輪新增，尚未
    開始第1關**。回顧已死的11條timing/regime類假設共同模式（#10/#15/
    #26/#28/#30/#31/#32/#33/#34/#35/#37皆是「單一外部序列水位/成長率」
    當訊號），設計出經濟機制真正不同的第三類regime假設——不看任何
    單一序列的水位，改看整個股票宇宙內部的橫斷面共同運動結構（個股
    報酬彼此的平均成對相關係數），**零新資料源需求**（複用既有300檔
    快取宇宙的逐日報酬，跟`f_bab`/`f_idio_vol`已示範過的rolling
    covariance程式碼路徑同源，只是換成個股vs個股而非個股vs大盤），
    現在排隊第一，下一輪從第1關cheap gate開始（先寫
    `avg_pairwise_correlation_gate.py`，沿用`day_trading_ratio_gate.py`
    同一套train/val+洗牌null框架），不跳關。剩餘#5/#6/#8/#10仍卡外部
    依賴（本輪重新查證仍未解鎖，無新進展）。完整內容見下方新章節。
...
本輪依`HYPOTHESIS_QUEUE_PROTOCOL.md`第1節「佇列已空」流程，回顧
`STRATEGY_GRAVEYARD.md`已死的11條timing/regime類假設共同模式（清一色
用「單一外部序列水位/成長率」當訊號），設計出這條經濟機制真正不同的
第三類regime假設（橫斷面共同運動結構，非單一序列水位），本輪工作到
此為止，下一輪從第1關cheap gate開始（先寫
`avg_pairwise_correlation_gate.py`，沿用`day_trading_ratio_gate.py`
同一套train/val+洗牌null框架），不跳關。剩餘#5/#6/#8/#10仍卡外部
依賴，本輪重新查證仍未解鎖（`BACKLOG.md`第1311行`題材動能榜／未來性
濾網：紙上交易中`、第1164行`value_board_v2`狀態仍`回測未通過`，跟
上輪查證結果一致，無新進展）。
