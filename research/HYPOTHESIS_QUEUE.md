...
36. 個股融券使用率（Short Sale Utilization Ratio）——**2026-09-05
    ...
    `TRIALS_LEDGER.md`新增列。**尚未結案**——依alpha顯著性+beta拆解同一
    把尺，TRAIN期alpha不顯著這件事仍要留到後續關卡一併檢視，不能只看VAL
    單期顯著或第3關高原漂亮就樂觀。`is_holdout_consumed()`執行前後皆確認
    False，**下一輪從第5關leave-one-out開始，不跳關**，剩餘#5/#6/#8/#10
    仍卡外部依賴（同上）。
