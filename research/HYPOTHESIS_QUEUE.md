...
- **進行中**：回填已用`run_detached.py`投遞（job`20260908-061051-6640`，
  ...
  （每批約8~9分鐘處理250天）。
- **狀態**：**資料源已確認可得，回填進行中，尚未開跑GATE_SEQUENCE第1關**。
  下一輪TW軌接手：先用`run_detached.py status`/`log`收成本輪投遞的job，
  未跑完就再投遞下一批（`--batch-size 250`會自動跳過已快取日期），跑完
  2,609天後才進入`#57`第1關sanity（比照`#53`/`#54`/`#55`同一套
  `cross_sectional_*_gate5X.py`框架）。
...
4. **#57**（逐檔當沖資料源已查證可得，回填進行中）——現在排隊第一，資料回填完成後才進第1關sanity。
5. ~~#56~~（撤案）
