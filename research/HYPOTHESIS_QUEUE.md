...
**狀態（2026-09-03第二十六輪排程，已結案：FAIL）**：接手時發現前一輪
留下**4個重複的`piotroski_fscore_gate_v1.py`背景行程**同時在跑（PID
58420/42672/48592/67728，headless呼叫結束時背景行程未被清理，累積成
殭屍行程，跟`#4`Carry條目記錄的「假設背景行程存活到下一輪」問題同一種
根因但這次是反過來——沒被清理而是持續累積），本輪確認時4者皆已自然
執行完畢並結束，`data/piotroski_fscore_gate_v1_results.csv`已產出（多個
重複行程各自寫入同一輸出檔，最後完成者的結果覆蓋前面的，內容仍完整
可用，未觀察到檔案損毀）。**核心比較結果**：自適應選定F>=6（最寬鬆
門檻）。原始baseline：TRAIN alpha=+6.26%(p=0.2672)/mine_rate=25.3%/
return=+75.87%；VAL alpha=+12.38%(p=0.1441)/mine_rate=16.9%/return=
+85.52%。+F-score gate(F>=6)：TRAIN alpha=+7.92%(p=0.4743)/mine_rate=
29.1%/return=+71.79%；VAL alpha=+5.24%(p=0.1772)/mine_rate=5.7%/
return=+33.31%(交易數718→98)。依本條「驗證改造」小節事前訂的判準：
兩期alpha的p值皆變得**更不顯著**（惡化非改善），地雷率沒有一致改善
（TRAIN惡化25.3%→29.1%、VAL改善16.9%→5.7%，一升一降），且VAL期return
大幅流失機會成本，**判定FAIL**，依快殺標準（使用者事前訂的便宜且
決定性判準）直接結案，未投入100次隨機控制組成本。完整見
`STRATEGY_GRAVEYARD.md`、`TRIALS_LEDGER.md`#94。**佇列#23結案，接續
佇列第一順位#24（除權息季節行為效應）**。
