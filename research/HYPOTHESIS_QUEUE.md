...
41. 內部人（董監事/大股東/經理人）持股轉讓——**2026-09-06 hypothesis_
    queue排程新增，地基查證卡在工具限制（非資料確定不可及）**：TWSE
    openapi`t187ap11/12/13`三個候選端點皆只有最新單一快照、無歷史
    查詢參數（跟#38同一種死法）；MOPS互動查詢頁本輪嘗試三個候選功能
    代碼（`t05st07`/`t34sc01`/`t108sb01`）皆連線被拒，**未能確認正確
    代碼**（此headless環境無網路搜尋工具，土法猜代碼未猜中，不等於
    MOPS真的沒有這個查詢頁）。**不比照#38/#39直接判FAIL**——那兩條是
    三條路徑全部查證窮盡才判死，這條只窮盡了openapi一條路，MOPS互動
    頁路徑仍待有網路搜尋能力的下一輪/session查證正確代碼。現在排隊
    第一，完整經濟理由/已知背景/查證細節見下方新章節。
...
**狀態（2026-09-06 hypothesis_queue排程接續，地基查證卡住，非資料
確定不可及的最終判定，跟#38/#39「三條路窮盡」等級不同）**：**不比照
#38/#39直接判FAIL**——那兩條是三條候選路徑都逐一查證窮盡後才判「資料
不可及」；這條只確認了openapi快照路徑不可行，MOPS互動查詢頁**可能
存在但本輪沒找到正確代碼**（工具限制：這個環境缺網路搜尋能力去查證
MOPS完整功能代碼清單，不是代碼真的不存在的證據）。**下一輪（或有
網路搜尋工具的互動session）優先事項**：查證MOPS官方「內部人持股轉讓
事前申報表」查詢頁的正確功能代碼（可能透過MOPS首頁選單巡覽、或查
第三方財經資料網站是否有引用這個查詢頁的完整URL）；若最終確認MOPS
真的沒有帶日期範圍的互動查詢版本，才依快殺標準「資料不可及」正式
判FAIL；若找到可行路徑，接續第1關cheap gate（比照#40事件研究框架或
橫斷面IC框架，視資料形態決定）。**現在排隊第一，尚未過關也尚未判死，
是本佇列少見的「卡在工具限制而非機制/資料本身」的中繼狀態，需要
如實記錄避免被誤判成已窮盡查證。**
