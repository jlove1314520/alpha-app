...
逐筆tick落地（非本track範圍），下一輪從設計#67開始。**
...
**狀態（2026-09-09 hypothesis_queue排程接續新增，尚未開始第1關）**：
本輪僅完成假設設計、經濟機制比對（跟已測15類機制逐一比對確認無重複，
本假設為第⑯類）、初步文獻/制度背景查證（盤中零股制度2020年10月上路），
**未核對資料可行性**（下一輪待辦第一項）、未寫任何程式碼、未跑任何
測試。下一輪從查證TWSE/TPEx是否有免費開放的逐檔逐日零股成交統計端點
開始（比照`CLAUDE.md`「搜尋紀律：三來源查證」，若三來源都查不到才可
判定資料不可及），若可行則接著寫`odd_lot_ratio_gate67.py`第1關cheap
gate腳本（比照既有`factor_ic.py`框架），跑完才判CHEAP_PASS/FAIL，不
跳關。`is_holdout_consumed()`開工/收工前皆確認`False`，全程零新增API
呼叫（純設計文件+讀既有紀錄，資料可行性查證留給下一輪執行）。**本輪
工作到此為止（一輪一個有界工作單位）**。
