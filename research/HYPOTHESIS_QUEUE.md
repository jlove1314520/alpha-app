...
40. 庫藏股買回公告效應（Share Buyback Announcement Effect）——
    **2026-09-06 hypothesis_queue排程接續（上一輪鎖檔陳舊回收）新增，
    資料可行性已查證確認可行，尚未開始第1關**。重新查證#5/#6/#8/#10
...
**狀態（2026-09-06本輪排程，新假設軸設計完成+資料可行性已查證，
尚未開始第1關）**：這輪工作單位到此為止（依協定「一輪只做一個有界
工作單位」，本輪同時處理了鎖檔陳舊回收的異常情況跟新假設軸設計，
預算考量下未繼續往下推進），未寫任何因子/測試程式碼。下一輪從
第1關cheap gate開始：先寫`buyback_announcement_probe.py`確認歷史
回溯範圍後，寫正式回補腳本（`BeautifulSoup`解析，逐年查詢累積存檔）
+事件研究CAR檢定腳本（train/val同號+贏過隨機日期控制組percentile
>=90.0門檻三項判準），不跳關。
