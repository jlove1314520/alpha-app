...
33. ~~美國公債殖利率曲線（10Y-2Y利差）~~——**2026-09-05接續排程第1關
    ...
    **佇列#1~33全數結案，剩餘#5/#6/#8/#10仍卡外部依賴（本輪重新查證
    ...
    跟內容區塊標題編號不同步情況。）
34. ~~銅金比（Copper/Gold Ratio）~~——**2026-09-05 hypothesis_queue排程
    第1關已完成：CHEAP_PASS**（資料可行性先查證yfinance `HG=F`/`GC=F`
    2015-01-02~2026-06-29完整涵蓋、無NaN、無>14天缺口，確認可行後新增
    `copper_gold_ratio_gate.py`，沿用`fx_twd_gate.py`/
    `fred_yield_curve_gate.py`同一套指數層級時序相關性框架，訊號用比值
    水位本身而非變動率）。結果：TRAIN(n=1413) Pearson r=-0.2467
    (p<0.0001) null percentile=100.0、VAL(n=917) Pearson r=-0.1758
    (p<0.0001) null percentile=100.0——三項cheap gate判準（幅度非零/
    train-val同號/VAL贏過洗牌null）全過，判**CHEAP_PASS**。**誠實記錄
    方向不符事前預期**：事前綁定預期銅金比走高（風險偏好升溫）對應TAIEX
    後續報酬轉強（正相關），實測兩期皆為**負相關**且量級不小（|r|
    0.18~0.25，遠高於本佇列其他cheap gate的|r|量級如#32/#33僅0.02~0.07）
    ——依`fx_twd_gate.py`/`fred_yield_curve_gate.py`docstring既定原則，
    cheap gate本身只看train/val同號+贏過null，不會因為方向不符預期就
    自動判FAIL，但這個方向反轉必須帶著明確警示進入下一關，不能默默照
    原敘事包裝成「證實了Dr. Copper假說」——真實情況更像是這個相關性存在
    但機制可能跟「全球製造業景氣外溢到台股」的原始敘事不同（例如可能是
    美元強弱或Fed政策預期同時驅動銅金比與台股的共同因子，屬於待後續
    deep_dive才能釐清的替代解釋，此輪不下定論）。**尚未結案**——下一步
    是轉具體overlay規則（比照#31/#32/#33同一套流程：把cheap gate的時序
    相關性轉成固定門檻+二元曝險切換的具體regime規則，走第2關隨機控制組
    N=100），**規則方向必須依這輪實測的負相關訂定（銅金比走高降曝險/
    走低維持曝險），不能沿用原始正相關敘事的方向**，否則等於用已被推翻
    的方向假設在跑一個注定錯的策略。完整見`TRIALS_LEDGER.md`（本輪新增
    條目）、`MARATHON_LOG.md`本輪心跳。
