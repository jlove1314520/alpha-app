...
27. ~~多因子z-score複合評分（GP+value_pb+revenue_surprise等權）~~——
    **2026-09-04排程接續已結案：FAIL**（第1關cheap IC gate
    ...
    具體因子等權組合。**佇列#1~27全數結案。剩餘#5/#6/#8/#10仍卡外部
    依賴（本輪2026-09-04重新查證：B24-500仍不及格、題材動能榜PIT
    引擎仍是紙上交易中/無回測，均無新進展），依協定第1節設計新假設軸
    #28（市場廣度背離Breadth Divergence當regime擇時訊號，見下方新
    章節），現在排隊第一，尚未開始第1關，下一輪從sanity（含既有300檔
    快取樣本能否直接算出可用的%高於200日均線時間序列可行性查證）開始，
    不跳關。**
