...
**狀態**：尚未開始第1關。下一輪從最便宜的檢定開始：用`TaiwanOptionDaily`
既有欄位（若含implied volatility或可從價格反推）計算IV，與TAIEX後續
已實現波動度比較，確認IV系統性高於RV（台股版VRP存在性）後，才進入
具體賣方策略構造。資料可行性（TXO歷史IV資料是否可從FinMind取得或需要
反推）需在第1關前先查證。
