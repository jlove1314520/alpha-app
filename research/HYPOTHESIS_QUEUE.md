...
36. 個股融券使用率（Short Sale Utilization Ratio）——**2026-09-05
    ...
    percentile=100.0，見上方#36條目與`TRIALS_LEDGER.md`#129）。**第2關
    （隨機控制組）本輪已完成並判PASS**（`short_sale_utilization_
    portfolio_v1.py`，測訊號多頭鏡像半邊，放空腿因引擎不支援尚未驗證，
    見上方#36條目誠實揭露段落）：TRAIN真實策略+59.53%vs買進持有+58.86%、
    alpha年化+7.44%但p=0.3717不顯著、隨機控制組percentile=100.0；
    VALIDATION真實策略+72.46%vs買進持有+54.58%、alpha年化+11.88%
    p=0.0354顯著、隨機控制組percentile=100.0，完整數字見上方2026-09-05
    hypothesis_queue排程接續最新狀態與`TRIALS_LEDGER.md`新增列。**第3關
    （參數密集高原）本輪已完成並判PASS**（新增`short_sale_utilization_
    gates.py`，逐字比照`f52w_high_gates.py`#17第3關同一套TOP_N/
    REBALANCE_DAYS雙維度網格精神，TRAIN期1x成本）：TOP_N in
    [10,15,20,25,30]報酬依序+74.19%/+77.74%/+59.53%/+54.37%/+41.01%，
    REBALANCE_DAYS in [10,21,42,63]報酬依序+47.29%/+59.53%(錨點)/
    +54.05%/+53.65%，網格共8點**全部為正（100%，遠高於60%門檻）**，非
    單點孤峰，見`data/short_sale_utilization_gate3_grid.csv`與
    `TRIALS_LEDGER.md`#134。第5/6關（`short_sale_utilization_gate5_
    loo.py`）本輪已完成並判**PASS**：TRAIN逐年報酬2015~2020分別為
    -1.92%/+5.69%/+21.87%/+2.13%/+14.24%/+8.22%，第5關拿掉貢獻最大的
    2017年後剩餘複利總報酬+30.90%仍為正、第6關5/6=83.3%年度為正皆過關，
    見`TRIALS_LEDGER.md`#135。**尚未結案**——TRAIN期alpha不顯著
    (p=0.3717)但VAL期顯著(p=0.0354)這個組合跟本佇列既往FAIL案例常見
    型態相反，需要更審慎判斷才能下最終alpha顯著性判準，本輪刻意不妄下
    結論。`is_holdout_consumed()`執行前後皆確認False。**第9關（下檔
    保護）本輪已完成並判機械PASS**（`short_sale_utilization_gate9_
    regime_overlay.py`，把#10`regime_overlay.py`首次正式接上真實候選，
    TRAIN/VAL兩期MDD跟地雷率皆改善，但2022窗口顯示未經校準的通用regime
    先驗會誤殺選股訊號本身的特異性報酬——baseline+25.53% vs overlay僅
    +9.49%，完整數字與誠實保留見`TRIALS_LEDGER.md`#136）。**佇列排隊
    順序仍是#36（未結案，1~3/5/6/9關皆PASS，唯一剩下的未決問題是TRAIN/
    VAL alpha顯著性不一致的最終判斷）**，剩餘#5/#6/#8/#10仍卡外部依賴
    （同上）。
