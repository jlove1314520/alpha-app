...
if __name__ == "__main__":
    ...
    print("  1.【2026-09-15已完成】company_info.json 已補officialwebsite/official_domain/")
    print("     official_website_source三欄，來源MOPS t187ap03_L（上市）/mopsfin_t187ap03_O")
    print("     （上櫃）/mopsfin_t187ap03_R（興櫃），3137檔中2342檔有官網（見")
    print("     research/build_company_official_websites.py），與既有9家人工核實種子")
    print("     清單比對僅2家網域字面不同（2317鴻海honhai.com vs foxconn.com、3130")
    print("     一零四corp.104.com.tw vs 104.com.tw，皆同集團不同官方網域，非錯誤）。")
    ...
    print("     negative_control.py自己的測試迴圈（該檔案docstring原本誤判斷theme_")
    print("     keywords.json唯一消費者是測試腳本本身，2026-09-15下一輪已查證並")
    print("     更正：真正的正式消費者是scripts/build_themes.py的KWMAP，見")
    print("     negative_control.py docstring更正段落）。並確認classify_sentence_v1/")
    print("     v2的使用前提（呼叫端須先做關鍵詞比對，這兩個函式本身不做）。")
    print("  【2026-09-15假設佇列第八輪已完成】待辦4延伸範疇的123題材全面跨行業")
    print("     誤判掃描：見research/theme_keyword_ambiguity_scan.py。發現7個跨題材")
    print("     重複關鍵詞、51個短ASCII縮寫關鍵詞（38個題材）；對496句真實內文引言+")
    print("     300則標題語料實測，找到2筆裸substring命中但詞界比對不命中的案例，")
