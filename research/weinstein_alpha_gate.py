"""weinstein_stage2_unbiased 的 alpha/beta 顯著性關卡（`WEINSTEIN_ALPHA_GATE_TASK.md`）。

2026-08-25 使用者直接指示：解鎖 holdout 前，先把 train/val 兩期的策略報酬對大盤
（TAIEX）回歸，報告 alpha（截距）、beta、扣成本後有沒有贏買進持有；若 alpha 沒有
明確為正、且沒有通過顯著性關卡，直接判否決（純 beta，非真 alpha），不解鎖
holdout。這裡本身不會呼叫 `unlock_holdout_once()`——通過關卡只代表「可以回報使用者
由使用者決定」，不是自動解鎖。

沿用既有工具（`long_only_vs_market.decompose_alpha_beta()`、
`validation.control_group.extract_trade_schedule()`、`strategies.run_weinstein_unbiased.main()`）。
唯一新寫的部分：`validation/control_group.py::run_matched_control_group()`的
`one_draw()`只回傳最終淨值純量，這裡平行寫一個回傳完整`date`/`equity`每日序列的
版本（`_matched_draw_equity_curve()`，不改動原函式本身），因為 alpha/beta 拆解需要
完整日報酬序列，不能只看最終淨值。

方法論更新：任務文件原始措辭用「Bonferroni」，但`MARATHON_PROTOCOL.md`第2節已於
2026-08-25 修正為 BH-FDR（q=0.10，三軌獨立分母），這裡遵循現行版本，不是任務文件
字面上的舊版校正法。

Run with: `python weinstein_alpha_gate.py` from research/.
"""
...
from strategies.weinstein_stage2 import prepare_market_data, prepare_price_data, stage2_signal
from strategies.run_weinstein_unbiased import (
    SAMPLE_SEED, SAMPLE_SIZE, START_DATE, load_sample_price_data,
    main as run_weinstein_main,
)
from universe import universe as build_universe
from long_only_vs_market import decompose_alpha_beta

N_RANDOM = 200
CONTROL_SEED = 20260822  # 沿用 run_weinstein_unbiased.py/control_group.py 既有慣例種子
SINGLE_TEST_THRESHOLD = 90.0  # 沿用專案慣例的單測門檻
...
def _matched_draw_equity_curve(trade_schedule, indexed_price_data, calendar, initial_capital,
    ...
    """跟 `run_matched_control_group()` 的 `one_draw()` 同一套候選/選股邏輯（同進出場
    日期，只隨機換股，候選限定「entry_date 跟 exit_date 都有價格」的股票），但這裡
    回傳完整每日 date/equity 序列，不是只回傳最終純量。
    """
...
def run_alpha_gate_for_period(label, result, market_df, indexed_price_data, n_random=N_RANDOM,
    ...
    print(f"  配對式隨機控制組（n={len(valid)}/{n_random} 有效抽樣，同進出場日期，只換股，"
          f"每次抽樣也做alpha/beta拆解）：")
    if valid:
...
def main():
    print("=== weinstein_stage2_unbiased alpha/beta 顯著性關卡（WEINSTEIN_ALPHA_GATE_TASK.md）===")
    ...
    print("\n步驟1-3：重用 run_weinstein_unbiased.main()（同一批快取樣本，零新API呼叫）")
    results, train_result, val_result, cg = run_weinstein_main()

    market_raw = load_dev("TaiwanStockPrice", "TAIEX", START_DATE)
    holdout.assert_no_holdout_leakage(market_raw, context="market_raw in weinstein_alpha_gate")
    market_df = prepare_market_data(market_raw)

    print("\n重建與 run_weinstein_main() 內部同一批 price_data（同SAMPLE_SEED，完全命中本機快取）...")
    u = build_universe()
    ...
    print(f"  {len(price_data)}/{SAMPLE_SIZE} 檔可用（應與run_weinstein_main()內部一致）")
    ...
    ]).to_csv("data/weinstein_alpha_gate_summary.csv", index=False)
    pd.DataFrame(val_cost).to_csv("data/weinstein_alpha_gate_val_cost_sensitivity.csv", index=False)
    pd.DataFrame(train_cost).to_csv("data/weinstein_alpha_gate_train_cost_sensitivity.csv", index=False)
    print("\n輸出：data/weinstein_alpha_gate_summary.csv、"
          "data/weinstein_alpha_gate_{val,train}_cost_sensitivity.csv（皆gitignored）")
