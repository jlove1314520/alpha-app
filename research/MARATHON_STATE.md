...
**最後更新：2026-09-05T19:00+08:00**

**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 373 輪。最新一輪：第 373 輪 · 2026-09-05T19:00+08:00 · TW（取鎖乾淨，依輪替選TW，TW 10:05最舊）**。**本輪工作單位＝`MARATHON_PROTOCOL.md`第3節「季節性（月效應/財報季效應）」家族橫斷面構造可行性查核**（round365下一步(a)點名候選，純讀`pit.py`/`factors.py`/`DATA.md`既有紀錄，零新增FinMind API呼叫）。**結論：三條可能構造路徑全部卡在資料源結構性限制**——(1) 財報公告時點差異：`pit.py`的`pit_date`對所有股票全用「期末+45天」同一公式，天生無跨股票變異；(2) 月營收公告時點差異：真實`create_time`僅約2026-04起才有值，本專案TRAIN/VAL回測窗口完全早於此，窗口內無可用變異；(3) 規模別一月效應：`TaiwanStockMarketValue`付費限定，台股軌無市值資料源可分層。**跟round365「成長與預估上修」同一類「資料源限制」正式標記結案**，不進入統計檢定、不計入`bonferroni_n`。`is_holdout_consumed()`開工/收工前皆確認`False`。零新增API呼叫。詳見`TW_MARATHON_STATE.md`/`TW_LOG.md`/`TW_LEADS.md`第373輪記錄、`TRIALS_LEDGER.md`新增列。`MARATHON_PROTOCOL.md`第3節TW軌尚未碰過的因子家族現在只剩籌碼類融資券市場整體水位（`MI_MARGN`）一項。
