...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 374 輪。最新一輪：第 374 輪 · 2026-09-05T19:30+08:00 · US（取鎖乾淨，依輪替選US，US 10:34最舊）**。**本輪工作單位＝收成round366投遞的脫離session背景工作**：`deep_dive_f_us_value_bm_leave_extreme_out.py`（`f_us_value_bm` universe假影leave-top10-out集中度檢查）已在本輪開工前正常跑完並產出CSV。**結果：排除own-return前10名極端贏家後（125檔），ann_return不降反升（+121.38%→+148.17%），beta-0.027，遠超腳本事前綁定的「>+60%＝集中度假說REFUTED」門檻，判定REFUTED**——比round360原猜測的「少數熱門股主導」更嚴重，代表`cached_ticker_ids()`池子的異常量級分佈廣泛在中段贏家，不是集中在少數幾檔名字，未來要挽救這個池子上的因子測試，目標式排除幾檔股票不是解方，必須換非熱門股偏誤的隨機/分層抽樣宇宙重建。`f_us_value_bm`本身FAIL判定不受影響、不重啟，同一結論回頭適用於`f_us_low_vol`（#15）。`is_holdout_consumed()`開工/收工前皆確認`False`。零新增API呼叫（純讀取round366已產出的CSV）。詳見`US_MARATHON_STATE.md`/`US_LOG.md`/`US_LEADS.md`第374輪記錄、`TRIALS_LEDGER.md`#139。
