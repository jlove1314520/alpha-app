...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 495 輪（含補記的第405輪，見下方缺口說明）。最新一輪：第 495 輪 · 2026-09-09T17:30+08:00 · TW軌（取鎖乾淨，cycle`20260909-173036`）**。**本輪工作單位**：發現並提報架構性問題——`#50`（容量受限小型股）的逐筆tick累積樣本股票不在目標成交值區間。過去8輪（487~494）心跳暗示「等18個交易日就好」，本輪核對`data/ticks/20260908.parquet`實際涵蓋股票（零新增API呼叫）發現訂閱清單來自App即時報價用的`DEFAULT_TW_WATCHLIST`（2330/2454/2317/1513/3231），15檔全部是大型/中型股，用既有tick估算單日成交值最小的1513也約2.1億元，遠超`#50`要求的500萬～5,000萬區間上限4倍以上——成交值區間由「訂閱了哪些代號」決定，不是由「訂閱了多久」決定。已寫`PROPOSAL_2026-09-09_gate50_tick_universe_mismatch.md`提報：建議利用`MAX_DYNAMIC_SUBSCRIPTIONS=100`目前僅用5個的餘裕加入目標區間代號，但因牽涉常駐服務訂閱名單變更與外部API頻率上限風險，依「提案先於執行」規則未直接執行，待核准。`trial_registry.py --check`本輪未重跑（無新試驗）。`is_holdout_consumed()`開工/收工前皆確認`False`。全程零新增外部API呼叫。詳見`REPORT.md`第495輪記錄、`TW_MARATHON_STATE.md`第495輪記錄、`PROPOSAL_2026-09-09_gate50_tick_universe_mismatch.md`。
