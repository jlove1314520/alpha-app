...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 77 輪。最新一輪：第 77 輪 · 2026-08-25 22:31 · US · 取鎖時偵測到LOCK_STALE（上一輪疑似異常中止），接手完成第12項子步驟(b)(c)——filer category反推分類器跑PLTR重測；結論：PLTR全部財年分類LAF跟既有假設一致，filer category誤判假說被證偽，第12項結案（負向結果），不建議再投入輪次。 這是跨 TW/US/FUT 三軌共用的單一遞增計數器，不是各軌自己 commit message 裡那種各軌獨立的輪號。**下一輪的義務：讀這行的數字 N，這輪結束前（見 `MARATHON_PROTOCOL.md` 第 6 節）在 `REPORT.md` 最上面的「心跳記錄」區塊插入一筆 `## 第 N+1 輪 · ...`，並把這行的數字改成 N+1。** 不管這輪做了什麼、有沒有成功，都要留這一筆，這樣任何人（包含使用者自己）打開 `REPORT.md` 最上面就能一眼看出馬拉松最近有沒有在跑、多久跑一次、有沒有卡住。
...
**進度追蹤**：這份任務預期跨多輪才能做完，每輪做到哪個修正項目、下一輪接續哪裡，更新在這一段（覆寫，不要疊加流水帳）。**目前進度（2026-08-25 晚，互動session完成，非排程輪次）：修正1（BH-FDR重新評分）第一步已完成——criteria_v2已鎖定（`CRITERIA_V2_LOCK.md`）、37筆既有試驗已用新標準重新評分完畢（完整對照表見`TRIALS_LEDGER.md`底部）。結果：TW軌1筆復活（`f_value_pe`，改列「待複驗候選CANDIDATE」）、FUT軌0筆復活（22次策略試驗維持全部FAIL/不確定，誠實陰性結果）。三個train/val方向反轉的假說（`f_foreign_streak`/`f_rel_strength`/`f_quality_roe_stability`深挖）已標記轉入修正2的情境依賴候選調查，但情境分群IC本身還沒有算。完整過程見`REPORT.md`2026-08-25T22:10條目。下一輪從修正2（情境條件式分群IC框架）開始，或先做`f_value_pe`的成本敏感度測試（修正1的收尾）。**
