...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 345 輪。最新一輪：第 345 輪 · 2026-09-04T21:39+08:00 · US（取鎖時偵測到`LOCK_STALE`，pid 52820持有30.0分鐘後被回收，第344輪疑似異常中止未釋放鎖。新增`us_factors_value.py`：US軌第一個PIT基本面因子`f_us_value_bm`(book-to-market)，發現並修正兩個真bug——XBRL維度context誤植離群值、以及股數未隨未來股票分割回溯調整導致book-to-market被分割倍數放大偽影(AAPL修正前2009年約12倍，修正後0.01~0.43合理範圍)，已加分割偵測回推調整。純1c地基+資料品質修正，未跑cheap gate，無候選判定，`is_holdout_consumed()`確認`False`，零FinMind呼叫。詳見`US_MARATHON_STATE.md`/`US_LOG.md`/`REPORT.md`第345輪）。上一輪：第 344 輪 · 2026-09-04T21:06+08:00 · FUT（取鎖時偵測到`LOCK_STALE`，pid 52496持有30.0分鐘後被回收，第343輪疑似異常中止未釋放鎖，複查確認`deb2a67`已完整推送非資料遺失。三軌時間戳：TW 20:47第343輪最新／US 19:35第342輪／FUT 18:36第341輪最舊——依輪替選FUT。接續round341下一步(a)，新增`fut_stock_futures_liquidity_screen.py`對511檔F結尾個股期貨候選池做分層抽樣（每15檔取1，37檔含round341既有快取CDF/CCF）2024年front-month流動性初篩：寬鬆門檻（`active_days>=200`且`mean_volume>=50`口/日，執行前先寫死）下19/37(51.4%)達標，外推511檔中約262檔可能可用——比現有TX/MTX/TE/TF大一個數量級，是FUT軌首次出現接近股票橫斷面精神的候選池規模。誠實限制：寬鬆門檻不代表可交易，只查2024一年，round341下一步(b)轉倉規則、(c)dispersion_ratio/PCA離散度實測（決定整個方向能否成立的關鍵）仍未做。非TRIALS_LEDGER判定，補記`FUT_LEADS.md`。**額外異常觀察**：round341遺留的`fut_probe_stock_futures_info.py`本輪開工時已從磁碟消失（從未commit過），跟第343輪TW記錄的「腳本執行後檔案本身從磁碟消失」是同一異常模式第二次出現，超出FUT軌範圍未深究根因，如實記錄供使用者參考。35次新API呼叫，全程守節流未遭限流，`is_holdout_consumed()`確認`False`。詳見`REPORT.md`第344輪條目、`FUT_MARATHON_STATE.md`/`FUT_LOG.md`/`FUT_LEADS.md`本輪記錄）。上一輪：第 343 輪 · 2026-09-04T20:47+08:00 · TW（取鎖時偵測到`LOCK_STALE`，pid 51012持有29.9分鐘、第342輪疑似異常中止未釋放鎖，已回收；同時發現並補commit round342遺留6個未commit檔案。嘗試延續round340補完整關卡(成本敏感度+隨機控制組n=15)於`ic_weighted_train_only`，但`train_only_ic_weight_bigsample_full_gate.py`三次執行遭遇腳本檔案本身從磁碟消失的未知異常，無新候選判定，詳見`REPORT.md`第343輪條目）。上一輪：第 342 輪 · 2026-09-04T19:35+08:00 · US（判讀round336/339背景process完成的深挖結果，201檔可用1b深挖：TRAIN percentile54-75未過、VAL percentile100但beta-0.90，`f_us_low_vol`家族FAIL重現且徹底結案，`TRIALS_LEDGER.md`#115/`US_LEADS.md`#15；另補齊FUT第341輪心跳缺口）。上一輪：第 341 輪 · 2026-09-04T18:36+08:00 · FUT（查證個股期貨資料源存在，1139檔含CDF/CCF，粗估F結尾候選約509檔，補記`FUT_LEADS.md`，非TRIALS_LEDGER判定；原commit`76334b9`漏同步本檔，本輪補齊）。上一輪：第 340 輪 · 2026-09-04T18:10+08:00 · TW（取鎖時偵測到`LOCK_STALE`，pid 17608持有30.1分鐘後被回收，第339輪疑似異常中止未釋放鎖，已回收；依輪替選TW，接續第337輪「回頭評估組合策略層級是否有其他迭代方向」，新增`train_only_ic_weight_bigsample.py`——用同一批300檔安全樣本池的TRAIN-only mean\|IC\|重算`portfolio_multifactor_v2`的`ic_weighted`權重，修補`LEADS.md`「誠實限制(2)」點出的權重洩漏疑慮（原權重用VAL期IC，等於看過VAL表現才拿去測VAL）。結果：VALIDATION quarterly alpha+4.83%(p=0.4314)、monthly alpha+5.93%(p=0.1996)，皆比round327原權重（p=0.5314）略有改善但仍遠未過0.05門檻，**判定FAIL不變**，但排除了「原FAIL是加權方法論瑕疵造成偽陰性」的潛在反駁，寫入`TRIALS_LEDGER.md`#109、`LEADS.md`本輪補充。零新增API呼叫，執行約9分鐘，詳見TW_MARATHON_STATE.md）。上一輪：第 339 輪 · 2026-09-04T17:32+08:00 · US（取鎖偵測到陳舊鎖檔已回收；接續round336未完成的deep_dive_f_us_low_vol_cached_universe.py 1b深挖重跑，重新啟動背景process確認存活，但本輪session預算再次於完成前耗盡，process仍在背景繼續跑未kill，該輪commit只更新了US_MARATHON_STATE.md未同步REPORT.md本檔，第340輪已補齊心跳，詳見US_MARATHON_STATE.md）。上一輪：第 338 輪 · 2026-09-04T09:33+08:00 · FUT（延續round335選項(a)，新增`fut_multi_commodity_dispersion.py`量化TX/MTX/TE橫斷面離散度：dispersion_ratio=0.1143、PC1解釋變異=97.91%，兩指標互證跨商品池離散度不足以支撐排列檢定，round335選項(b)查證台股個股期貨資料源變成下一輪優先方向，純診斷不佔TRIALS_LEDGER列，詳見FUT_MARATHON_STATE.md/FUT_LOG.md）。上一輪：第 337 輪 · 2026-09-04T09:03+08:00 · TW（重跑`#91` revenue_trend_surprise_low_attention 300檔cheap gate，低關注度組FAIL(50.2)、高關注度組CHEAP_PASS(100.0)但方向與原假說相反且TRAIN無訊號，整體維持FAIL不進待深挖清單，`CALIBRATION_PROBE.md`三項TW軌未定任務全部處理完畢，詳見TW_MARATHON_STATE.md/TRIALS_LEDGER.md#106）。上一輪：第 336 輪 · 2026-09-04T08:33+08:00 · US（取鎖偵測到陳舊鎖檔已回收；接續#104開放問題，新增deep_dive_f_us_low_vol_cached_universe.py重跑未分層版1b深挖，但session budget耗盡process未完成，未偽造結果，下一輪先檢查是否已產出再決定重跑或讀取，詳見US_MARATHON_STATE.md）。上一輪：第 335 輪 · 2026-09-04T08:03+08:00 · FUT（修正`continuous_contract.py`真bug——MTX週合約`contract_date`格式如`201308W1`未過濾導致`build_continuous_series(contract='MTX')`crash，已修好；新增`fut_multi_commodity_pool.py`建出TX/MTX/TE首版跨商品日報酬池，零新增API呼叫。重要發現：重疊窗口內TX-MTX相關係數0.997、TX/MTX-TE約0.954~0.955，三者幾乎是同一大盤beta的三種包裝、非相對獨立標的，「跨商品橫斷面池」方向需先評估離散度是否足夠支撐排列檢定，非TRIALS_LEDGER判定，詳見FUT_MARATHON_STATE.md/FUT_LOG.md）。上一輪：第 334 輪 · 2026-09-04T07:33+08:00 · TW（接續第331輪完成`#79`後的下一步，重跑`#77 f_rel_strength`產業中性版300檔樣本cheap gate：248/300可用，train/val同號但null percentile從82.8驟降到41.9，反駁「更大樣本統計力改善」的猜測，判定從「未定」正式改回確定FAIL，寫入`TRIALS_LEDGER.md`#101，詳見TW_MARATHON_STATE.md/TW_LOG.md）。上一輪：第 333 輪 · 2026-09-04T07:03+08:00 · US（接續第329輪「下一步」(a)+(b)，新增`us_portfolio_pilot_real_data.py`把`us_portfolio_backtest.py`引擎接上138檔真實快取美股+SPY，零新增API呼叫。訊號用已定案FAIL的`f_us_momentum_12m`純粹當接線測試，不寫TRIALS_LEDGER：TRAIN(2015-2020) total_return+132.55% MDD-34.51% beta+0.867 alpha_ann+6.21%(p=0.242不顯著)；VAL(2021-2024) total_return+62.13% MDD-47.92% beta+0.902 alpha_ann+6.03%(p=0.691不顯著)，引擎全程無crash、unresolved_at_end皆空。US軌「組合回測地基」至此完成，下一步卡點是US軌至今無任何深挖後成立的PASS因子，需先開SEC EDGAR基本面因子家族或擴大既有因子測試樣本規模，詳見US_MARATHON_STATE.md/US_LOG.md）。上一輪：第 332 輪 · 2026-09-04T06:33+08:00 · FUT（依2026-09-03裁示「US/FUT主軸也是組合策略層級」，把round328記下的未來方向——測試TX以外TAIFEX商品當多標的池——往前推一步：新增`fut_probe_multi_commodity.py`唯讀查證MTX/TE/TF三商品資料可用性，皆有資料、深度足夠（2000/2001年起～2024-12-31），唯TF只有日盤無夜盤；非因子判定不佔TRIALS_LEDGER列，跨商品pooling方法論設計待未來輪次，詳見FUT_MARATHON_STATE.md/FUT_LOG.md/FUT_LEADS.md）。上一輪：第 331 輪 · 2026-09-04T06:10+08:00 · TW（接續第330輪，重跑`#79 f_inst_streak_days`300檔樣本cheap gate並跑到完成，248/300可用，train/val正負號仍相反、null percentile 86.1未過90.0門檻，`CALIBRATION_PROBE.md`「未定」正式改回確定FAIL，寫入`TRIALS_LEDGER.md`#100；同時補齊第330輪漏掉的心跳更新，詳見TW_MARATHON_STATE.md/TW_LOG.md/REPORT.md）。上一輪：第 330 輪 · 2026-09-04T05:32+08:00 · TW（啟動#79 300檔重跑，session預算提前結束、process結果未知，該輪commit只更新TW_LOG.md未同步本檔/REPORT.md，第331輪已補齊）。上一輪：第 329 輪 · 2026-09-04T05:07+08:00 · US（暫停規則已解除，恢復實質工作；新增`us_portfolio_backtest.py`組合回測地基，合成資料自我測試PASS，零API呼叫，純基礎建設無候選判定，詳見US_MARATHON_STATE.md/US_LOG.md）。上一輪：第 328 輪 · 2026-09-04T04:31+08:00 · FUT（取鎖偵測到陳舊鎖檔已回收；查證CALIBRATION_PROBE.md「#34同理重跑」指令對FUT軌不可行，TRIALS_LEDGER.md#34維持FAIL，詳見FUT_MARATHON_STATE.md/LEADS.md）。上一輪：第 327 輪 · 2026-09-04T04:15+08:00 · TW（修好safe_pool_ids真bug＋300檔quick scan首次拿到大樣本p值，portfolio_multifactor_v2仍FAIL，詳見TW_MARATHON_STATE.md/LEADS.md）。最新一輪：第 325 輪 · 2026-09-04T03:20+08:00 · TW（暫停規則已解除，調查換更大樣本重跑地基，本輪未及跑完整回測，session預算用盡提前收工）。**取鎖乾淨（非陳舊鎖檔）。三軌時間戳：US 23:03（第321輪，最舊）、TW 23:31（第322輪）、FUT 2026-09-04T00:01（第323輪，最新）——依輪替選US。獨立複查三個解除條件皆未成立（`PORTFOLIO_STRATEGY_SPEC.md`第3行仍「狀態：待使用者確認」、`LEADS.md`最新`portfolio_multifactor_v2`仍FAIL、暫停規則未被修改）。US軌沒有組合策略相關工作可做（規格書全部圍繞TAIEX/TWSE台股樣本），沒有已知的剩餘允許工作項目，本輪整輪跳過，未做任何實質工作，零API呼叫。`is_holdout_consumed()`確認`False`（本輪成功以既有指令路徑重新驗證）。`git status`確認：既有非本輪產生的殘留變更（`.github/workflows/market.yml`／`.github/workflows/quotes.yml`已修改、約17個未追蹤log/腳本殘留檔案），未觸碰、未查看、未納入本輪commit。**提醒使用者：自第110輪暫停規則生效以來，三軌合計已連續跳過約215輪、跨度約197.9小時（約8.25天），需使用者親自確認`PORTFOLIO_STRATEGY_SPEC.md`或裁示選項(a)/(b)/解除暫停規則三者之一才能恢復進度。** 詳見`REPORT.md`第324輪條目、`US_MARATHON_STATE.md`/`US_LOG.md`本輪記錄。
