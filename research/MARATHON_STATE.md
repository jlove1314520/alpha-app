...
**最後更新：2026-09-06T00:32+08:00**

**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 381 輪（計數器先前落後於`REPORT.md`實際心跳輪次——TW軌round377/379/380在計數器停留376時已持續進行且`REPORT.md`逐輪未斷，本輪對齊`REPORT.md`最新心跳編號380、本輪記為381，同round372/375先例「維修/落後期間計數器落後但REPORT.md逐輪未斷」）。最新一輪：第 381 輪 · 2026-09-06T00:32+08:00 · US（取鎖乾淨，TW最新22:00／US次舊20:35／FUT最舊18:30但配額已滿跳過，選US）**。**本輪工作單位＝接續round376「下一步」**：`us_stratified_universe_sample.py`分層抽樣宇宙續抓（冪等重跑，已抓到的72檔large tier命中快取不重複耗額度）。開工前確認`data/rate_limit_state.json`已不存在（round376記錄的鎖定22:38+08:00已過期超過1.8小時），用`run_detached.py submit`投遞續抓（job_id`20260906-003203-4f80`，timeout 40分鐘），**session內監看約3分鐘證實FinMind額度確實已恢復**（第80/300檔起持續抓到第146/300檔mid tier中段，無新的402錯誤）。本輪未等待job跑完即收工（符合第0b節規則），job以breakaway繼續在背景執行，估計8分鐘內可跑完。下一輪接手：查job是否`finished`，統計最終可用檔數，用乾淨宇宙重跑`f_us_value_bm`/`f_us_low_vol`驗證量級是否恢復合理。`is_holdout_consumed()`開工/收工前皆確認`False`。詳見`US_MARATHON_STATE.md`/`US_LOG.md`第381輪記錄。
