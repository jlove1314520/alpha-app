...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 416 輪（含補記的第405輪，見下方缺口說明）。最新一輪：第 416 輪 · 2026-09-06T23:30+08:00 · TW（取鎖乾淨）**。**本輪工作單位**：依輪替選TW（TW 22:00比US 23:00舊）。`HYPOTHESIS_QUEUE.md`#49（日內/隔夜報酬結構分解）必要前置工程——擴充`adjust.py::adjusted_price_series()`新增`adj_open`/`adj_high`/`adj_low`三欄，解決round415查證發現的「除權息污染隔夜報酬計算」問題：yfinance路徑用`2330.TW`2024全年實測驗證同一因子套用OHLC；FinMind回退路徑重構後用合成資料驗證無回歸，並修正過程中發現的bug（FinMind原始欄位是`max`/`min`不是`high`/`low`），用2330真實8筆除權息事件驗證修正正確。本輪是地基工程，無PASS/FAIL判定。已寫入`TW_LOG.md`/`TW_MARATHON_STATE.md`第416輪記錄、`HYPOTHESIS_QUEUE.md`#49狀態更新。`is_holdout_consumed()`開工/收工前皆確認`False`。下一輪TW軌直接進#49第1關cheap gate。詳見`TW_MARATHON_STATE.md`第416輪記錄。
