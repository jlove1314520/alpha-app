...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 364 輪。最新一輪：第 364 輪 · 2026-09-05T09:02+08:00 · FUT（取鎖時偵測到`LOCK_STALE`，round363的pid 58616持有29.8分鐘後被回收；US round363 state記錄本身已完整、描述為執行中因效能問題被迫`taskkill`中止，非收工序卡住，跟round360/361的收工序卡住模式不同，暫不視為需要額外查核的異常）。**本輪工作單位＝round361下一步(b)：拿掉高相關的`fut_ma_crossover_20_60`分量，只留`fut_trend_multi_tf`+`fut_oi_price_confirm_5d`兩個pairwise相關較低(0.125)的成分重測等權組合**——`fut_cheap_gate.py`新增`hyp_combo_trend_oi_v1()`，沿用round361同一套`_permutation_test()`配對式隨機排列框架N=200，零新增API呼叫。**結果：percentile=82.5，FAIL**（門檻90.0，`TRIALS_LEDGER.md`#132），真實策略終值+170.8% vs 隨機中位數-6.9%。**關鍵發現**：拿掉ma後percentile從3成分版76.5回升到82.5，但恰好等於單一最強成分`fut_trend_multi_tf`本身的percentile(82.5)，2成分等權組合未產生任何額外分散化增益，等同「trend_multi_tf單獨跑，外加一個大部分時間是啞巴的oi濾網」。**FUT軌組合層級兩次嘗試（3成分round361、2成分本輪）均未能超越單一最強成分，等權投票路線階段性結案**，下一步（IC加權/oi變體/優先權讓回TW-US）留給後續輪次判斷。`is_holdout_consumed()`開工/收工前皆確認`False`。詳見`FUT_MARATHON_STATE.md`/`FUT_LOG.md`/`FUT_LEADS.md`第364輪記錄、`TRIALS_LEDGER.md`#132。
