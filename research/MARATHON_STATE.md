...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 351 輪。最新一輪：第 351 輪 · 2026-09-05T02:34+08:00 · US（取鎖時偵測到`LOCK_STALE`，pid 55480持有29.9分鐘後被回收，對應第350輪疑似失敗留下未commit產物；本輪回收稽核後commit，並用90秒硬超時逐一單獨測試第350輪卡死的4檔`WMT`/`XPER`/`ZETA`/`ZSQR`，全部瞬間完成、無法重現卡死，推翻「尾端某檔拖慢」的假說，判斷較可能是暫態環境因素，詳見`US_MARATHON_STATE.md`/`US_LOG.md`第351輪記錄）。
