...
**最後更新：2026-09-08T04:30+08:00**

**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 429 輪（含補記的第405輪，見下方缺口說明）。最新一輪：第 429 輪 · 2026-09-08T04:30+08:00 · US軌（取鎖乾淨，cycle`20260908-043037`）**。**本輪工作單位**：收成job`20260907-054341-a450`（`deep_dive_f_us_gross_profitability`）並補做leg分解判讀——新增`deep_dive_f_us_gross_profitability_contamination_check.py`，確認5檔已知污染ticker（`AMTX`/`CIIT`/`DVLT`/`MNTS`/`WULF`）96%的腿位分配落在空頭腿，排除後VAL 1x ann_return由+33.77%驟降至+1.53%（retained_fraction=0.05），觸發CONFIRMED門檻。**判定：`f_us_gross_profitability`（#191 CHEAP_PASS）降級為FAIL**，跟`#20`/`#21`同一個`adj_close`資料完整性陷阱，排除出`portfolio_multifactor_v2`（US版）候選池。已登記`TRIALS_LEDGER.md`#193、`US_LEADS.md`#23更新、`US_MARATHON_STATE.md`第429輪記錄。`trial_registry.py --check`確認PASS（exit=0，195列，下一可用編號#194）。**下一輪US軌接手**：評估是否從宇宙層級永久排除這批已知污染ticker（`WATT`/`AMTX`/`MNTS`/`DVLT`/`WULF`/`CIIT`/`PALI`/`LEE`）再重驗`#20`/`#21`/本因子有無殘餘訊號，或改測不依賴空頭腿的Top-N構造；TW軌待辦：`#53`第2關隨機控制組+相位敏感度掃描。`is_holdout_consumed()`開工/收工前皆確認`False`，零新增API呼叫。
