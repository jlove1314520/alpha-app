...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 359 輪。最新一輪：第 359 輪 · 2026-09-05T06:03+08:00 · TW（取鎖乾淨。開工交叉核對`ps aux`/`Get-CimInstance Win32_Process`，發現round356留下的背景process（`deep_dive_loo_no_low_vol_independent_sample.py`）已異常消失，`data/deep_dive_loo_no_low_vol_independent_sample.csv`未產生、log只停在4行輸出無錯誤訊息——判定異常中止，根因未查明（推測round356 session結束時背景process未真正脫離父行程）。依round356留下的`DEEP_DIVE_CADENCES`環境變數指示，改用`DEEP_DIVE_CADENCES=monthly`（限定monthly單一cadence）＋`-u`unbuffered（比照US round357改良做法）重跑，確認Windows PID 50060存活、log已開始輸出，process留在背景繼續跑（`round359_deep_dive_loo_no_low_vol_independent_sample_monthly.log`），未殺掉。暫不更新`TRIALS_LEDGER.md`/`TW_LEADS.md`候選判定欄位。`is_holdout_consumed()`確認`False`。零新增API呼叫。詳見`TW_MARATHON_STATE.md`/`TW_LOG.md`第359輪記錄）。
