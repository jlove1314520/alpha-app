...
**最後更新：2026-09-07T03:36+08:00**

**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 423 輪（含補記的第405輪，見下方缺口說明）。最新一輪：第 423 輪 · 2026-09-07T03:36+08:00 · US軌（取鎖乾淨，cycle`20260907-033036`）**。**本輪工作單位**：round421「下一輪US軌接手」明列待辦——查是否有除FinMind外的免費來源能取得`US_LEADS.md`#20/#21常駐短腿ticker（`WATT`/`AMTX`/`MNTS`/`DVLT`/`WULF`/`CIIT`/`PALI`）真實未還原歷史收盤價。新增`us_alt_source_raw_price_probe.py`查證三候選：yfinance（用`DVLT`實測，證實Close仍是split back-adjusted、與FinMind adj_close逐位元相同）、stooq.com（觸發JS反機器人驗證，依取得方式鐵律不繞過）、Alpha Vantage `TIME_SERIES_DAILY`（官方文件宣稱raw as-traded但需API key，本輪未實測）。**判定**：「免key免登入替代來源」假說REFUTED，Alpha Vantage列未驗證候選。`#20`/`#21`維持EXPERIMENTAL不變，且確認`adj_close`膨脹問題是Yahoo Finance/FinMind共通限制。已寫入`TRIALS_LEDGER.md`#188、`US_LEADS.md`#20/#21、`US_MARATHON_STATE.md`/`REPORT.md`第423輪記錄。`is_holdout_consumed()`開工/收工前皆確認`False`，零新增付費/需登入API呼叫。**下一輪US軌接手**：(1)若使用者同意可註冊Alpha Vantage免費key實測；(2)否則`#20`/`#21`誠實記錄「異常成因收斂到主流免費資料源共通限制」並考慮降級；(3)或改找US軌新因子候選。

**心跳缺口補記（第422輪，TW軌）**：`TW_MARATHON_STATE.md`/`REPORT.md`已有標記「第422輪」的TW軌內容（`HYPOTHESIS_QUEUE.md`#51子事件1第1關cheap gate，FAIL，`TRIALS_LEDGER.md`#186），但這份全局計數器當時未同步更新（同round405/408曾補齊過的同款缺口）——本輪（第423輪）已一併補上422與423。
