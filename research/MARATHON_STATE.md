...
**最後更新：2026-08-22T15:00:00+08:00**
...
**主要決策點：`weinstein_stage2_unbiased` 要不要做一次性 holdout 測試？** 這個候選 train/val 四項關卡全過，是目前唯一的 `EXPERIMENTAL`（非 FAIL）候選。Holdout 測過即焚、需要明確授權，這裡不會自己決定，等使用者指示。

背景待辦（優先度較低，不阻塞上面）：
1. 美股成本模型補到 `costs.py`。
2. 擴大抽樣規模，或真的逐檔掃全市場（目前 82/100 樣本，統計把握度還有限）。
3. 紙上前測影子帳本（里程碑 3 剩下的部分）。

## 目前沒有在等待使用者回覆的事

`weinstein_stage2_unbiased` 是否進 holdout——這個**是**在等使用者，因為 holdout 一次性、不可逆，不會自己決定。其餘背景待辦不阻塞，可以先做。
