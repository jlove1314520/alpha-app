...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 431 輪（含補記的第405輪，見下方缺口說明）。最新一輪：第 431 輪 · 2026-09-08T05:31+08:00 · US軌（取鎖乾淨，cycle`20260908-053037`）**。**本輪工作單位**：round429「下一輪US軌接手(1)」——從宇宙層級永久排除已知污染ticker、並用具名黑名單排除法重驗`#20`/`#21`是否有殘餘訊號。新增`us_contamination_blacklist.py`（8檔`WATT`/`AMTX`/`MNTS`/`DVLT`/`WULF`/`CIIT`/`PALI`/`LEE`共用模組），修改`us_factor_ic_value_clean_universe.py::load_clean_universe_tickers()`預設排除黑名單，新增`deep_dive_f_us_value_bm_contamination_check.py`/`deep_dive_f_us_low_vol_contamination_check.py`（沿用`#23`round429方法論，零新增API呼叫）。**結果：`#20`排除8檔後ann_return+142.25%→+50.95%，retained_fraction=0.36落PARTIAL；`#21`排除後+92.46%→+62.97%，retained_fraction=0.68達REFUTED——已知黑名單非`#21`異常主因，跟`#20`不同**。`#20`/`#21`FAIL判定皆不變（round425已關閉），本輪完成宇宙層級一次性修復並量化具名排除法解釋力。已登記`TRIALS_LEDGER.md`#196/#197、`US_LEADS.md`#20/#21更新。`trial_registry.py --check`確認PASS（exit=0，199列，下一可用編號#198）。**下一輪US軌接手**：掃描全樣本其他潛在反向分割污染標的，或改測`f_us_gross_profitability`單獨Top-N長多構造；TW軌待辦：`#55`三大法人買賣超截面離散度速度，開跑前須先做T86資料缺口二選一裁定。`is_holdout_consumed()`開工/收工前皆確認`False`，零新增API呼叫。
