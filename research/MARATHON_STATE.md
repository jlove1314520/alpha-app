...
**最後更新：2026-09-06T02:00+08:00**

**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 384 輪（計數器先前落後於`REPORT.md`/各軌STATE實際心跳輪次——US軌round381/382/383在計數器停留381時已持續進行，本輪對齊US_MARATHON_STATE.md最新心跳編號383、本輪記為384，同round372/375/381先例「維修/落後期間計數器落後但各軌逐輪未斷」）。最新一輪：第 384 輪 · 2026-09-06T02:00+08:00 · TW（取鎖乾淨，TW最舊22:00／US最新01:30／FUT最舊18:30但配額已滿跳過，選TW）**。**本輪工作單位＝接續`TW_MARATHON_STATE.md`round380「下一步(a)」**：盤點其他用完全打散`_shuffle_percentile()`框架測「慢變訊號×重疊窗口目標」的cheap gate是否有同款自相關高估問題。鎖定風險最高、尚是CHEAP_PASS未結案的`copper_gold_ratio_gate`（#125，|r|=0.18~0.25本佇列同類最強）優先查核，新增`copper_gold_ratio_circular_shift_control.py`做circular-shift自相關保留版控制組。**結果：VAL期percentile從完全打散版100.0降到circular-shift版62.0（跌破90門檻），確認第1關CHEAP_PASS判定本身在嚴謹控制組下不成立**，跟`margin_debt_level_v1`（#143）同款系統性問題確實存在；此候選本已因第5關集中度問題結案FAIL，本次是獨立補充的第二個死因，不改變既有判定。`is_holdout_consumed()`開工/收工前皆確認`False`。詳見`TW_MARATHON_STATE.md`/`TW_LOG.md`第384輪記錄、`TRIALS_LEDGER.md`#147。
