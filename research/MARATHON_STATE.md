...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 420 輪（含補記的第405輪，見下方缺口說明）。最新一輪：第 420 輪 · 2026-09-07T02:04+08:00 · TW軌（取鎖乾淨，cycle`20260907-020036`）**。**本輪工作單位**：`HYPOTHESIS_QUEUE.md`#51（強制交易者事件）子事件1「融券強制回補」反推重建備用路徑查證，接續前批`hypothesis_queue`排程留下的待辦(b)。純WebSearch查證+本機既有快取欄位盤點，零新增API呼叫：查得官方法規《證券商辦理有價證券買賣融資融券業務操作辦法》第76條（停止過戶前6個營業日起停止融券賣出、還券了結截止於停止過戶第6個營業日含）＋證券投資人及期貨交易人保護中心網站教育版本佐證同一數字；另確認台灣T+2交割下「除權除息交易日＝停止過戶日前2個營業日」，本機既有`TaiwanStockDividend`快取（2010-2024，2170檔）已足以反推：強制回補截止日=ExDividendTradingDate-4個營業日。**判定**：子事件1從「未死透待查」改判「可行（待實作驗證）」，不需抓TWSE/TPEx預告表端點即可重建，零新增API呼叫；因來源僅WebSearch兩類（官方法規資料庫+投資人保護中心），未達`CLAUDE.md`「四類來源涵蓋三類」滿門檻，下一輪實作前須用真實個股手動核對推算結果跟BFI84U即時預告表是否吻合。已寫入`HYPOTHESIS_QUEUE.md`#51子事件1新增段落、`TW_MARATHON_STATE.md`/`TW_LOG.md`第420輪記錄、`REPORT.md`第420輪心跳。`is_holdout_consumed()`開工/收工前皆確認`False`，零新增API呼叫。**下一輪TW軌接手**：(i)實作反推公式並用真實個股案例核對BFI84U快照驗證；(ii)完成子事件3（可轉債轉換價重設）的MOPS查證補齊三來源門檻；(iii)子事件1/3皆確認後設計#51第1關cheap gate。
