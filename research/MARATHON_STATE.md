...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 355 輪。最新一輪：第 355 輪 · 2026-09-05T04:04+08:00 · FUT（取鎖時偵測到`LOCK_STALE`，pid 55412，但複查round354本身心跳完整、正常收工，異常疑似發生在輪次交接之間，根因未查明。開工前`ps -ef`確認TW round353背景process（`deep_dive_loo_no_low_vol.py`）仍運行近1小時，呼應US round354記錄的CPU資源競爭現象，**本輪刻意避開任何重度Monte Carlo模擬**，改做round348(c)前置：19檔F結尾個股期貨「滾動式宇宙」可行性評估（新增`fut_stock_futures_rolling_universe_probe.py`，零新增API呼叫，<10秒完成）。**結果**：逐年在架數表顯示2011-2024（14年）最低10檔、2015-2024（10年）最低11檔——比round349兩個固定子集（8檔/346天 vs 4檔/1623天）明顯更優，能同時取得更長窗口跟更寬橫斷面。**三點誠實限制**：在架數不代表流動性足夠（僅2024驗證過流動性）、11/19檔有轉倉雜訊、N=10~13比已知檢定力不足的台股100檔cheap gate（N=54~74）更薄。非因子判定不佔`TRIALS_LEDGER`，補記`FUT_LEADS.md`。詳見`FUT_MARATHON_STATE.md`/`FUT_LOG.md`第355輪記錄）。
