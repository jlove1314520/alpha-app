...
**最後更新：2026-09-05T08:31+08:00**

**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 363 輪。最新一輪：第 363 輪 · 2026-09-05T08:31+08:00 · US（取鎖時偵測到`LOCK_STALE`，round360的pid 44236持有30.2分鐘後被回收，上一輪疑似失敗，根因未查明，`git log`確認round360工作本身已成功commit+push`6406cc5`，疑似只是收工序release lock前process卡住，同round361對FUT軌pid58488的先例。本輪延續round360「(c)先對現有135檔做leave-one-out集中度檢查（輕量版）驗證假影假說」的建議，新增`deep_dive_f_us_value_bm_leave_extreme_out.py`：預先綁定（hash-lock）排除VAL期自身報酬前10名「極端贏家」後重跑VAL期1x成本decile long-short回測，PASS/FAIL判準（<+30%=確認集中度假說／>+60%=否證／中間=部分）皆在程式碼docstring寫死於執行前。**執行時發現新的效能問題（本輪最主要收穫）：`load_value_sample()`載入226檔（135檔可用+91檔drop）的SEC company-facts JSON純本地解析（`book_value_per_share_pit()`，零新增API呼叫，純磁碟快取讀取）耗時約20分鐘（226檔loading loop在kill前才剛好跑完226/226），比先前任何一輪假設的「純快取命中=接近零成本」慢得多——這幾乎吃光鎖檔25分鐘陳舊門檻的全部預算，本輪在loading loop剛跑完、尚未印出後續回測結果時因逼近鎖檔上限被迫`taskkill`中止，腳本本身未產出`data/deep_dive_f_us_value_bm_leave_extreme_out.csv`，本輪判定為「未完成，非失敗」（程式碼邏輯本身正確，純粹是時間預算問題）。**下一輪待辦：直接重跑`python deep_dive_f_us_value_bm_leave_extreme_out.py`（程式碼不用改），但要抓緊在鎖檔前~20分鐘內完成收工序，或考慮先用`nohup`/背景process模式跑（同round350`f_us_value_bm`原始深挖用的模式）讓載入階段跨輪次持續，下一輪只讀結果。**`is_holdout_consumed()`開工/收工前皆確認`False`。零新增API呼叫（全程SEC company-facts本地快取讀取+FinMind價格快取讀取，未觸發任何新的外部請求）。詳見`US_MARATHON_STATE.md`/`US_LOG.md`第363輪記錄。

**上一輪（供對照）：第 362 輪 · 2026-09-05T08:01+08:00 · TW（讀取round359啟動的獨立樣本外驗證背景process結果，補齊`TRIALS_LEDGER.md`#118最後一個前提。monthly cadence獨立樣本alpha+4.55%但p=0.5647不顯著，對照round353原樣本p=0.0489，觸發round356腳本檔頭預先寫死的「p值遠高於0.05→選擇偏誤假說成立」判準，**最終判定FAIL**——確認`loo_no_low_vol`子版本先前的近似顯著結果是leave-one-out多重比較的假陽性，不是真實edge。已寫入`TRIALS_LEDGER.md`#131、`TW_LEADS.md`#13、`STRATEGY_GRAVEYARD.md`。`portfolio_multifactor_v2`探索線至此告一段落。`is_holdout_consumed()`確認`False`，零新增API呼叫。詳見`TW_MARATHON_STATE.md`/`TW_LOG.md`第362輪記錄）。
