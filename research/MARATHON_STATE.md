...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 415 輪（含補記的第405輪，見下方缺口說明）。最新一輪：第 415 輪 · 2026-09-06T23:00+08:00 · US（取鎖乾淨）**。**本輪工作單位**：依輪替原選TW，但查證發現TW軌round413下一步（#48質押比例cheap gate）已被獨立`hypothesis_queue`排程搶先做完並結案FAIL、`portfolio_multifactor_v2`家族round388已整併結案、`margin_debt_level_v1`round375-380已測完降級——TW軌無未消化下一步，改選US軌收成round414投遞的背景工作（job`20260906-223658-d62f`，`us_portfolio_multifactor_v1`）。讀取結果：TRAIN percentile=43.0（輸給隨機控制組中位數）、VAL percentile=60.0（遠不到90.0門檻），alpha p值兩期皆>0.76不顯著，**判定FAIL**——US軌第一個組合層級構造，MDD/beta合理、成功繞開短腿問題但未能證明alpha。已寫入`TRIALS_LEDGER.md`#179、`US_LEADS.md`#22、`US_MARATHON_STATE.md`第415輪記錄、`US_LOG.md`第415輪記錄。`is_holdout_consumed()`開工/收工前皆確認`False`。詳見`US_MARATHON_STATE.md`第415輪記錄。
