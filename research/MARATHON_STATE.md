...
**馬拉松全局輪次計數器（2026-08-23 新增，使用者要求）：目前累積 404 輪。最新一輪：第 404 輪 · 2026-09-06T17:00+08:00 · US（取鎖乾淨）**。**本輪工作單位**：三軌時間戳TW 16:30（round403，最新）／US 16:06（round402）／FUT 12:00（round399，依例外條款讓回TW/US）——選較舊的US。TW背景job`20260906-163407-6cc8`（`tw_deep_dive_value_pe_cost_sensitivity`）開工時仍`running`（27.1分鐘/30分鐘），heavy-job-slot佔用中，改做`US_MARATHON_STATE.md`round402「下一輪接手」建議(2)：對`f_us_value_bm`(#20)補做VAL期逐年分解＋leave-2022-out（`f_us_low_vol`已於#152做過，value_bm尚未做過），新增`deep_dive_f_us_value_bm_val_year_breakdown.py`（逐字比照#152方法論，單次真實回測非100 draws重跑）。**結果**：VAL期2021+92.02%/2022+81.51%/2023+235.78%/2024+198.34%四年全部為正，leave-2022-out+167.43%（同號、retained_fraction=1.18，不降反升）。判定**REFUTED（不是單一2022年份驅動）**，跟`f_us_low_vol`#152同一種模式，`#20`判定維持EXPERIMENTAL不變。已寫入`TRIALS_LEDGER.md`#167、`US_LEADS.md`#20更新。**附帶發現**：session過程中TW job`20260906-163407-6cc8`逾30分鐘timeout被砍（只跑完TRAIN 1x一組，6組×100draws的完整跑法需要遠超30分鐘），留給下一輪TW軌用更長timeout重投。`is_holdout_consumed()`開工/收工前皆確認`False`。零新增API呼叫，原地執行約2分鐘（非重度工作）。詳見`US_MARATHON_STATE.md`第404輪記錄、`TRIALS_LEDGER.md`#167、`US_LEADS.md`#20。
