...
- **SEC EDGAR Form 25 下市核實（2026-08-23 馬拉松第五輪已實測，見 `DATA.md`「美股存活者偏差調查（續）」小節、`US_LOG.md`、`research/sec_edgar_delisting_probe.py`）**：`TWTR`（25-NSE 2022-10-28）、`SIVB`（25-NSE 2023-05-02，另有 2017-2018 一組較早的申報，疑似不同證券類別下市，未分辨）確認為真下市，跟第四輪憑記憶的日期大致吻合。`BBBY` 現行 `company_tickers.json` 把 ticker 指向 CIK 1130713（公司名「NEIGHBORHOOD INTELLIGENCE, INC.」，不是 Bed Bath & Beyond）——**獨立確認代號重用是真的，且連 SEC 自己現行的 ticker 對照表都會被重用代號誤導，之後任何用「ticker→CIK/身分」單一對照表的設計都要考慮這個陷阱**。`SBNY` 這輪查證失敗——腳本裡手寫的備援 CIK（1288776）猜錯，查到的是 Google 的申報記錄，結果作廢，需要重查正確 CIK。`FRC` 查不到 Form 25（`filings.recent` 視窗 43筆全無，可能要查 `filings.files[]` 分頁或 FDIC 接管走不同申報機制，未解決）。**重要更正**：`sec_edgar_delisting_probe.py` 原本假設「手寫備援 CIK 比手寫日期安全」，`SBNY` 案例證明這個假設錯——任何手寫、未經查證的識別碼都有猜錯風險，不是 CIK 就比較安全。
- 美股成本模型：完全沒有（`validation/costs.py` 目前只有台股手續費/證交稅邏輯）。

**下一輪建議工作單位（只做其中一項，不要一次全做）：**
1. **修正 `sec_edgar_delisting_probe.py` 並重查 `SBNY`**：先用可信管道（例如公開新聞、或 SEC 全文搜尋 `efts.sec.gov/LATEST/search-index?q=...` 查公司名稱）查證 Signature Bank 正確的 CIK，不能再用手寫猜測值；順便修正腳本 docstring 裡「CIK 比日期安全」的錯誤假設。
2. **`FRC` 查 `filings.files[]` 分頁檔案**：`filings.recent` 視窗裡沒有 Form 25，需要抓分頁檔案內容才能確定是視窗覆蓋不到，還是 FDIC 接管清算真的走不同申報機制。
