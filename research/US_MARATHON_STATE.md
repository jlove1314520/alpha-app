...
- **XBRL company facts API（`data.sec.gov/api/xbrl/companyfacts/CIK{cik}.json`）已實測驗證，見 `DATA.md`「美股 PIT 資料源調查（續）」小節、`US_LOG.md` 第十輪、`research/sec_edgar_xbrl_facts_probe.py`。可用，但發現重要陷阱：這是「每個資料點」層級不是「每次申報」層級，同一財報期間（`end`）會因為比較期揭露在多次申報裡重複出現（AAPL 74個`end`日期裡71個/96%被超過一次申報引用），天真算 `filed-end` 會嚴重高估PIT gap（實測出現772/1034天離群值）。**如果之後要用這個API當美股版`pit.py`資料源，必須先按`end`分組取最小`filed`，不能直接用原始欄位值。**另外發現XBRL concept名稱隨時間/公司不穩定（`Revenues`只回溯到2016年，`EarningsPerShareDiluted`回溯到2007年，PLTR甚至沒有`Revenues`這個concept）。**修正後（取最小filed分組）的gap統計還沒重新算**，這是下一步。
...
3. **重算 XBRL company facts 的修正後 PIT gap**：對第十輪發現的「同一`end`取最小`filed`」邏輯實際寫出來，重跑 AAPL/MSFT/PLTR，看修正後的gap分布是否比submissions API（第三輪，33.1/27.5/41.5天平均）更精確或差不多；順便查一下`EarningsPerShareDiluted`以外1-2個常用concept是否也有同樣的比較期重複問題（目前只驗證過這一個concept）。
