...
重抓fetch_error：本輪執行時FinMind額度仍在封鎖窗口內（`_finmind_blocked()`
偵測到`blocked_until`未到），依規則不空等，直接跳過，**60檔全數列為
`unresolved`**，下面worst-case分析把它們（連同5檔no_data_found、13檔
price_too_short，共78檔）全部計入。額度解除後若要重試，直接重跑
`multibagger_five_questions_v2.py`即可（有quota檢查與提前中止保護，
不會意外撞額度）。

**2. worst-case敏感度分析**：

`[自行裁量]` window層級的Q5（下面舊表）無法對「完全沒有任何資料」的
78檔股票指定它們屬於哪個規模窗口（沒有liquidity觀測值），所以worst-case
分析改在**股票層級**做（同時也是四之「Q1改股票層級」需要的同一份底表，
見下方）：每檔ok股票取其所有窗口`avg_trading_money_20d_pre`的中位數當
規模代理，`has_moonshot`＝該股票觀察期內是否至少一次達標。78檔未解決的
delisted股票以phantom列加入，規模代理設為「比已觀察最小值再小1」
（確保落在全部5個累計分位門檻裡，模擬總司令原文「假設全部沒起飛且下市」
且刻意偏小以壓力測試「小型股最佳」這個結論），`has_moonshot=False`、
`is_delisted=1`、用delisted分層的population_weight。

先看**股票層級重算的原始版**（未加phantom，198/203檔有規模代理可用）：

| 規模代理分位 | 股票數 | 起飛率(加權) | 下市率(加權) | 比值 |
|---|---|---|---|---|
| 20% | 40 | 51.37% | 5.66% | 9.077 |
| 40% | 79 | 63.03% | 6.46% | **9.763（最佳）** |
| 60% | 119 | 67.18% | 7.41% | 9.061 |
| 80% | 158 | 70.72% | 7.59% | 9.319 |
| 100% | 198 | 70.36% | 7.96% | 8.839 |

**光是換成股票層級（還沒套worst-case），最佳分位就已經從window版的20%
移到40%**，且五個分位的比值差距很窄（8.84~9.76，全距約10%），不像window
版20%到100%有38%的單調遞減差距——這代表window層級「20%最佳」這個排名
本身就對「用哪把尺量」很敏感，不是穩健結論。

再套worst-case（78檔phantom加入）：

| 規模代理分位 | 股票數 | 起飛率(加權) | 下市率(加權) | 比值 |
|---|---|---|---|---|
| 20% | 78 | 0.00% | 100.00% | **0.000** |
| 40% | 111 | 32.68% | 39.02% | 0.838 |
| 60% | 166 | 54.30% | 20.54% | 2.643 |
| 80% | 221 | 62.43% | 16.82% | 3.712 |
| 100% | 276 | 64.63% | 15.46% | **4.181（最佳）** |

**最佳分位從40%翻轉到100%（全樣本），且原本被認為「最佳」的最小規模
分位在worst-case下比值直接歸零（0%起飛率、100%下市率）**——這正是
總司令原文branch 3的「翻轉」情境。

**3. 結論：明文撤回Q5「規模代理最小20%分位比值1.662最佳」這句話**。
改寫為：**「小型股規模門檻在下市資料補齊（60檔fetch_error重抓成功、
或另外設法取得no_data_found的5檔）之前，方向與效果量級都無法判定——
現有資料同時支持『小型股風險報酬比最好』與『小型股只是下市率被低估
所以看起來好』兩種相反的解讀，且股票層級重算後連window層級的排名本身
都不穩健，不是只有worst-case下才翻轉。」** 在此之前，Q5數字不得被引用
於任何規則設計或報告結論（總司令原文「這一項沒做完，不准有任何人引用
Q5的數字」——現在做完了，但答案是「不能判定」，不是一個可引用的門檻值）。
...
取代原本窗口層級（同一檔股票多個窗口重複貢獻分母）：

| 年 | 窗口層級(舊) | 股票層級(新) | | 年 | 窗口層級(舊) | 股票層級(新) |
|---|---|---|---|---|---|---|
| 2002 | 12.40% | 21.27% | | 2014 | 5.71% | 10.32% |
| 2003 | 10.04% | 16.14% | | 2015 | 1.13% | 2.23% |
| 2004 | 6.12% | 11.16% | | 2016 | 3.00% | 4.99% |
| 2005 | 3.63% | 7.07% | | 2017 | 4.61% | 8.15% |
| 2006 | 8.56% | 14.43% | | 2018 | 2.45% | 3.88% |
| 2007 | 15.58% | 26.05% | | 2019 | 1.83% | 3.59% |
| 2008（金融海嘯） | 0.13% | **0.21%** | | 2020 | 3.64% | 6.32% |
| 2009（海嘯後反彈） | 15.25% | 28.64% | | 2021 | 15.34% | **25.35%** |
| 2010 | 15.16% | 27.48% | | 2022 | 1.62% | 3.23% |
| 2011 | 2.96% | 5.48% | | 2023 | 5.97% | 10.29% |
| 2012 | 0.66% | 1.29% | | 2024 | 7.33% | 12.20% |
| 2013 | 5.21% | 9.95% | | | | |
...
形狀完全保留**——2008金融海嘯仍是全樣本最低（0.21%）、2009/2010海嘯後
反彈與2021資金行情仍是高峰（25~29%），Q1原本「飆股是強順週期現象」
這個結論不受這次修正影響，只是分母定義從「窗口」改成「股票」，數字
量級變了但形狀沒變。

### 本輪未做／已知限制（誠實揭露，供下一輪接手）

- ★一.1的60檔`fetch_error`重抓本輪因額度封鎖窗口未過而**沒有實際執行**，
  下市率被低估的問題**仍未解決**，worst-case分析是為了在資料補齊之前
  先回答「結論會不會翻」，不是資料補齊本身——資料補齊仍是待辦。
- worst-case分析用「比已觀察最小值再小1」當78檔phantom股票的規模代理，
  這是刻意的最壞情況假設（總司令原文明確要求「這是上界，不是估計值」），
  不代表這78檔股票真實規模就是最小，只代表在最悲觀假設下Q5結論站不住。
