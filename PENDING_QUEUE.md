...
- [x] **稽核二.二** **已完成（2026-09-15開發佇列cycle_id=20260915-171602，commit
  ...
  紅燈，違規773檔不變）。
- [~] **稽核二.三** **查證完成，需總司令裁示（2026-09-15開發佇列cycle_id=
...
- [ ] **P0產品.四** 全站一致性（骨架屏、每頁資料日期單一處、空狀態文案統一格式）——
  **部分完成，誠實不勾選**（2026-09-15開發佇列自走cycle_id=20260915-203102）。這條
  範圍是「全站」，本輪做了有界、可驗證的第一批，不是全部104處逐一改完：
  ①**骨架屏**（已做）：新增共用CSS（`.skel-line`/`.skel-block`置滾動shimmer動畫）
  ＋JS共用函式`skeletonHtml()`/`skeletonBlockHtml()`/`skeletonRowsHtml()`，套用在
  6個最高曝光的載入點——首頁自選股列表(`#wl-list`初始HTML)、首頁大盤速覽
  (`#home-idx-rows`初始HTML)、市場頁類股成分股清單(`sector-sheet-list`)、選股榜
  列表(`#picks-list`)、個股頁總覽（走勢圖/月營收圖/選股理由卡三個容器）、個股研究
  報告頁（`report-factors`等5個容器）。全站仍有其他約40餘處純文字「載入中…」未換
  （多數是低曝光的次要卡片，例如各籌碼子卡），留給後續輪次逐批處理，不在一次commit
  裡機械式全部替換（風險是來不及個別驗證每處骨架形狀跟實際內容形狀搭不搭）。
  ②**每頁資料日期單一處**（稽核結論：目前已符合，非本輪新增變更）：首頁已在
  P0產品.一合併成單一可展開狀態列；個股研究報告頁本來就只有`report-asof`一處；
  個股頁(`scr-stock`)每張卡各自標「來源：」是刻意設計——法人/融資融券/外資持股/
  借券/內部人交易/13F/8-K來自不同官方檔案、更新頻率各不相同，合併成一個日期反而
  是假造一致性、違反CLAUDE.md「每個關鍵欄位要有回退鏈/帶source標記」的資料誠實
  原則，本輪核實後判定不需要改。
  ③**空狀態文案統一格式**（部分完成）：定義了「尚未X｜原因｜下一步」這個典範格式
  （沿用自本輪P0產品.三新增「事件」分頁的空狀態），但全站既有約40+處空狀態文案
  多數已經包含「缺什麼＋為什麼＋能做什麼」的內容，只是標點/順序不統一，機械式
  一次性重寫全部有改壞既有精確措辭的風險（例如把「查無報價（原因X）」硬套進
  pipe格式可能扭曲原意），本輪判斷不值得冒這個風險換一個純格式上的一致性，留給
  後續有更多上下文時逐條人工核對再改。**驗證**：`node scripts/smoke_test.mjs`
  49/50 PASS（僅#39既有已知紅燈，理由同前兩則commit）；另跑臨時Playwright腳本
  （驗完即刪）確認骨架屏在初次載入時真的可見、資料到位後正確被真實內容取代、
  不會卡在骨架狀態，全程無新增console錯誤（既有的FinMind CORS錯誤是測試環境
  對外連線限制造成，與本次改動無關，正式`smoke_test.mjs`用fixture模擬不受影響）。
