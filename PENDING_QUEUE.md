...
- [!] **金流一.4** **阻塞：等資料累積**——目前法人歷史 **14 個交易日**（20260901~20260922），可用視窗 [1, 5]；**20 日視窗還需 6 個交易日**、**60 日視窗還需 46 個交易日**。
...
- [x] **審.二** [研究] f52w用資訊比率重審——**已完成（2026-09-23）**：
  ...
  等總司令裁示。
- [!] **資料.一** [研究] BLOCKED（FinMind額度/封鎖，預計解除
  2026-09-23 22:44台北時間）——f52w樣本延伸到2007(核准執行)，天條一
  須涵蓋2008，f52w價值主要在低回撤，沒2008無從判定。**進度**：
  `f52w_2007_extension.py`已執行，樣本池300檔（沿用`SAMPLE_SEED=
  20260822`同一組——`universe.universe()`本身已是2003年起survivorship
  -bias-mitigated，跟日期範圍無關，延伸不需要重新抽樣，滿足「延伸樣本
  須含2007-2008年間下市股票」這條要求）。抓取到42/300檔時命中FinMind
  402額度上限，**依裁示「遇402停斷點續跑不得繞過」，已正確停下**，
  未執行任何回測(資料不完整、跑出來的數字沒有意義)。已達成34檔可用
  (另6檔資料不足或延伸期間尚未上市)。checkpoint機制
  (`data/f52w_2007_extension_checkpoint.json`)已存已完成部分，重跑
  本腳本會用`finmind_client.py`的parquet快取跳過已抓到的，從斷點續跑。
...
- [ ] **驗.一第4點續（剩餘8支）** [研究] 尺.二已完成，前置條件滿足，
  **2026-09-23裁示【f52w結案＋研究策略轉向「新資料單發檢定」＋佇列
  清理】清理.一第4點：現在開跑**，一律用新引擎+新量尺+修正後DSR。
  與資料.一並行。(16支中已完成/進行中8支，見上方馬拉松軌進度)。
...
- [x] **結案.三** [債務] f52w（#385）FAIL結案——**已完成**：
  ...
  註記。
- [ ] **轉向.一** [研究] 新資料單發檢定——待第1點日期查證完成後才能
  進行第2點事前登記。1.查證f52w(#84/#85/#86/#370/#377/#378/#385)與
  dividend(#75/#371/#376)所有相關試驗實際用過的最早日期，切分
  2007-2014「完全未使用」vs「因子層已看過」兩段，**先做先回報**。
  2.事前登記(寫進TRIALS_LEDGER，登記後才可執行)：候選=f52w_high_
  portfolio_v1+dividend_yield_portfolio_v1(參數沿用現行版本不得調整)，
  期間=資料.一補齊後的2007-01-01~2014-12-31，主判準(二者都要過)：
  ①對0050含息總報酬年化IR>0單尾p<0.025(2候選Bonferroni，Newey-West+
  Dimson beta) ②2008年策略MDD優於0050同期MDD且全期MDD>-50%(天條一)；
  分段另報「完全未使用段」同樣指標供參考不作主判準。3.只跑一次，結果
  無論好壞照登記判定，不得換參數重跑。4.兩者都通過才另外提案是否動用
  holdout(2025-01起)，需總司令個別核准不得自行呼叫。待資料.一補齊後
  才能執行第2點以後。
