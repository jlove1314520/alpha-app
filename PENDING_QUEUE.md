...
- [ ] **資料源一.2** [債務] 財報改 SEC EDGAR XBRL companyfacts／frames，以申報日對齊 PIT
- [ ] **資料源一.3** [債務] 價格改 `yf_price_client.py` 為主＋查證 Stooq 為第二來源，回報兩來源收盤差異率
- [ ] **資料源一.4** [債務] 4 個美股因子在新宇宙全部重跑，回報判定是否改變；重跑前既有美股結論一律標「宇宙待驗證」
...
- [ ] **外部一改.3** [研究] 期貨軌：海龜／Donchian 完整版／Keltner／波動度突破／CTA 多時間框架，台指期為主
- [ ] **外部一改.4** [研究] 三軌試驗全部 register，回報各軌新 N 與新門檻，跨軌同概念依同家族規則
- [ ] **外部二改** [研究] 抄新策略首批 8 條（美股 3／期貨 3／台股 2），先寫 SPEC 再實作
- [ ] **外部三.1** [債務] CLAUDE.md 寫入外部策略匯入紀律（每條計入 N、單批上限 8 條）
- [ ] **外部三.2** [債務] `docs/EXTERNAL_STRATEGY_SOURCES.md`＋signal_status 標註來源與公開後衰減風險
...
- [ ] **深讀四.1** score_longshort_v1 依關卡 7 重評並更新 LEADS.md，不再掛 PENDING
- [ ] **深讀四.2** 放空腿硬規則：借券成本與可借量未接入前，含放空的回測一律標「資料缺陷，不得採信」
- [ ] **深讀四.3** 新機制優先連續曝險縮放；控制組須為「同縮放幅度、訊號內容無意義」版本
- [ ] **深讀四.4** data_audit.py 自查：有幾道恆等式是「稽核內部重播引擎」形狀，全部判無效重寫
