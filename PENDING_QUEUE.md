...
- [ ] **零件.一** [研究] 零件清單與事後歸因（描述性，不產生新試驗）——
  建立`research/COMPONENT_INVENTORY.md`，把既有全部結果（`TRIALS_
  LEDGER.md`、portfolio v2的12組、regime5個候選+FUT網格、`CORE_TILT_
  SPEC.md`的規劃構造）拆解成正交零件，至少涵蓋八個維度：訊號源／換倉
  頻率／持股檔數／加權方式／資格池篩選／regime overlay／停損規則／
  產業與beta約束。**不跑新回測**，只重新解讀既有結果：每一筆試驗標註
  用了哪些零件、以及（若記錄得到）死在哪一個零件上。**分支**：某零件
  出現次數<3→標「證據不足」，不納入階段二；出現≥3次→進階段二。互動
  視窗CC正在執行中（進行中，見下方進度）。
- [ ] **零件.二** [研究] [依賴零件.一輸出，需先確認哪些零件≥3次證據
  才能決定要隨機抽樣哪些維度組合] 穩健性地圖（跑新東西，但輸出是分布
  不是贏家）——組合空間裡隨機抽樣（不窮舉）約200~300組，全程只用
  TRAIN期。對每個零件計算：出現vs不出現時績效差的分布（中位數/IQR/
  正貢獻比例/在幾成組合裡為正）。輸出`research/COMPONENT_ROBUSTNESS_
  MAP.md`。**硬性紀律**：①禁止報告「最佳組合」是哪一個，也不准寫進
  任何檔案，只報零件層級的貢獻分布；②200~300組全部計入`selection_
  bias_ledger.py`的N，不得規避；③VAL期完全不碰，holdout更不准碰。
  **分支**：有零件「正貢獻比例≥70%且中位數超過該換倉頻率的損益兩平
  線」→列為穩健零件，進階段三；全部零件都不符合→誠實回報「零件層級
  找不到穩健正貢獻」，不為了有東西交而放寬門檻。
- [!] **零件.三** [研究] BLOCKED：少數預先登記的組合（待零件.二結果
  出來再開工，`[自行裁量]`標為BLOCKED而非`- [ ]`——總司令原文雖寫
  `- [ ]`格式，但內文明確要求「待階段二結果出來再開工」，這是真實
  的執行順序依賴，不是可以現在動手的項目，比照本檔案其他「依賴前一
  階段」項目的既有慣例標BLOCKED，解除條件＝零件.二完成）。用穩健零件
  組出5~8個有理論理由的組合，每一個事前把規格寫死（沿用「看結果前
  先commit規格」的既有做法），逐一走完整六關＋牛熊分制度表＋損益
  兩平檢定力前置關卡。不得超過8個，想測第9個要先回報。
- [ ] **重構.減資** [研究] [自走補入，來源：`research/HYPOTHESIS_QUEUE.md`
...
- [ ] **FUT.basis均值回歸regime複驗** [研究] [自走補入，來源：`research/
...
- [ ] **零股失衡度.連續曝險版重測** [研究] [自走補入，來源：`research/
