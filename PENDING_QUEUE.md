...
- [ ] **驗.一** [研究] 回測引擎健全性——**2026-09-23總司令裁示【稽核
...
- [ ] **尺.一** [研究] 組合層alpha／基準量尺修正（最優先，家族共用）——
  `portfolio_backtest_v2.py`的`buy_and_hold_index_pct()`/`alpha_
  significance()`用TAIEX(價格指數不含息)當基準，策略端卻是含息
  adj_close，系統性高估超額報酬約「殖利率x beta」；OLS標準誤未處理
  自我相關/異質變異，也未修正非同步交易造成的beta低估。1.基準改0050
  含息總報酬(沿用`load_0050_full_history()`)，舊行為保留為
  `benchmark="taiex_price_legacy"`供重現舊結果。2.alpha回歸：標準誤
  改Newey-West(HAC,lag=5)；beta改報Dimson beta(大盤報酬lag0/1/2三項
  係數加總)，alpha用Dimson版本算，普通OLS beta並列對照；另報「平均
  投入比例」(持股市值/總權益)。3.自我測試：0050對0050回歸alpha≈0、
  beta≈1；0050報酬落後1天的序列，Dimson beta≈1、OLS beta明顯<1。
  4.稽核前5支若有存equity_curve直接用新量尺重算不必重跑回測，輸出
  三欄對照(舊引擎+舊量尺/新引擎+舊量尺/新引擎+新量尺)。自我測試與
  三欄對照表一出來就回報。
- [ ] **審.一** [研究] f52w(#85/#370)補完審查，AWAITING_REVIEW暫不
  核准——待`尺.一`完成後開始。1.新引擎重跑#86(第3/5/6關，尤其逐年
  一致性)。2.用新量尺重算TRAIN/VAL的alpha/p值/beta，報酬直接對比
  0050含息總報酬。3.VAL報酬歸因：列貢獻最大10檔+產業，算半導體/電子
  類報酬佔比；另跑「產業中性」(每產業最多N/4檔)對照版本。4.以目前
  N_valid算DSR，並列Bonferroni結果，事前宣告DSR未過不得核准，不因p
  值好看放寬。5.登記缺口：樣本2015起不含2008，天條一無法判定；只
  提案「延伸樣本到2007」的資料成本與時間，不執行。6.全部完成後才回
  AWAITING_REVIEW，附一頁摘要供裁示。
- [ ] **驗.一第4點續（剩餘16支）** [研究] 待`尺.一`完成後續跑，一律
  用新引擎+新量尺，避免跑兩次。`dividend_yield`(p=0.0511)與
  `portfolio_v2`(p=0.0514)這兩個「差一點」案例用新量尺重算即可，不得
  因接近門檻升級或降級，照數字判。
