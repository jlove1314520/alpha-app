...
- **一（家族結案）** ✅：`STRATEGY_GRAVEYARD.md`新增「機制類別結案：門檻觸發式
  ...
  再新增第六個門檻式二元降曝險變體。
- **二（換機制形式）** ✅已排入佇列：`regime.替代A`（連續型曝險調節）、
  `regime.替代B`（regime用在選股權重）已完整登記為`- [ ]`項目，含完整規格
  鎖定要求、必報表、分支邏輯。**進度更新**：`regime.替代A`已被馬拉松自走
  輪次取走，規格已鎖定寫進`REGIME_OVERLAY_PROTOCOL.md`（原寫第15節，跟
  互動視窗CC同時鎖定的FUT網格章節撞號，已改號為第16節），成本前置關卡已算
  （連續版成本約二元版53%但保護力度先天較小），尚未跑TRAIN判定，留給下一
  個驗證帽輪次。
...
- **五（佇列深度）** ✅規則已改：門檻`- [ ]`（不含`- [!]`）低於12項時自行
  補件到20項，同步更新`PENDING_QUEUE.md`前言規則第3點與`dev_queue_runner.py`
  提示詞模板。**[自行裁量]計數基準明確化**：只算真正可動手的`- [ ]`，不含
  `- [!]`（詳見規則本文的理由說明）。**補件執行中**：目前`- [ ]`4項
  （`regime.替代A`/`regime.替代B`/`深讀一.2`/`金流一.5`），正依優先序
  （常備backlog→REPORT.md/LEADS.md/STRATEGY_GRAVEYARD.md→HYPOTHESIS_
  QUEUE.md）尋找更多候選補到20項，找到的候選會標「[自走補入]」+來源，
  過程與結果另行補記於本節。
