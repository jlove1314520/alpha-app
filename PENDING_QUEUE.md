...
- **一.1** 🔲 進行中：建立「不同持有期打平0050所需毛alpha」計算表，
  t5/t20/t60/t120 × 手續費折扣情境，含0.3%證交稅。本輪進行。
- **一.2** ⛔ 阻塞（客觀前提未滿足，非拖延）：查證`picks_ledger.json`
  現況，`t20`回填率**0.0%（0/940）**、`t60`/`t120`同樣0.0%，只有`t5`
  28.2%（265/940）有資料——**t20目前完全沒有可用資料，無法切換主評估
  窗口**，維持`t5`為現行顯示窗口，待t20累積出有意義樣本後再議，不在
  此輪動作。
- **一.3** 🔲 進行中：本輪一併把「SPEC階段先估周轉率與成本、吃掉毛報酬
  >50%不進gate1」寫進研究紀律文件。
- **二** 🔲 待辦：明天（2026-09-16）17:00台北排程跑過後，確認
  `sparklines.json`的`generated_at`是否跳離09-05，並重跑`data_audit.py`
  回報三個數字（765筆／32.95%／3,972筆的新值）。**在此之前不得回報完成**，
  沿用前一則【sparklines解凍】條目二.1/二.2的既有阻塞狀態，本條不重複
  另開，只更新「明天」這個具體時間點供下一輪session查閱。
