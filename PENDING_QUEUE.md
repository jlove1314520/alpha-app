...
- **成本.一** ✅已完成：`research/validation/breakeven_alpha_table.py`
  ...
  假設差異。
- **成本.二** 🔄進行中（另起research agent搜尋比對，結果待補記）：
  已確認`regime_overlay_*.py`整個家族的`COST_PER_UNIT_EXPOSURE_CHANGE`
  常數寫死用`commission_discount=1.0`（無折扣），比新的1.8折更高估，
  且不是簡單線性可換算（手續費部分隨折扣線性縮小，但稅與滑價不隨折扣
  變動，實際成本比值介於1.0~1.5倍而非1.7~2.9倍，且成本對MDD這種非
  線性指標的影響需要真的重跑才知道，不能用比例推算），完整清查與
  逐條回報待agent結果。
- **成本.三** ✅已完成：`research/power_budget.py`拆成獨立的
...
- [!] **成本.二** [債務] BLOCKED（互動視窗CC，2026-09-19，另起research
  agent搜尋比對中）——git log -S與TRIALS_LEDGER清查所有因成本/損益
  兩平線論證被判FAIL或擋掉的項目。**已知線索**：`regime_overlay_*.py`
  整個家族的`COST_PER_UNIT_EXPOSURE_CHANGE`常數寫死`commission_
  discount=1.0`（無折扣，比新的1.8折更高估），但手續費/稅/滑價三個
  成分裡只有手續費隨折扣線性縮小，稅與滑價不隨折扣變動，實際新舊
  成本比值約1.0~1.5倍（不是總司令原文提到的1.7~2.9倍那個範圍，那個
  倍數是純手續費部分的比例，不是含稅含滑價的完整比較），且成本對MDD
  這類非線性指標的影響**必須真的重跑回測才知道，不能用比例推算**。
  **預計解除時間**：等research agent回報完整清查名單後，本輪或下一輪
  互動視窗CC逐條判斷是否需要重跑；`power_budget.py`六組季頻UNDERPOWERED_
  BLOCK構造可直接用已有的`realized_alpha_ann_pct`/`mde_80pct_power_
  alpha_pct`數字套新兩道關卡計算（不需重跑回測），這部分待`成本.三`
  的驗證跑完後一併回報。
- [!] **分群.一** [研究] BLOCKED：等`[自行裁量]`具體化「既有已跑過的
  策略」這個指代後才能開工——總司令原文的例子（「2330用策略A、2317用
  策略B」）暗示這是針對**個股層級**的策略異質性問題，但本專案絕大多數
  已跑過的試驗是**投資組合層級**（買一籃子股票），不是「每一檔股票
  各自配一條策略」，需要先決定用哪個資料集/哪個粒度做組間vs組內變異數
  分解才能真正動工。**候選方案（供下一輪執行者參考，非最終裁定）**：
  (a) 用`portfolio_multifactor_v2`或既有因子的**個股層級IC貢獻**
  （例如`f_low_vol`/`eps_family`對每一檔股票個別的橫斷面貢獻）當
  應變量，按分群維度（流動性/波動度/市值/產業分位）分組做ANOVA；
  (b) 用`multibagger`起飛股歸因分析（重構.B系列）裡已經有的
  `avg_trading_money_20d_pre`等特徵做類似的變異數分解，但那個資料集
  問的是「哪些股票會起飛」不是「哪個策略在哪群股票上表現好」，語意
  不完全對應，需要調整問題定義；(c) 等更多single-instrument類型的
  試驗（如借券費率/零股失衡度這類針對特定訊號的候選）累積更多樣本後
  再做。**分群維度已事前鎖定（總司令原文，不得事後增減）**：20日均
  成交金額分位／60日波動分位／市值分位／產業大類／（可選）法人持股
  比例。**判準（總司令原文）**：群間變異數 vs 群內變異數，群間>>群內
  →分群有結構（群數上限5，各群策略選擇需登記進多重比較帳）；群間≈
  群內→商品差異是雜訊，不准做分群策略，誠實記錄結案。**這一題先做，
  沒過就不准往下做任何「針對單一商品」的東西**——這條限制即刻生效，
  在分群.一做出結論前，任何新交辦都不應該包含「這檔股票用這個策略」
  這種代號配對式的設計。
...
- [!] **原子.二** [研究] BLOCKED：等總司令審閱`原子.一`的原子與算子
  清單（`ATOM_LIBRARY.md`）後裁示是否可以繼續——這是總司令原文明確
  要求的審閱關卡（「這一關我要看，因為原子定義錯了後面全錯」），不是
  可還原的技術選擇，不適用`[自行裁量]`繼續做的原則。depth-1表達式
  （算子(原子,n)）IC地圖，描述性掃描，事前算出空間大小並登記，全部
  計入`selection_bias_ledger.py`的N。禁止在這階段報告「最佳素材」或
...
- [!] **原子.三** [研究] BLOCKED：依賴原子.二完成（間接依賴原子.一
  ...
  最重要**：任一存活表達式必須寫出一句「為什麼這會有效」的經濟機制，
  寫不出來就砍，不管IC多漂亮（人工判斷不是統計，不得自動化）。判定
  看贏家vs同搜尋空間隨機表達式分布的上尾，不看絕對t值；同時套用DSR。
  上限5個表達式進VAL驗證，超過要先回報。
...
- [x] **零股失衡度.連續曝險版重測** [研究] ✅結案（馬拉松自走，研究帽，2026-09-19：檢定力前置關卡STATISTICAL_GATE_FAIL，依協定1a-0b修正4'不准開跑，未跑回測、未登記試驗；MDE 9.9~15.8% vs 鎖定目標2.89%，詳見`research/ODD_LOT_CONTINUOUS_POWER_PRECHECK.md`，含解除條件與`[自行裁量]`） [自走補入，來源：`research/
  ...
  解讀需附此但書。
- [ ] **資料源.fx_twd_gate統一改央行源** [研究] [自走補入，優先序最低，
...
- [!] **regime.替代B** [研究] regime用在選股權重而非總曝險——總曝險
  ...
  **BLOCKED原因**：載體`core_tilt_backtest.py`未實作（`重構.C4`等總司令核准市值
  重建方案）；`[自行裁量]`不以v2等權構造頂替（載體差異失效、與v2不可比）。
  **解除條件**：`重構.C4`解除並完成首跑後，由驗證帽輪次跑規格第18節、登記4筆試驗、
  跑完才可改`- [x]`。
...
- [ ] **金流一.5** [研究] [BLOCKED分流.已解除，2026-09-19總司令裁示【裁示】四查核]
