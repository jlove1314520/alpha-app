...
- [!] **稽核.三(a)** 季報回補續跑（38.44%→目標0）。**分支**：FinMind
  ...
  　**2026-09-20 01:20 馬拉松自走（維運帽，交辦佇列無`- [ ]`時檢查BLOCKED解除）**：`rate_limit_state.json`的`blocked_until`（台北00:56）已過，接續回補：job`20260920-010131-3b3e`（179檔，income/balance各179成功、err=0）→`build_stock_financials_history.py`併入（22檔補進更多季度）；再投遞job`20260920-011056-c32f`（剩14檔，14/14成功）→併入後`find_gap_codes()`**350→179→14→8檔**。剩8檔（2237/2758/2760/6565/6812/6911/6947/7752）`2025-01-01__latest`快取皆已有資料（最新期2026-06-30），再打也不會多出東西，**推測**為結構性缺季（如上市前無申報，未逐檔驗證），非額度或程式問題。`data/audit_report.json`的`completeness_gap_rate`（0.38359）是舊值，須等audit.yml下次重算才知新數字，故本項維持`- [!]`：阻塞原因＝等audit重算確認缺口率，解除後若仍>0且原因是結構性缺季則改標完成並註明殘餘8檔。本輪未動`data/audit_report.json`（他人未提交的改動）。
