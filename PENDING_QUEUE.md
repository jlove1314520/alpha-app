...
- [!] **資料.一** [研究] BLOCKED（FinMind額度/封鎖，預計解除
  2026-09-24 01:00台北時間）——f52w樣本延伸到2007(核准執行)，天條一
  須涵蓋2008，f52w價值主要在低回撤，沒2008無從判定。**進度（第2輪，
  2026-09-23 22:47~22:53）**：FinMind第一次封鎖解除後重跑
  `f52w_2007_extension.py`，從斷點(42/300)續跑到89/300檔（累計約73檔
  可用，另15檔資料不足/延伸期間尚未上市），**再次命中402額度上限，
  依裁示正確停在斷點**，未執行任何回測。**誠實揭露進度速度**：兩輪
  合計約89/300檔、耗時約2小時（含等待封鎖解除），**以這個速度完成
  全部300檔預估還需要4-5輪次、約8-10小時**——FinMind的每小時額度對
  300檔延伸抓取(每檔含價格+股利+多個財務因子共約13-15次API呼叫)
  是硬性瓶頸，不是可以加快的部分。checkpoint機制(`data/f52w_2007_
  extension_checkpoint.json`)已存已完成部分，重跑本腳本會從斷點續跑，
  不重複已完成的。**解除條件**：`data/rate_limit_state.json`
  `blocked_until`=2026-09-24T01:00台北時間，屆時重跑
  `python research/f52w_2007_extension.py`即可續跑剩餘約211檔。
