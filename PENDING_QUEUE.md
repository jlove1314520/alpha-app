...
- [ ] **regime.替代A** [研究] 連續型曝險調節取代二元門檻——曝險=f(訊號
  ...
  **分支**：上檔捕捉>85%且淨MDD改善→進深挖；上檔捕捉仍<75%→連續形式
  ...
  形式（除非總司令另行核准換函數形式本身）。
  **進度（馬拉松自走，2026-09-19T12:03+08:00）**：規格已鎖定寫進`research/REGIME_OVERLAY_PROTOCOL.md`第15節
...
- [x] **regime.FUT提案** [研究] ✅提案已寫完（馬拉松自走，2026-09-19，依指示寫完就停、未執行新一輪測試）：`research/PROPOSAL_2026-09-19_fut_regime_overlay_bear035.md`，**等總司令裁示 (A)核准以0.35重測／(B)不重測結案／(C)其他**；提案已誠實標出主要風險（0.35為看過#244結果後選定＋上檔捕捉率很可能跌破75%）。原文如下： [自走補入]（來源：`research/
...
- [!] **金流一.4** **阻塞：等資料累積**——目前法人歷史 **12 個交易日**（20260901~20260918），可用視窗 [1, 5]；**20 日視窗還需 8 個交易日**、**60 日視窗還需 48 個交易日**。
