...
- [!] **資料.一** [研究] BLOCKED（FinMind額度/封鎖，預計解除
  2026-09-24 02:05台北時間）——f52w樣本延伸到2007(核准執行)，天條一
  須涵蓋2008，f52w價值主要在低回撤，沒2008無從判定。**進度（第3輪，
  2026-09-24 00:02~00:29）**：132/300檔已嘗試（約108檔可用，24檔
  資料不足/尚未上市），再次命中402額度上限，依裁示正確停在斷點，
  未執行任何回測。**速度**：三輪累計132/300，約每輪40-45檔（受
  FinMind每小時額度硬性限制）；以此速度預估**還需3-4輪、約6-8小時**
  才能補齊剩餘168檔。checkpoint持續累積(`data/f52w_2007_extension_
  checkpoint.json`)，下次解除後重跑腳本自動從斷點續跑。**解除條件**：
  `data/rate_limit_state.json`的`blocked_until`=2026-09-24T02:05
  台北時間。
...
- [ ] **轉向.一續** [研究] 候選名單定案——待審.三第3點成本估算回報、總司令
  ...
  **審.三已完成，回報等候選名單裁決（2026-09-24）**：value_board的DSR證據
  ...
  屬於總司令裁決範疇，互動視窗CC不代為判斷，僅誠實提供數字。**繼續
  BLOCKED於總司令裁示；資料.一在此之前依裁示原文僅能做資料品質檢查，
  不得回測**。
...
- [ ] **定案.一** [研究] 轉向.一最終候選名單——value_board_v2不列入
