...
- [ ] **驗.一** [研究] 回測引擎健全性——S1(max_positions=1單押0050)/
  S2(全體等權重月頻)/S3(隨機8檔100 seed)/S4(逐項加回成本/停損/產業
  上限/下市股)，S1-S3任一不過先修引擎不得往下做；修正複利(compounding
  旗標，新預設True，舊行為False)；下市股mark改最後有效價格+報殭屍
  部位數量(處理規則另外提案不自行選定)；稽核所有用run_backtest且涉及
  0050/TAIEX/買進持有比較的TRIALS_LEDGER列，重跑列「舊判定/新判定」，
  翻轉寫AWAITING_REVIEW不自行改判，純隨機對照組百分位關卡另列「影響
  較小」欄但仍要重跑。**#358改記FRAMEWORK_CHECK_FAILED**（撤銷「框架
  跑得動」結論）。S1-S3結果一出來即回報，不等稽核跑完。
- [ ] **驗.二** [研究] spillover前視偏誤——#346判定改FAIL(前視偏誤)，
  AWAITING_REVIEW結案；改用t日開盤→收盤報酬重跑全部9關(0050開盤價或
  ^TWII Open，ETF稅率)，登記新試驗；#88 cheap gate同樣改開盤到收盤，
  另列「開盤跳空vs美股報酬」r當對照量化外溢在跳空裡的比例。
