...
- [ ] **App 兩個 UX 問題排進今晚（線圖不顯示 + 數字不會跳動）**——**已派給
  獨立background agent處理中（2026-09-02 00:3x），尚未收到完成通知**。
  給agent的補充提醒（已包含在派工指示裡）：`.rise`/`.grow`這兩個既有
  CSS class其實不是漲跌閃爍動畫（分別是淡入進場/寬度成長動畫），已提醒
  不要硬套，改新增專用的flash動畫。
