...
### 【產業地圖】[產品] （即本檔第 1069 行「零之三」，2026-09-04 登記至今未執行）

1. 先查 `ic.tpex.org.tw` 的 robots.txt 與使用條款，確認可讀；
   **不可則立即停止並回報，不繞。**
2. 抓櫃買中心「產業價值鏈資訊平台」每條產業鏈頁面（形式為
   `/introduce.php?ic=<代碼>`，約 30+ 條鏈，代碼如 D000 半導體、C100 生技製藥）。
   無 API 無批次下載，只能讀公開頁：一次抓完後每月更新一次即可，
   **間隔 ≥3 秒/頁**，遵守 CLAUDE.md 頻率清單。
3. 產出 `data/industry_chain.json`：每條鏈的名稱、上/中/下游分段、
   各段成員公司代號與名稱。台股代號對不上的（興櫃、未上市）保留但標記，
   **不混進上市櫃統計**。
4. 個股頁「所屬產業」改顯示：產業別 ＋ 價值鏈位置（上/中/下游）＋
   同鏈同段公司數，並可點開看同段公司清單。
5. 覆蓋率誠實揭露：回報全市場有多少檔對得上價值鏈位置、多少檔對不上，
   對不上的顯示「未收錄於產業價值鏈平台」**而非留空**。
