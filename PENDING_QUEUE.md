...
2. **阻塞≠停止**：某一項標`- [!]`BLOCKED後，同一輪要立刻換下一項
   ...
   解除就轉回`- [ ]`。
3. **佇列深度下限5項**：`- [ ]`加`- [!]`的未完成項低於5項時，執行者
   有責任自己補（來源：本檔「常備backlog」區塊→`REPORT.md`/
   `LEADS.md`/`STRATEGY_GRAVEYARD.md`裡的未進佇列待辦→
   `HYPOTHESIS_QUEUE.md`排隊中的假設），補入標「[自走補入]」+來源。
...
執行者發現`- [ ]`+`- [!]`低於5項時，先看這裡有沒有可以直接排進佇列的
候選；沒有才往`REPORT.md`/`LEADS.md`/`STRATEGY_GRAVEYARD.md`/
`HYPOTHESIS_QUEUE.md`找。**目前是空的**（本輪的六大類任務已直接排進
下方機器索引，不需要從這裡取用）——下一次佇列見底、且下面三個備援
來源也補不出東西時，才會真的用到白名單第7條「佇列真的空了」。
...
- [ ] **重構.A2** [研究] `research/HYPOTHESIS_QUEUE.md`「#74續」章節裡
  ...
  **分支（2026-09-18【改為連續自走】新增，原指令不變）**：理論值與
  實測接近→GATE6沒bug只是嚴格，寫成提案停下等裁示（屬白名單第5條，
  可以停）；實測遠低於理論→往去均值化查，查到根因就修量測腳本（不是
  關卡），修完重跑，這段不用請示。
...
- [ ] **重構.C4** [研究] `core_tilt_backtest.py`實作與首跑——依SPEC v3
  ...
  BLOCKED回報。**[自行裁量]前置決定（C2遺留的兩個開放問題，本輪
  一併決定，不再等總司令核准）**：金融股與面額比值偏離±15%以上的
  個股，實作時直接排除出資格池（不納入基底市值加權，也不納入模型版
  「0050」proxy的候選）——理由是C2已確認這兩類的重建市值不可信，
  排除比硬用已知有問題的數字更保守安全，若總司令認為應該用別的處理
  方式（例如金融股改用總權益、或另訂exclusion門檻），下一輪讀到這
  裡可以推翻。
- [ ] **重構.B2** [研究] 飆股歸因打散順序版重跑+五題——FinMind額度可用
  時投遞打散處理順序的150+150（分層抽樣正式規模，見續9章節已完成的
  `stratified_sample()`程式碼），收成後答五題。**分支**：delisted
...
- [ ] **稽核.五** e_pe 19.5%不一致查證——抽10檔（含1506/2329），列
  我方近四季EPS四個季度值與來源日期、官方PER計算基準日，判斷是
  「基準不同」還是「我們EPS拼錯」。**分支**：基準不同→寫進audit
  說明，收工；我們拼錯→回報受影響比例後**繼續修**，修完重跑稽核。
- [ ] **稽核.三(a)** 季報回補續跑（38.44%→目標0）。**分支**：FinMind
  額度擋住→標BLOCKED附解除時間，換下一項；解除後自動接續，不需請示。
- [ ] **稽核.四.3** 相對強弱日期對齊（market_tw taiex補
  sparkline_dates、`_relative_strength`改日期交集對齊、對不齊回None
  寫missing_factor_notes）。**分支**：全程自走。回報修正前後
  relative_strength有值檔數與前50名變動幾檔。
- [ ] **實測.八** [產品] App既有bug——**分支**：全程自走，修完附一行
  「怎麼驗證的」。
- [ ] **實測.九** [產品] App既有bug——**分支**：全程自走，修完附一行
  「怎麼驗證的」。
- [ ] **實測.十** [產品] App既有bug——**分支**：全程自走，修完附一行
  「怎麼驗證的」。**⚠️這三項原文只有編號沒有具體內容**（總司令原話
  「App三個既有bug」未附細節），下一輪執行者需要先找出這三個bug的
  具體描述（可能在對話記憶外的地方，例如`PENDING_QUEUE.md`更早的
  章節、`BACKLOG.md`，或需要向總司令確認），**找不到具體內容視為
  「白名單第7條佇列真空但補件規則補不出東西」的特殊情況，可以標
  BLOCKED附「缺具體bug描述」並換下一項，不算違反連續自走規則**。
- [ ] **重構.D2** [研究] Cybex #55（三大法人買賣超廣度速度）、#57
  （當沖比重速度）——兩條都跑完才可以對「橫斷面離散度速度（台股）」
  這個家族結案。**分支**：任一條通過→進深挖清單；兩條都FAIL→家族
  結案寫進`STRATEGY_GRAVEYARD.md`。**不得因前面0勝3敗提前判死。**
...
- [!] **金流一.4** **阻塞：等資料累積**——目前法人歷史 **11 個交易日**（20260901~20260917），可用視窗 [1, 5]；**20 日視窗還需 9 個交易日**、**60 日視窗還需 49 個交易日**。
...
- [!] **稽核.五** 其餘佇列照序——**部分完成，部分阻塞（2026-09-15開發佇列
  ...
  2. **四/新四（本地摘要）GPU/記憶體確認已完成**：`nvidia-smi`實測本機
     ...
     供總司令參考，非CC自行決定）。
- [!] **稽核.六（a_price_source根因與分佈，總司令2026-09-15交辦，只報不修）**
