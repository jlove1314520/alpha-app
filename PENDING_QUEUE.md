...
- [x] **重構.B3** ✅已完成（互動視窗CC自走，2026-09-19，全程自走不逐項
  ...
  **執行狀態**：四項依序完成，新增`research/multibagger_five_questions_v2.py`
  ...
  - **★一**：78檔未納入delisted分類完畢（fetch_error 60／no_data_found
    ...
    封鎖跳過，額度解除後補跑結果見下方「本輪未做」段落的最新狀態；
    worst-case敏感度分析（不受重抓結果影響，全程用最壞假設，股票層級，78檔
    phantom全部計入最小規模分位）顯示最佳分位從40%翻轉到100%（比值
    9.763→4.181，原本被認為最佳的最小分位worst-case下比值直接歸零）——
    **Q5「20%分位1.662最佳」結論明文撤回**，改寫為「規模門檻在下市
    資料補齊前無法判定」。
  ...
  - **四**：Q1改股票層級（按stock_id+year去重、population_weight加權），
    數字全面高於窗口層級版（符合預期），但排名/週期形狀不變（2008金融
    海嘯仍最低0.21%、2009/2010/2021仍是高峰）。
  - **本輪未做／待補**：fetch_error重抓在額度解除後以背景行程補跑，
    實際恢復結果補記在`research/MULTIBAGGER_ATTRIBUTION.md`對應章節
    （若恢復0檔，需要下一輪查是重抓邏輯本身有bug還是這60檔真的都還是
    抓不到，不能假設是額度問題就不深究）；no_data_found的5檔
    （2398/2512/3001/8710/8714）維持另列、不重抓；82個episode
    （375−293）PS分解仍拿不到營收資料。
  - **未生成任何選股規則**，依總司令原文明令。
