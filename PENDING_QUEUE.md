...
- [!] **金流一.4** **阻塞：等資料累積**——目前法人歷史 **15 個交易日**（20260901~20260923），可用視窗 [1, 5]；**20 日視窗還需 5 個交易日**、**60 日視窗還需 45 個交易日**。
...
- [!] **資料.一** [研究] BLOCKED（FinMind額度/封鎖，預計解除
  2026-09-24 04:21台北時間）——f52w樣本延伸到2007(核准執行)，天條一
  須涵蓋2008，f52w價值主要在低回撤，沒2008無從判定。**進度（第4輪，
  修.三修正後首次執行，2026-09-24 02:07~02:21）**：從驗.四稽核後的
  斷點(121/300，已排除11檔股利快取缺失待重抓)續跑到169/300檔已嘗試
  （140檔可用，29檔資料不足/尚未上市），於[170/300]2450**在factor準備
  階段命中402額度上限，依修.三修正正確停在斷點**（這是修正後的新行為
  ——修.三之前，factor階段的額度錯誤會被靜默吞掉誤記成功，現在正確
  往上拋並停止，跟價格階段命中額度的處理一致）。**本輪factor_warnings
  為0**（沒有非額度類的因子降級案例，48檔新處理的股票因子資料都乾淨
  抓到）。**速度**：本輪48檔/14分鐘（含API節流等待），比修.三之前的
  40-45檔/輪略快，累計169/300，預估還需**2-3輪、約4-6小時**補齊剩餘
  131檔（原168檔+驗.四排入重抓的11檔-本輪已處理的48檔）。checkpoint
  持續累積(`data/f52w_2007_extension_checkpoint.json`)，下次解除後
  重跑腳本自動從斷點續跑。**解除條件**：`data/rate_limit_state.json`
  的`blocked_until`=2026-09-24T04:21台北時間。
...
- [x] **結案.三** [債務] f52w（#385）FAIL結案——**已完成**：
  ...
  註記。
- [!] BLOCKED（hypothesis_queue軌2026-09-23確認）：第2/3點事前登記與
...
- [!] BLOCKED（hypothesis_queue軌2026-09-23確認）：第二部分（開盤到收盤
...
- [ ] **閘門.一** [研究] 開考前資料品質閘門(抓完300檔、跑回測前)——
  ...
  才准執行轉向.一單發檢定。BLOCKED於續抓剩餘168檔完成。
