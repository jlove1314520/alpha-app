...
- [!] **H.一** [研究] （新.一已完成，解除阻塞；`[自行裁量]`自走輪不執行：互動視窗正在做修.四、且單次解鎖只有一次機會，避免兩個行程重複消耗，留給互動視窗）——holdout單次解鎖(總司令已核准，
  ...
  **進度（2026-09-26T00:5x~01:0x UTC，互動視窗CC）**：新增
  `holdout_2025_dividend_account_test.py`。**執行前安全驗證**（全程用
  2007-2014等已核准歷史資料，未觸碰holdout）：①用`uncapped_adjusted_
  price_series("0050",...)`與既有`load_0050_full_history()`比對重疊
  日期——day-over-day報酬差異僅約1e-16（浮點雜訊），2008 MDD兩者算出
  同一數字到小數點後10位，確認uncapped重建演算法正確（絕對價位有固定
  比例差異是anchor點不同的預期結果，不影響任何報酬類統計量）。②
  `simulate_account()`70/30邏輯用既有VAL期(2021-2024)與2007-2014
  dividend equity curve各測一次，數字合理（股票MDD→帳戶MDD依比例縮小，
  含再平衡效應）。③2007-2014參考值(裁示要求的「不作判準」對照)機械
  重現：股票部位MDD=-53.72%（與#399既有值逐位元吻合）、帳戶(70/30)
  報酬+132.43%／MDD-40.40%。**正式執行（holdout解鎖，不可逆）**：
  `holdout.unlock_holdout_once()`已成功呼叫，**holdout現在對整個專案
  永久解鎖**（`HOLDOUT_LOCK.json`/`HOLDOUT_LOG.md`已記錄，reason/
  approved_by皆已載明）。**⚠️但緊接著崩潰**：載入第27檔個股(3467)
  uncapped股價時撞上FinMind額度上限(HTTP 402)，`finmind_client.py`的
  `_throttle()`依「資料源禮儀」規則正確拒絕發送請求並RuntimeError，
  腳本未捕捉此例外而崩潰。**崩潰發生在任何H.一正式結果數字被印出之前**
  （當時只完成了安全的參考值計算，尚未進入正式股票部位/帳戶回測），
  依裁示規則3「崩潰前無結果數字，可修bug後重跑，須記錄崩潰原因與修正
  內容」——已記錄於`TRIALS_LEDGER.md`#400附加段落。**已完成的修正**：
  `unlock_holdout_once()`呼叫改為條件式(`is_holdout_consumed()`為False
  才呼叫)，因為holdout已經全域解鎖，重跑時不能也不需要再呼叫一次；
  資料載入/回測邏輯本身不變（那段邏輯本身不是崩潰原因，FinMind額度
  是外部限制不是bug）。**BLOCKED**：FinMind額度冷卻至
  **2026-09-26T11:01:19台北**才能續跑（`data/rate_limit_state.json`
  確認），依「資料源禮儀」規則不提前重試、不排隊。**下一步**：冷卻
  解除後重跑`python research/holdout_2025_dividend_account_test.py`
  （會直接跳過已完成的unlock步驟，從資料載入接續），完成後才會印出
  H.一正式結果數字（IR/MDD/PASS-FAIL），屆時視為已消耗only-once資格。
  在此之前先接續處理登記.二（不需要FinMind，不受此阻塞影響）。
  **第二次執行（2026-09-26T11:03~11:22，互動視窗CC）**：冷卻於11:01:19
  解除後重跑，`finmind_client.py::_fetch()`的parquet快取機制確認有效
  （前一次成功抓過的股票這次直接吃快取，不重新發送請求）——但251檔
  股票需要約500次FinMind呼叫，單一冷卻周期內的額度不夠跑完全部，
  進度到約50~70檔新增股票後**再次撞上FinMind額度上限而崩潰**（同一種
  崩潰模式，非新bug，一樣發生在任何H.一正式結果數字印出之前，只是安全
  的參考值計算完成而已）。**驗證：本機uncapped價格快取檔案數已累積到
  73檔股價／60檔股利**（`research/data/*.parquet`，`__2024-01-01__
  latest.parquet`結尾的檔案，250檔目標的約29%），確認快取機制正確
  跨執行累積進度、每次重跑不會浪費額度重抓已成功的股票。**新的BLOCKED**：
  FinMind額度再次冷卻至**2026-09-26T13:21:37台北**（`data/rate_limit_
  state.json`確認）。**如實評估**：以目前速度(每個冷卻周期約新增
  50~70檔)，完整跑完251檔可能需要**再2~4個冷卻周期(約4~8小時)**，這是
  FinMind免費額度的真實限制，不是bug，也不打算為了加速而違反「資料源
  禮儀」規則（不換UA、不繞過、不提前重試、不平行多開行程搶額度）。
  下一步：冷卻解除後（13:21:37台北+緩衝）繼續重跑同一支腳本，重複這個
  「重跑→累積進度→再次冷卻」的循環直到251檔全部載入完成，才會進入
  正式回測與結果印出階段。
  **第三次執行（2026-09-26T13:23~13:41）**：同一種崩潰模式，快取累積至
  108/251檔股價、93檔股利。新冷卻至**2026-09-26T15:41:21台北**。持續
  依「資料源禮儀」規則循環重跑，不加速、不繞過。
  **第四次執行（2026-09-26T15:43~16:01）**：同一種崩潰模式，快取累積至
  144/251檔股價、128檔股利。新冷卻至**2026-09-26T18:01:47台北**。
  **第五次執行（2026-09-26T18:03~18:20）**：同一種崩潰模式（卡在2330以外
  的第9934檔股價載入），快取累積至**186/251檔股價**。新冷卻至
  **2026-09-26T20:20:32台北**。持續依「資料源禮儀」循環重跑。
  **第六次執行（2026-09-26T20:20~20:21）**：同一種崩潰模式（卡在第2809
  檔股價載入），快取累積至**226/251檔股價**，只剩約25檔。新冷卻至
  **2026-09-26T22:40:41台北**。持續依「資料源禮儀」循環重跑。
...
- [x] **登記.二** [情報] 主動式ETF每日持股，只登記只提案不執行不回測——
  **完成（2026-09-26，DevQueue cycle 20260926-084601，情報帽）**：產出
  `docs/ACTIVE_ETF_HOLDINGS_PROPOSAL.md`（純文件，無程式碼變更、未抓任何持股資料）。
  ①**查證結論**：TWSE主動式ETF頁、TWSE OpenAPI swagger（143端點，僅`/ETFReport/ETFRank`
  與ETF有關，無持股/成分股/PCF）、TPEx主動式ETF頁**三者都沒有每日持股**；第三方彙整站/GitHub
  封存庫存在（第4類來源）反推原始來源在各投信官網。**官方原始層（投信官網）有，但七家中沒有
  任何一家被確認允許程式讀取**：TWSE條款第6條明禁自動化下載（政府資料開放平臺授權資料除外）；
  群益/中信/第一金robots為Allow:/；台新僅見Googlebot段；統一robots取不到（302自我迴圈）、
  野村/復華連線失敗、TPEx robots被WAF 302擋（**未換UA繞過**）；七家網站條款皆未親自讀完。
  →**判定「合規未確認」，非「資料不存在」，目前無合規可抓來源**。②App三項提案（持有此股的
  主動ETF與加減碼、共識價帶須標VWAP推估、公司行動還原前置）寫在文件第2節，資料源合規前全不開工。
  ③研究假設「一週內≥3檔主動ETF共同加碼」已登記於文件第3節，含事前預登記要點；**未進
  `TRIALS_REGISTRY.jsonl`／`HYPOTHESIS_QUEUE.md`可執行區**（凍結.二＋holdout：2025-05起資料
  全在holdout內，禁回測，只能前瞻收集）。④EPS預估不列入。
  **[自行裁量]**：假設不放進HYPOTHESIS_QUEUE避免自走軌道誤撿；robots被擋一律不繞過；
  條款多數來自搜尋摘要/小模型摘要，已在文件第5節揭露非逐字原文。
  **待總司令裁示（已列入`research/AWAITING_REVIEW.md`）**：①採書面詢問投信或人工下載？
  ②是否授權純查證data.gov.tw（第5個獨立來源）？**心跳**：本條標`- [x]`＋文件存在。
  **原文如下（保留）**：
  ...
  待修.四~H.一完成後接續**。
  **互動視窗CC查核註記（2026-09-26，完成修.四~H.一後接續本項時發現）**：
  DevQueue已完整完成（比我原本準備做的更嚴謹——實際查了7家投信的
  robots.txt並讀出TWSE ToS第6條明禁自動化下載，結論「合規未確認」比
  我原本只抽查一家的結論更精確），不重複做。唯一的介入：發現DevQueue
  刻意不把研究假設放進`HYPOTHESIS_QUEUE.md`（避免自走軌道誤撿）——
  互動視窗一度手滑加了一條`#83`進去，發現這個衝突後已移除並在原處
  留一句話說明去向，避免製造DevQueue刻意迴避的風險。`AWAITING_
  REVIEW.md`已有DevQueue登記的2項待裁示（書面詢問投信 vs 人工下載；
  是否授權查證data.gov.tw），無需互動視窗重複處理。

## 2026-09-26 總司令裁示【資料源.主動ETF：各投信官網每日持股（凍結.二 允許範圍：資料抓取）】（原文登記）

> 【資料源.主動ETF：各投信官網每日持股（凍結.二 允許範圍：資料抓取）】
> 依證交所《主動式ETF懶人包》：每營業日於淨值結算後，於「投信公司網站」揭露當日
> 實際投資組合。不得使用 etfshenteam 或任何第三方彙整站（其條款禁止抓取，
> 且主要功能有登入與用量限制）。
>
> 1. 清單：從 TWSE、TPEx 官方主動式 ETF 清單列出全部代號、名稱、發行投信。
> 2. 逐家投信查證（每家一列表格）：
>    - 每日持股揭露頁網址、格式（HTML／Excel／CSV／JSON）
>    - 是否能查詢歷史日期（能的話可以回溯多久）
>    - robots.txt 內容、網站使用條款中與自動讀取有關的條文
>    - 判定：允許／不明／禁止。不明的一律視同禁止，另外列出可行替代方案
>    已知初查：群益 robots 為 Allow: /；野村與統一尚無法判讀。
> 3. 只對「允許」的來源建立每日收集器：
>    - 每日一次，收盤淨值公布後執行；每家之間間隔數秒；帶可識別的 User-Agent
>    - 原始回應原樣存檔；交易日一律取自資料本身，不得用執行日
>      （避免重蹈 daily_price 的錯誤）
>    - 比對持股變化前，先做除權息／分割還原
> 4. 研究限制：2025-05 以來的歷史落在 holdout 期間，一律不得回測；
>    只做前瞻收集與紙上追蹤。
> 5. 完成後回報查證表，收集器先跑 5 個交易日並附抽樣核對，再納入排程。

- [x] **資料源.主動ETF** [維運] 依證交所《主動式ETF懶人包》：每投信須
  每營業日淨值結算後於官網揭露當日實際投資組合，不得用第三方彙整站。
  ①從TWSE/TPEx官方清單列全部代號/名稱/發行投信。②逐家投信查證(每家
  一列表格)：每日持股揭露頁網址/格式(HTML/Excel/CSV/JSON)、能否查歷史
  日期(可回溯多久)、robots.txt內容、網站使用條款自動讀取相關條文，
  判定允許/不明/禁止(不明視同禁止並列替代方案)。已知初查：群益robots
  為Allow:/，野村與統一尚無法判讀。③只對「允許」來源建每日收集器：
  每日一次收盤淨值公布後執行、每家間隔數秒、帶可識別User-Agent、原始
  回應原樣存檔、交易日一律取自資料本身不得用執行日(避免重蹈daily_price
  錯誤)、比對持股變化前先做除權息/分割還原。④2025-05以來歷史落在
  holdout期，一律不得回測，只做前瞻收集與紙上追蹤。⑤完成後回報查證表，
  收集器先跑5個交易日並附抽樣核對，再納入排程。
  **完成（2026-09-26T10:1x+08:00，互動視窗CC；⚠️中途一度誤用「fork」
  委派這項研究、只回報狀態沒有實際產出文件，經總司令當場指正後撤回
  委派、由本session直接執行完成，如實記錄這個過程不掩蓋）**：
  ①**完整清單已取得**——直接查TWSE官方ISIN系統(`isin.twse.com.tw/
  isin/C_public.jsp?strMode=2`上市／`strMode=4`上櫃)，比DevQueue先前
  只查頁面說明更進一步拿到實際資料表：**上市32檔＋上櫃7檔＝共39檔
  主動式ETF**，去重後**16家發行投信**（國泰/摩根/安聯/統一/聯博/富邦/
  中信/第一金/復華/永豐/野村/台新/元大/群益/兆豐/貝萊德），比DevQueue
  先前只查過的7家多出9家。②**逐家查證16家完成**，直接curl/WebFetch
  讀robots.txt原文與官網ToS頁面（不經第三方摘要），完整表格與16家
  robots.txt實測內容/ToS原文摘錄見`docs/ACTIVE_ETF_ISSUER_
  VERIFICATION.md`。**結論：16家全數判定「不明/禁止」，0家「允許」**
  ——其中安聯、第一金兩家**確認讀到ToS原文**明文「非經書面授權同意，
  不得以任何形式轉載、傳輸、傳播、散布、展示、出版、再製或利用」，
  連robots.txt允許都不能凌駕這條著作權限制；其餘因robots.txt本身
  不明(無`User-agent: *`規則/連線失敗/SPA無法讀取/ToS未讀到原文)而
  視同禁止。③**收集器不建置**——依裁示「只對允許來源建收集器」，
  0家允許，第3步驟(每日收集器/5日試跑)**不執行**。④提案（待總司令
  裁示，本輪不做）：書面詢問投信、總司令人工每日下載、查data.gov.tw、
  或鎖定1-2家(貝萊德/聯博的robots條件相對寬鬆)優先發函詢問。
  **已知侷限**：多數投信官網是JS渲染SPA，本次工具讀不到實際內容；
  摩根/貝萊德/聯博等全球性機構的robots多為全球共用版本，TW專屬子網域
  部分未單獨確認；第一金`llms.txt`、群益其他子路徑條款頁未讀。
  **⚠️ 上面這段（互動視窗初版）已過時，現行結論見下方「DevQueue v2 更正」與
  `docs/ACTIVE_ETF_ISSUER_VERIFICATION.md` v2**（舊文字保留供稽核）。
  **DevQueue v2 更正（2026-09-26 10:2x，cycle 20260926-094601，[維運帽]）**：
  互動視窗初版寫完的同時，DevQueue 已派 4 個子任務並行查證，整合成 v2 並
  覆蓋同一份文件。**更正六點**：①**檔數 39→40**：重抓 ISIN 並用程式計數，
  上市33＋上櫃7＝40（代號清單無誤，是加總寫錯）。②**第一金**：初版引用的
  `www.fsitc.com` 是 GoDaddy 待售網域、不是第一金官網；官網 `www.fsitc.com.tw`
  的 robots 是 404 頁＋Imperva 痕跡；ToS 原文有「用以賺取利益」限定語。判定仍
  不明→禁止，但理由改寫。③**「0家允許」不成立**：**富邦**持股頁所在主機
  `websys.fsit.com.tw` 的 robots 對 `/FubonETF` **明確 Allow**、`ddate` 參數可查
  歷史（實測至 2026/06/15）、無網站專屬使用條款禁止程式讀取，判「**有條件允許**」
  （殘餘不確定性：投信首頁一句「請勿重製、轉載」範圍寫「本文內容」；只涵蓋 3 檔
  00405A/00982D/00983D）。④**條款明文禁止 5 家**（讀到原文）：摩根(明禁機器人/
  爬蟲)、安聯、中信、永豐、貝萊德；不明→禁止 10 家。⑤初版「持股頁 16 家全未找到」
  → 本輪找到 15 家持股/PCF 頁網址（貝萊德被 Akamai 403 擋下停手）。⑥國泰/統一
  robots 結果與初版不同（見 v2 第 4 節）。**證據**：v2 第 2 節逐家原文引用＋各
  URL；未驗證項目全列於 v2 第 7 節。**[自行裁量]**：富邦雖符合「robots 明確允許
  且未讀到禁止條文」，但因殘餘不確定性屬法遵疑慮（白名單第6條），**本輪不建收集器**
  （裁示第3點）；已登記到 `research/AWAITING_REVIEW.md` 等總司令二選一：
  ①授權富邦3檔收集器（每日1次、間隔≥3秒、誠實UA、原始回應存檔、日期取自頁面資料）；
  ②先書面詢問富邦投信。**心跳**：本條目維持 `- [x]`（查證①②⑤已完成），③收集器
  的解除條件＝總司令裁示，屆時另開 `- [ ]`。**子任務用量**：4 個並行子任務。

## 2026-09-26 總司令裁示【UX.一：App 介面改版提案（只借鑑概念，只提案＋原型，不實作登入／金流）】（原文登記）

> 【UX.一：App 介面改版提案（只借鑑概念，只提案＋原型，不實作登入／金流）】
> 參考 etfshenteam.com 的「概念」；其版面、程式碼、圖表呈現受著作權保護，
> 不得複製，只能學習設計原則，用我們自己的視覺語言重做。
>
> Cowork 已查證的對方做法（供參考）：Cloudflare Pages＋D1；伺服器端 Google OAuth；
> 會員分級＋限時免費試用；Web Push；9 步驟新手導覽；IntersectionObserver 捲動淡入；
> requestAnimationFrame 數字跳動；手繪 SVG 圖表；系統字型；無前端框架。
>
> 1. 現況盤點：我們 App 各頁面的截圖（手機寬度），列出每一頁「想回答使用者什麼問題」。
>    答不出來的頁面標註出來。
> 2. 首頁故事化提案：一句話價值主張；用一檔真實個股，示範如何一層一層看懂我們最核心
>    的那張圖；首屏直接放真實資料（今日重點），不要空殼。
> 3. 設計規範：色彩、字級、間距、圓角、陰影，定成 CSS 變數；同時支援深色與淺色模式。
> 4. 互動與動畫：捲動淡入、數字跳動、載入中與無資料狀態的畫面、新手導覽（可跳過、
>    記住看過沒）。必須尊重使用者的「減少動態效果」系統設定。
> 5. 方法論頁：每個分數怎麼算、資料從哪來、更新時間、已知限制。
>    比照「股票股利不是加碼」的寫法，先回答內行人會質疑的地方。
> 6. 用語審查：全 App 中所有可能被解讀為「推介建議」的文字（買進、推薦、必漲等），
>    列出清單並提出中性替代說法。
> 7. 產出：提案文件＋一個可開啟的原型頁（不影響現行 App），等總司令看過再決定實作。
>
> 不在本次範圍：Google 登入、推播、會員、金流。
> 收費相關一律暫緩，須先經律師確認《證券投資信託及顧問法》第 4 條、第 107 條的適用範圍。

- [x] **UX.一** [開發]（2026-09-26 已交付提案＋原型，等總司令審閱，見`docs/UX_REDESIGN_PROPOSAL.md`／`prototype/ux_v2.html`；未實作index.html） App介面改版提案，只借鑑概念(etfshenteam版面/程式碼
  受著作權保護不得複製)，只提案+原型不實作登入/金流——①現況盤點：各頁面
  手機寬度截圖，列每頁想回答使用者什麼問題，答不出來的標註。②首頁故事化：
  一句話價值主張+用一檔真實個股示範核心圖表+首屏放真實資料非空殼。③設計
  規範：色彩/字級/間距/圓角/陰影定CSS變數，同時支援深色淺色模式。④互動
  動畫：捲動淡入/數字跳動/載入中與無資料畫面/新手導覽(可跳過記住看過)，
  尊重prefers-reduced-motion。⑤方法論頁：每個分數怎麼算/資料來源/更新
  時間/已知限制，比照「股票股利不是加碼」寫法先答內行人會質疑處。⑥用語
  審查：全App「買進/推薦/必漲」等推介文字列清單+中性替代說法。⑦產出
  提案文件+可開啟原型頁(不影響現行App)。不含Google登入/推播/會員/金流；
  收費相關暫緩，須先經律師確認證券投資信託及顧問法第4/107條適用範圍。

## 2026-09-26 總司令裁示【holdout 防護收窄＋停擺告警＋資料信賴度標示＋主動 ETF 條款查證】（原文登記，插隊，依三之一節排隊，UX.一先開工者優先做完手上段落再處理，不影響H.一重跑）

> 【裁示：holdout 防護收窄＋停擺告警＋資料信賴度標示＋主動 ETF 條款查證】
> 先寫進 PENDING_QUEUE 再動工。全程繁體中文。不影響 H.一 重跑。
>
> ────────────────────────────────
> 守.一　holdout 防護收窄（總司令核准的是「僅限 #400」，不是全專案）
> ────────────────────────────────
> 現況：unlock 後 assert_no_holdout_leakage() 全域變成 no-op。
> 1. 改為允許清單：只有 holdout_2025_dividend_account_test.py（#400）可以使用
>    VAL_END 之後的資料；其他任何腳本碰到 VAL_END 之後的資料一律照舊 raise。
> 2. 自我測試：#400 腳本通過、任一其他研究腳本讀到 2025 年資料必須 raise。
> 3. 註記：往後新研究已無回測用的乾淨資料，唯一的樣本外檢定是紙上交易（往前）。
>
> ────────────────────────────────
> 警.一　本機停擺告警（核准 E 的方向，做法如下，不新增任何秘密）
> ────────────────────────────────
> push 被卡住時本機推不出任何東西，所以偵測必須放在雲端。
> 1. 新增 GitHub Actions 排程（每 30 分鐘）：檢查最近一筆本機排程 commit
>    （IBKR quotes／DevQueue）的時間；在應該運作的時段超過 60 分鐘沒有新 commit，
>    就讓這個 job 失敗。GitHub 預設會寄信通知 repo 擁有者，不需要任何 token。
> 2. 同時在 status.json 寫入「本機排程最後活動時間」，App 狀態列顯示。
> 3. 另查：結案.一 的 git 鎖是否已裝進三支 wrapper，以及每晚寫 audit_report.json
>    的排程是否也走同一把鎖；沒有的話列出修正方案，等總司令裁示。
>
> ────────────────────────────────
> 標.一　三大法人資料信賴度標示
> ────────────────────────────────
> 修.四 修復的 inst_trades 在 2026-08-21~09-26 屬「中信賴度（交叉推論）」。
> App 顯示這段期間的三大法人資料時，加註「此期間資料經修復，僅供參考」。
> 研究上若有任何試驗會用到這段資料，一律排除。
>
> ────────────────────────────────
> 源.二　主動 ETF 條款逐字查證（只讀條款，不抓持股）
> ────────────────────────────────
> 1. 群益、中信、第一金：逐字讀網站使用條款中與程式讀取、重製、轉載有關的條文；
>    第一金另讀 /llms.txt。引用原文。
> 2. 查 data.gov.tw 是否有主動式 ETF 每日持股的開放資料集。
> 3. 草擬一封「詢問是否同意以程式每日讀取持股揭露頁」的信件範本，
...
- [x] **源.二** [情報] 主動ETF條款逐字查證(只讀條款不抓持股)——①群益/
  ...
  集。③草擬「詢問是否同意程式每日讀取持股揭露頁」信件範本，供總司令
  自己寄給投信客服，CC不得代寄。④條款明文允許或未禁止且robots允許者，
  才可依原裁示建收集器。
  **完成（marathon自走軌，2026-09-26 16:3x–16:5x，情報帽；本輪未碰警.一/
  標.一，留給互動視窗）**：①**已完成**——今日重抓原始回應：群益robots
  200 `Allow: /`；中信robots 200 `Allow: /`；第一金`/robots.txt`與
  **`/llms.txt`皆200但body是站內404錯誤頁**（＝兩者都不存在）。群益
  sitemap(2,815網址)中再讀`/capital/statement`、`/capital/other/protect`
  兩頁，皆未見重製/轉載/自動讀取條文（僅©聲明；WebFetch小模型判讀，非
  逐字元核對）。中信/第一金條款原文沿用v2已逐字讀到的版本，**今日未重讀**。
  ②**部分完成，不得下「不存在」結論**——證交所OpenAPI(143端點)與櫃買
  OpenAPI(225端點)swagger逐端點比對：**兩者皆無主動式ETF持股/PCF端點**
  （僅ETFRank、外資持股、開放式基金造市商等無關項）；data.gov.tw站內搜尋
  是Nuxt前端渲染、內部目錄API的關鍵字參數探測4組×6個參數名皆未生效
  （回全站52,437筆），**已停止猜測**未公開端點；WebSearch(限data.gov.tw)
  未見任何主動式ETF持股資料集。GitHub端見多個公開專案自述爬各投信PCF，
  證明技術可行但**不構成授權**。③**已完成**——`docs/ACTIVE_ETF_INQUIRY_
  LETTER_TEMPLATE.md`（欄位以說明文字框起，無看似真值的佔位符；CC不代寄）。
  ④**判斷：本輪不建收集器**`[自行裁量]`——群益是唯一同時「robots明確
  Allow＋已讀條款頁未見禁止」者，但(a)條款頁只有小模型判讀、(b)持股資料
  端點未確認、要先讀其JS bundle找`CFWeb` API（摸內部API應先經總司令
  點頭）、(c)屬白名單第6條法遵疑慮，處理比照富邦「有條件允許」；中信
  （條款原文禁止）、第一金（無robots、條款須書面授權）不符第4點。
  完整內容見`docs/ACTIVE_ETF_ISSUER_VERIFICATION.md`第8節。**等總司令裁示**
  （已登記`research/AWAITING_REVIEW.md`）：①是否授權群益收集器（需先讀
  JS找端點）②是否寄詢問信（範本已備）③是否在瀏覽器手動搜尋data.gov.tw
  一次補上站內檢索缺口。**心跳**：本條目維持`- [x]`；收集器解除條件＝
  總司令裁示，屆時另開`- [ ]`。**子任務用量**：0個子任務，WebFetch 4次、
  curl約12次（皆單次請求、間隔≥3秒）。

**執行順序**：守.一(緊急，自走軌道正在背景運作，需盡快收窄holdout範圍
避免其他腳本意外讀到2025+資料) → 警.一/標.一/源.二 → UX.一(規模最大，
最後處理)。四段回報格式照舊。

## 2026-09-26 總司令裁示【緊急：H.一 立即暫停重跑，修正兩個 bug 後先做乾跑檢查】（原文登記，插隊，最高優先，先於其他所有工作）

> 【緊急：H.一 立即暫停重跑，修正兩個 bug 後先做乾跑檢查】
> 最高優先，先於其他所有工作。下一次冷卻（22:40）前必須生效。
>
> ────────────────────────────────
> 停.一　立即停止 H.一 的一切自動重跑
> ────────────────────────────────
> 不論哪個軌道或排程，在本裁示完成並經 Cowork 核對之前，
> 不得執行 holdout_2025_dividend_account_test.py 的正式回測段落。
>
> ────────────────────────────────
> 證.一　先印證據（只印資料診斷，不得產生任何策略結果數字）
> ────────────────────────────────
> Cowork 讀碼推論：
> ① factors.py::_dividend_yield_ttm_cash() 用 load_dev()，會截在 2024-12-31；
>    holdout 腳本沒有替換它 → 2025 年以後的股利事件不會進入訊號。
> ② 載入迴圈 except Exception 會把 prepare_factors() 往上拋的額度錯誤當成
>    一般失敗吞掉。
> 請用已快取的資料，對 5 檔 2025 年有除息的普通股，印出：
> - 因子資料裡 ttm_cash_dividend 最後一筆 pit_date
> - 不截斷的 TaiwanStockDividend 裡最新的除息日
> 兩者若不一致，即證實①。證據寫進 PENDING_QUEUE。
>
> ────────────────────────────────
> 修.五　修正（修完才能繼續）
> ────────────────────────────────
> 1. 把 _dividend_yield_ttm_cash 的計算邏輯抽成純函式（輸入股利 DataFrame），
>    考.一 路徑和 holdout 路徑共用同一份邏輯；holdout 路徑改用
>    load_full_history(allow_holdout=True) 取得的股利資料。
>    自我測試：用 2007-2014 資料重跑，#399 的股票部位報酬與 MDD 必須逐位元相同。
> 2. 逐一檢查 dividend 策略的 signal_fn 用到的所有輸入（股利率、流動性、產業、
>    資格篩選），列出每一項的資料來源與是否截斷。任何一項截斷都要比照修正。
> 3. 載入迴圈：額度類錯誤一律往上拋並中止，不得計入 n_factor_fail。
>
> ────────────────────────────────
> 乾.一　乾跑檢查（新增 --gates-only 模式：載入資料＋檢查，印完就結束，不跑回測）
> ────────────────────────────────
> 只允許印出計數與日期，不得印出任何報酬、IR、MDD：
> G1 額度相關失敗 = 0
> G2 股利新鮮度：凡是 2025 年後有除息紀錄的可用股票，因子最後 pit_date 必須
>    ≥ 該除息日；不一致檔數必須 = 0
...
- [!] **乾.一** [研究] 新增--gates-only模式(載入資料+檢查，印完即結束，
  ...
  除息日，不一致檔數須=0) G3可用檔數/價格失敗檔數/因子失敗檔數(附
  失敗原因分類) G4資料實際涵蓋到的最新交易日。結果寫進research/data/
  h1_gates.json並git add -f入庫，然後停下，等Cowork核對後才放行正式
  回測。
  **--gates-only模式已實作完成（見修.五提交），第一次實跑BLOCKED**
  （互動視窗，2026-09-26）：`python research/holdout_2025_dividend_
  account_test.py --gates-only`跑到第216/251檔(1452)時，因子計算內
  `_margin_utilization()`(與股利率因子無關的另一個因子，`prepare_
  factors()`會算全部因子庫，不只composite用到的那個)呼叫`load_dev(
  "TaiwanStockMarginPurchaseShortSale",...)`撞上FinMind冷卻中
  （blocked_until 2026-09-26T22:40:41台北，此為既有冷卻視窗，非新增）。
  **重要驗證**：這次崩潰的行為本身就是修.五第3點修正是否生效的直接
  證據——額度錯誤正確地`raise`往上拋、整支腳本立即中止（Traceback可見
  `_is_quota_error`判斷後的raise路徑），**沒有被靜默吞掉繼續跑**，
  跟修.五之前的行為（吞掉、繼續、n_factor_fail+=1）形成對照，證實
  bug③修正正確運作。崩潰發生在任何G1~G4輸出之前（`h1_gates.json`
  未產生），依裁示規則視為「產出結果前崩潰」，可等冷卻後重跑，不違反
  停.一。**附帶發現（如實記錄，非本次修正範圍，供總司令/Cowork參考）**：
  `prepare_factors()`會計算完整因子庫（含margin/short-sale/法人等
  dividend_yield_portfolio_v1完全用不到的因子），這可能是H.一過去
  六輪都在額度牆前提早撞牆的部分原因——只用單一因子的策略卻付出全
  因子庫的額度成本。**未在本次裁示範圍內處理**（裁示只要求修股利率
  PIT延遲bug與額度吞噬bug，不含因子計算範圍優化），如需優化需另行
  裁示。**下一步**：等冷卻解除（22:40台北）後重跑`--gates-only`，
  目標是讓G1~G4輸出真正產出並寫入`h1_gates.json`。

**執行順序**：停.一(立即，取消自動重跑) → 證.一(先印證據，已完成) →
...
- [!] **乾.二** [研究] 補強乾.一的G2閘門，避免「即使patch沒生效G2仍會是0」
  ...
  PASS紙.一前須另開試驗處理。
  **①②③④⑤⑧已完成（互動視窗，2026-09-26）**：程式碼實作與TRIALS_
  LEDGER附註皆已commit+push（commit bbafa63a）。G2a/G2b/G2c/G5四層檢查
  邏輯：G2b取`d`中第一個`date>=E`的列算`obs=f_dividend_yield_ttm×close`
  跟uncapped事件本身的`X`比對（不是重新呼叫同一個monkeypatch函式，是
  真的檢查`prepare_factors()`吐出來的資料）；G2c比對同一檔在VAL_END前
  最後交易日的ttm值是否與X不同，=0時`gates_pass`強制False；G5用第4步
  已算好的`all_dates`聯集交易日曆找「period_end往前10個觀察到的交易日」
  當門檻。**⑥⑦尚未執行**：FinMind冷卻至22:40台北才可執行`--gates-only`，
  尚未到時間，已排程等冷卻解除後重跑。
  **⑥⑦已完成（互動視窗，2026-09-26 22:45~23:02）**：冷卻解除後重跑
  `python research/holdout_2025_dividend_account_test.py --gates-only`
  **本次完整跑完，未再撞額度牆**（`G1_quota_related_failures=0`）。
  結果（`research/data/h1_gates.json`，已`git add -f`入庫）：
  - **G1**=0（無額度錯誤）
  - **G2a**（快取新鮮度）：檢查166檔有2025+除息紀錄的可用股票，**1檔
    不一致**——`2330`（台積電）最新除息日2026-09-16，但其股價最後
    快取日僅到2026-08-21（股價資料本身落後於除息日，不是修.五的bug，
    是這檔的uncapped股價快取還沒補到9月）。
  - **G2b**（patch生效檢查，**須=0**）：**3檔不一致**——`7758`
    （E=2026-07-23,X=0.5,obs=null）、`6539`（E=2026-08-27,X=1.6,
    obs=null）、`3531`（E=2026-06-09,X=1.3,obs=null）。三檔的`obs`
    皆為null，代表`d`中第一個`date>=E`的列，`f_dividend_yield_ttm`
    或`close`是NaN。互動視窗做了讀專用快取的初步排查（零新增API
    呼叫）：三檔在uncapped股價序列裡本身都有涵蓋到E之後的資料
    （7758共341筆、2025-05-08起；6539/3531各656/662筆、2024-01-02
    起），**排除「完全沒有股價資料」這個最簡單的解釋**，但尚未深入
    查出`f_dividend_yield_ttm`具體在哪個環節變成NaN——**這是未解決
    的疑點，如實記錄不猜測結論，留給Cowork核對時一併查**。
  - **G2c**（檢定力對照，**須>0**）：156檔（>0，**通過**——證明G2b
    這個檢查本身有辨別力，不是空包彈）。
  - **G3**：可用檔數=214、價格失敗檔數=37、因子失敗檔數=0。
  - **G4**：資料實際涵蓋到的最新交易日=2026-09-24。
  - **G5**（股價截止日分佈，**須≤5%**）：最小2025-09-17、中位數
    2026-09-24、最大2026-09-24，10個交易日前門檻日=2026-09-10，
    落後檔數4/214（**1.9%，通過**）——落後清單含`6806`/`3454`/
    `2809`/`2330`（後者與G2a抓到的同一檔）。
  - **`gates_pass = False`**（因G2a=1、G2b=3皆須=0而未過；G2c=156、
    G5=1.9%皆已通過）。
  **依裁示七、跑完即停，未印任何報酬/IR/MDD，未接著跑正式回測。
  H.一正式回測維持停.一的暫停狀態，等Cowork核對`h1_gates.json`與上述
  G2a/G2b疑點後才可能另行放行。**
