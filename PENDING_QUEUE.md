...
- [ ] **重構.C** [研究] 基準相對傾斜（core_tilt）SPEC——年化追蹤誤差
  ...
  雙軌判定（CAPM alpha + block bootstrap相對0050超額報酬），SPEC寫完
  先給總司令看，不先回測（見續2裁示原文階段二）。
  **進度更新（假設佇列自走，2026-09-18）**：✅ **SPEC已寫完，等待總
  司令核准**（本項維持`- [ ]`不改`[x]`，依SPEC文件自己約定的規則——
  「核准後才把這行改`[x]`」）：`research/CORE_TILT_SPEC.md`——對標
  0050（沿用`adjusted_price_series("0050",...)`）、因子成分沿用
  `PORTFOLIO_STRATEGY_SPEC.md`版本A（eps_family/revenue_surprise/
  low_vol共3獨立成分）、市值加權為底+因子傾斜（新增`tilt_strength`
  自由參數）、持股60~80檔動態、產業中性±3個百分點容忍帶、beta
  0.95~1.05約束、單檔權重上限5%、季頻換倉全成本、自由參數共4個
  （低於七之三節第11關5個上限）、內建`MARATHON_PROTOCOL.md`1a-0c節
  牛熊制度三項強制要求（分制度表/INSUFFICIENT_REGIME/空頭beta拆解）、
  雙軌判定（CAPM alpha照報+block bootstrap相對0050超額報酬為主判定）、
  新增(d)市值加權基準對照組拆解因子傾斜貢獻。文末列3個待總司令核准
  的問題（傾斜強度掃描範圍/容忍帶與權重上限數字/是否核准進入回測）。
  **本輪只寫SPEC，未寫任何回測程式碼、未跑任何回測、未動holdout**。
