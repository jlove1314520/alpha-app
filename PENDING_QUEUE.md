...
- [x] **原子.二** [研究] ✅查證＋腳本＋300檔全量執行皆已完成，
  ...
  **分支結論：暫不下判定**——規格要求的「分年份與分牛熊段穩定度」
  拆解尚未完成（現有腳本只算TRAIN/VAL兩段聚合IC），這是原子.二本身
  未完成的最後一步，不是新假設，下一步直接擴充`atom_ic_map.py`的
  聚合邏輯補上（不需重新載入300檔資料，理論上比本輪資料載入階段
  快很多）完成後才能對候選訊號叢集下真正的進/停判定。
- [!] **原子.三** [研究] BLOCKED：依賴原子.二完成分年/牛熊段拆解
  （原子.二本身資料已跑完，只差最後聚合分析，見上方條目「分支結論」
  段落；間接依賴原子.一
