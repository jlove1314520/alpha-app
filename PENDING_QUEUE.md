...
- [ ] **審.二** [研究] f52w用資訊比率重審——待尺.二完成後開始。1.算
  ...
  事前宣告：可比試驗V點估計算出DSR>=0.95才算通過，敏感度表僅參考不
  得挑有利格。3.全期逐年表：f52w vs 0050含息總報酬逐年報酬/逐年MDD。
  4.真正集中度檢驗：電子+半導體合計上限50%名額重跑VAL，原「每產業
  最多5檔」對照從未觸發不算數。5.完成後附一頁摘要回AWAITING_REVIEW。
- [ ] **資料.一** [研究] f52w樣本延伸到2007(核准執行)——天條一須涵蓋
  2008，f52w價值主要在低回撤，沒2008無從判定。1.延伸樣本須含2007-
  2008年間下市股票(不得存活者偏差)。2.只用官方API守速率限制，遇402
  或配額上限停斷點續跑不得繞過。3.完成後2007-2014視為新樣本外，只
  跑一次，參數不得再調。可與尺.二並行(背景抓資料)。
