...
- [!] **資料.一** [研究] BLOCKED（FinMind額度/封鎖，預計解除
  2026-09-24 06:37台北時間）——f52w樣本延伸到2007(核准執行)，天條一
  須涵蓋2008，f52w價值主要在低回撤，沒2008無從判定。**進度（第5輪，
  2026-09-24 04:22~04:37）**：從169/300續跑到222/300檔已嘗試（181檔
  可用，41檔資料不足/尚未上市），於[223/300]1435再次在factor準備階段
  命中402額度上限，依修.三修正正確停在斷點。**本輪factor_warnings仍為
  0**（累計兩輪修正後執行皆無非額度類因子降級案例）。**速度**：本輪
  53檔/約15分鐘，累計222/300，剩餘78檔，預估還需**約1.5輪、2-3小時**
  即可補齊全部300檔（含驗.四排入重抓的11檔）。checkpoint持續累積
  (`data/f52w_2007_extension_checkpoint.json`)，下次解除後重跑腳本
  自動從斷點續跑。**解除條件**：`data/rate_limit_state.json`的
  `blocked_until`=2026-09-24T06:37台北時間。
